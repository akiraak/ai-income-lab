...
AI を使って収入を稼ぐ方法を探索・検証するためのプロジェクト。

アイデア出しだけで終わらせず、「試す → 結果を記録する → 続けるか捨てるか判断する」というサイクルを回すことを目的とする。

## 進め方

- 収益化のアイデアを洗い出し、実際に小さく試す
- 各施策について、かかったコスト（時間・API 利用料など）と得られた収入を記録する
- 結果が出た施策は伸ばし、出なかった施策は理由を残して打ち切る

## Claude への依頼方針

- アイデアを出すときは、必要な作業量・初期コスト・収益化までの想定期間もあわせて示す
- 数値（収入、費用、期間）を書くときは、実測値か推測値かを必ず区別して明示する。根拠のない金額を断定しない
