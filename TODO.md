...
- [ ] 台帳の空白を埋める（未実施 13 件を手間の小さい順に回す。着手時にプランを作る）
  ...
  ⚠ **代償は小さい**【実測 2026-09-12】: 1 手法 ＝ 3 試行（θ 3 水準・形式 (A)・1 層・1 地平）の見積もりで、13 手法 ＝ **＋39 試行**。n_trials 160 → 約 199 でも ⚠ **強い手法の DSR は 0.941 → 0.932 にしか下がらない**
  ...
  関連: [ledger.md §3](docs/specs/experiments/feature-discovery/ledger.md)（未実施 16 件の一覧と「次の一手」）
  - [ ] F2-2 再帰的特徴量削減（RFE・SVM-RFE）— 手間 小。`ail/selectors/wrapper.py` に 10 行
  - [ ] F5-1 PCA・主成分 — 手間 小。⚠ **必ず訓練分割の内側で fit し `fitted/` に成分を残す**
  - [ ] F1-7 情報係数（IC）の時系列安定性 — 手間 小。既存の fold の枠をそのまま使える
  - [ ] F1-4 分散しきい値 — 手間 小。⚠ **ラベルを見ないので単独では判定できない**（前処理としてのみ）
  ...
  - [ ] 見送り 3 件を再判断する（F4-2 Featuretools ／ F5-2 オートエンコーダ ／ F2-4 GA 部分集合探索）
    ...
    依存: 「F5-1 PCA・主成分」／「F2-1 前進選択・後退除去」
...
- [ ] 下降トレンドでの売買のタイミングを他のパターンでも検証する [plan](docs/plans/downtrend-entry-timing.md)
  ...
  **上昇トレンドを使う意味（2026-09-12 の追加指示への現時点の答え。着手前に読む）**
  - ⚠ **いまの配線では上昇と下降は別物ではない**: [rules.md 13-2](docs/specs/experiments/feature-discovery/rules.md) 規約 1 が **売り% = 100 − 買い%** と決めているので、検知器の出力 1 本を上から読むか下から読むかの違いしかない。⚠ **「上昇トレンドの検知器」を足しても、13-3 か 14-1 を開けない限り 1bp も動かない**
  - ⚠ **B&H に対して、ふつうの上昇トレンド情報の価値は 0 である**: 基準線は検証期間ずっと買い持ちなので、**上乗せ ＝ 休んだ日の −y の合計 − コスト差**。⚠ **持っている日には上乗せが生まれない**（差はコストだけ）。⚠ **上昇の情報に値段が付くのは「休みを終える日」だけ**
  - ✅ **それでも上昇側に意味はある。むしろ残りの金は全部そこにある**【実測 2026-09-12】: 谷 → 回復の取り損ね（C3 で **−21,009bp**）は、下げで稼いだ **＋17,624bp** より**大きい**（[§4](docs/specs/experiments/downtrend-detection.md)）
  - ⚠ **ただし対象は「相場が上がっているか」ではなく「下げが終わったか」という狭い事象**（標本は 12 回しかない。⚠ **エピソードの回数は検定にならない**・§5）
  - ⚠ **取り損ねが稼ぎを上回る理由は「回復の日足が急」ではない**【推測】: 山 → 谷と谷 → 回復は対数で同じ大きさ（回復の定義が「山に戻る」）なので、⚠ **避けた下げの割合より、外した戻りの割合のほうが大きい**ということ。⚠ **Phase 0 の (i)(ii) で確かめる**
  - **開ける梯子は 3 段ある（上に行くほど上昇情報に値段が付き、上に行くほど高い）**
    - 1 段目 **非対称な保有帯**（θ_buy ≠ θ_sell）— 13-3 だけ開ける。⚠ **14-1 の契約は触らない**ので一番安い。⚠ **ただし帯をずらすだけで、入口に別の時間スケールを持たせられない**
    - 2 段目 **出力を 2 本にする**（入口% と出口%）— 14-1 を開ける。⚠ **「入口専用の検知器（谷からの反転）」はここに入る**。入口を短期・出口を長期にできるのはこの段から
    - 3 段目 **建玉に段階を持たせる** — 13-4 規約 2 の変更 ＝ 検証方式の変更（橋渡し対 13-8）。⚠ **保有中の上昇の強さに値段が付くのはこの段だけ**
  ...
  - [x] Phase 0: ⚠ **回す前に上限を見積もる**（[rules.md 14-2](docs/specs/experiments/feature-discovery/rules.md)）
    ✅ 2026-09-12 完了。記録: [entry-timing.md](docs/specs/experiments/entry-timing.md)。⚠ **新しい実行も試行も作っていない**（`n_trials` は 160 のまま）
    ⚠ **結論: 片側だけを完璧にしても測れない。入口と出口を同時に直す構成だけが測定可能域に入る**（⚠ **市場タイミングでも銘柄別でも同じ**）
    ⚠ **神託の何 % を取れれば壁（符号 5/5 を確率 0.8）を越えるか**【実測 2026-09-12】 — ⚠ **片側だけの 4 通りが 4 通りとも 1 超 ＝ 定義から不可能**
    - 市場タイミング: (i) 出口だけ **2.60** ／ (ii) 入口だけ **1.49** ／ (iii) 両方 0.87
    - 銘柄別: (i) 出口だけ **1.24** ／ (ii) 入口だけ **1.02** ／ ✅ **(iii) 両方 0.51**（t≥2 なら 0.33）
    上限 bp/fold: 市場 (i) ＋1,357 ／ (ii) ＋2,551 ／ (iii) ＋6,532 ‖ 銘柄別 (i) ＋4,548 ／ (ii) ＋3,774 ／ (iii) ＋10,430（現行 C3 は −1,650）
    ⚠ **上昇と下降のどちらが大きいかは粒度で逆転する**（追加指示への答え）: 市場タイミングは **入口 ＋21,009 ＞ 出口 ＋15,039bp**、⚠ **銘柄別は逆で 出口 ＋30,992 ＞ 入口 ＋27,119bp**（銘柄ごとの下げは 961 回あり市場の 12 回より多い）
    ⚠ **足し算にならない**: (iii) の取り戻しは (i)＋(ii) より ＋4,867bp 多く、差はちょうど平常時の空振り ＝ ⚠ **片方だけ直しても空振りは消えない**（テストで恒等式として縛った）
    検算: ✅ **エピソード 12 回・[§4](docs/specs/experiments/downtrend-detection.md) の 3 分割・fold 別の上乗せが記録と一致** ／ ✅ **表から組み直した B&H が 6,321/6,336 日一致** ／ ✅ **表から組み直した C3 の上乗せ −8,251bp（記録 −8,252）**
    ⚠ **踏んだ落とし穴 2 つ**: **fold の切れ目は日付の空きでは拾えない**（検証 fold は連続。`result.csv` の `検証日数` が正本。空きで切ると先頭が 253 日になり符号の数が変わる）／ **表の `ts` は UTC 付き・`daily.csv` は素**（揃えないと全部 NaN になり「エピソード 0 回」という静かな誤りになる）
    道具: `experiments/feature-discovery/cli/ceiling.py`（`--table` で銘柄別も）。テスト `tests/test_ceiling.py` 15 件・全体 **238 件 pass**
    ⚠ **残る限界**: 銘柄別に組み直せるのは**古典フィルタだけ**（学習ゲートは `fitted/` が要る）／ 生存バイアスは入口側の上限を**高く**見せる向き（⚠ **甘く見てなお壁に届かない**）
  - [x] Phase 0-1: 回すかどうかの裁定 → ✅ **回す**（利用者の決定 2026-09-12「積極的に埋める方針にします」。[rules.md 14-10](docs/specs/experiments/feature-discovery/rules.md)）
    選んだのは (a) **2 段目（14-1 の出力の契約）まで開けて、入口と出口を同時に動かす型だけ回す**
    ⚠ **片側だけの型は回さない**（Phase 0 で 4 通りとも「神託超えが要る ＝ 不可能」と出ている。⚠ **これは「可能性が低い」ではなく「測れない」なので 14-10 規約 1 の除外に当たる**）
  - [ ] Phase 1: 規約の更新（⚠ **Phase 0 の答えは 14-1 の出力の契約を入口% と出口% に広げる**。⚠ **変更規約の 2 問に先に答えてから**書く）
    依存: 「Phase 0-1: 回すかどうかの裁定」
  - [ ] Phase 2: 実装（入口と出口を別に持てる形。`ail/detectors/` と `ail/validation/simulate.py`。⚠ **leak 対照が跳ねることを先に確認**）
    ⚠ **入口だけ・出口だけの構成は回さない**（Phase 0 で「測れない」と判定済み）
...
- [ ] 出来高を `trend` 層に足して検知器を回す（✅ **回すと決定**。着手時にプランを作る）
  ...
  ✅ **安い**: 実装 半日 ／ 実行 数分〜数十分（CPU）／ **n_trials ＋12（160 → 172）**・⚠ **SR0 は 1.0071 倍にしかならない**【実測】
  ...
  関連: 「下降トレンドでの売買のタイミングを他のパターンでも検証する」（⚠ **どちらも「検知器を良くする」方向。先にどちらを回すかの裁定が要る**）
