  ...
  - [ ] 実売買を 4 役（予測・売買判断・実際の売買・ビュワー）に分け直すために決めること
    ...
    - [ ] D. 実際の売買
      ...
      - [ ] D14: 1 日の上限の数え方（買い 規模 A $1,000 ／ 規模 B $10,000〔⚠ 実取引では使えない可能性〕・注文数の上限を、複数回の合計で数えるか）
        ⚠ **いまは「1 日の合計」になっていない**（`--max-day-usd` は**トレーダーごと・1 起動ごと**にリセット）【実測 2026-09-20】＝ 3 人なら実質 3 倍。⚠ **1 日 1 回に決まった後も残る宿題**（[live-trading.md §0-9](docs/specs/experiments/live-trading.md) の末尾）。直し方の案: 起動時にその日の `orders.jsonl` を読んで全員・全回の買いの合計で数える
