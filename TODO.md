...
- [ ] GPU 系を閾値売買でも回せるようにする（⚠ **いまは 5 実行とも毎日往復しか無い**。着手時にプランを作る）
  ...
  ⚠ **空白は 2 方向ある**（台帳の実測。[ledger.md](docs/specs/experiments/feature-discovery/ledger.md)）
    ...
    ⚠ **(2) 選別の軸**: ✅ **2026-09-13 に F3-3 MDA と F3-1 Lasso を移して 6 本になった**（F1-4・F1-7・F2-2・F5-1 ＋ F3-3・F3-1）。⚠ **残る 7 手法が毎日往復にしか無い**（F1-1・F1-2・F1-3・F1-5・F2-3・F3-2・F3-5 ＝ 実装 13 − 閾値売買 6）。⚠ **ただし F3-1 は「選別として機能していない」と分かった**ので、⚠ **6 本のうち実質は 5 本**（下の「F3-1 Lasso の…」）
  ...
  - [ ] LightGBM × 断面（own cs rel ll）× 閾値売買（⚠ **`gpu_lgbm_cs` の層。手間 小**）
    ⚠ **`gpu_lgbm_cs` は F3-3 MDA と F3-1 Lasso も持っている**。⚠ **F3-1 は 2026-09-13 に「選別として機能していない」と分かった**ので、⚠ **入れるかどうかは下の「F3-1 Lasso の…」の裁定に従う**
  - [ ] GAN 増強 3 種 × 閾値売買（Ridge+・LightGBM+・⚠ **MLP+ は初回**。⚠ **手間 大 — 1 本 1.5〜2 時間 ＋ leak**）

- [ ] F3-1 Lasso の「本数 1.0」行は選別ではなくフォールバックだった — ⚠ **直すか、読み方を直すかを決める**（⚠ **利用者の裁定が要る**）
  派生元: 「LightGBM × 閾値売買に選別 2 本を足す」（✅ 2026-09-13 完了。[記録 §3](docs/specs/experiments/selectors-lgbm-two.md)）
  ⚠ **中身**: `LassoCV` が選ぶ罰則 α が強すぎて **35 本すべての係数が厳密に 0** になる【実測 2026-09-13。同じ表で直接叩いて再現】。⚠ **すると `keep or X.columns[argsort(-|coef|)][:1]` のフォールバックが動き、係数が全部 0 なので並び順が返って `X.columns[0] = own_ret_1` が入る**
  ⚠ **つまり台帳の「F3-1 Lasso・本数 1.0」の行が測っているのは「先頭列 1 本だけを使う」であって、選別ではない**。⚠ **数字は正しいが名前が中身と合っていない**
  ⚠ **今回の 6 行だけの話ではない**: 台帳の Lasso 行は ⚠ **本数 1.0 が多数（own 日足の Ridge・LightGBM・MLP・GAN 増強）／ 1.2〜1.6 は一部の fold だけフォールバック ／ 2.2・5.0・6.2 は本来の選別が働いている**
  ⚠ **今回直さなかった理由**: ⚠ **実装を直すと既存の Lasso 行が全部「別の手法」になる**（[14-10 規約 5](docs/specs/experiments/feature-discovery/rules.md) 既存行は再計算しない）／ ⚠ **「直す」の中身が 1 通りに決まらない**
  決めること: (a) α の下限を切る ／ (b) `lasso_path` に替えて必ず k 本残す ／ (c) フォールバックを「選別できず」として記録し行を落とす ／ (d) 実装は変えず台帳の表示名と注記だけ直す
  ⚠ **どれを選んでも既存行の扱い（残す / 作り直す / 注記を足す）をセットで決める**。⚠ **作り直すなら n_trials の数え方も決める**
  関連: [rules.md 14-10](docs/specs/experiments/feature-discovery/rules.md) ／ [ledger-role.md](docs/specs/experiments/feature-discovery/ledger-role.md)（落とした行が分母になる）
