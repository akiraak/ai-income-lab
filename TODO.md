...
- [ ] システム全体と予測モデルとトレーダーに関する情報をまとめる
  ...
  - [~] vibeboardのタブにトレーダーを追加 [plan](docs/plans/vibeboard-traders-tab.md)
    ...
    ✅ 2026-09-20: 作った（[DONE.md](DONE.md)）。見て 2 度組み直した ＝ 白地に固定 ／ 主役はモデル（使うモデルと特性を出す）／ 比較の文・人数・「3 人のちがい」のページをやめて 1 人 1 ページだけ。呼び名は利用者の決定 ＝ `T1` アキ ／ `T2` アリス ／ `T3` カエデ（⚠ 意味の無い名前。鍵は変えない）。管理画面の pytest 183 本
  ...
  - [ ] vibeboard の「予測モデル」タブを、予測モデルを詳しく解説するものにする（「システム説明」の説明とのバランスをとる）
    利用者の指示（2026-09-20）。着手時にプランを作る（`docs/plans/`）
    いまの形（2026-09-20）: 一覧 ＋ モデル 1 本 1 ページ（どんなモデルか ／ モデルの特性 ／ 何を見て、どう答えを出すか ／ 過去のデータで試した結果 ／ 気をつけること ／ 畳んだ「正式な名前」）。⚠ **1 項目 1〜2 行の短い紹介**で、図が無い。いっぽう「システム説明」は 5 ページ・図つき・「詳しく（用語あり）」の囲みつきで手厚く、⚠ **モデル 1 本ごとの図（入れるもの → 計算 → 出力スコア）は「システム説明」の「モデルを作る」の 6 段目にしか無い**（`models.toml` の `flow`。いま使っている 3 本だけ）＝ モデルの話なのにモデルのページが薄い、という逆転がある
    決めること（⚠ 利用者の裁定。プランで案を出す）: ① 2 つのタブの分担 ＝ 「システム説明」は全部のモデルに共通の流れ（作る・確かめる・使う）／「予測モデル」はモデル 1 本ごとの深掘り、でよいか。型ごとの図は「予測モデル」のページへ移す（か両方に出す）か ② モデルのページに足す中身の候補 ＝ 図（入れるもの → 計算 → 出力スコア。机上で試しただけの 9 本にも `flow` を書く）／ モデル ＝ 軸の組み合わせ（入れるデータ・下ごしらえ・計算の仕方・当てにいく対象・学ぶ範囲）のどれに当たるか ／ 計算の仕方を小さな例で段を追って ／ 出力スコアの出かた（幅・癖）と読み方 ／ 試した経緯（いつ・どの形で・何通り・内訳）／ 「詳しく（用語あり）」の囲み（正式な名前・ハイパーパラメータ・コードと config の場所・【実測】の数字と出典）③ 言葉づかいを「システム説明」と同じ「両方を並べる」（やさしい本文 ＋ 畳まない「詳しく」）にそろえるか。いまの畳んだ「正式な名前」をどうするか ④ 一覧のページの形（いまは札だけ）
    ⚠ 守ること: 言葉の正本は `dashboard/models.toml` 1 本（トレーダーのタブも同じ文を写す ＝ 長くした文がトレーダーのページにも出る。出し分けるなら欄を分ける）／ 同じ説明を `system.toml` と二重に書かない（共通の流れは「システム説明」へリンク）／ 本文はやさしい言葉・「出力スコア」・ほかのモデルとくらべる文を書かない・特性には「どこまで確かか」の印・確かめた事実だけ（記録とコードを読む）／ 試した結果の印は人が記録から写す（[dashboard.md §17-3](docs/specs/dashboard.md)）／ 売買結果は出さない
    関連: 「「システム説明」タブの直し（⚠ **利用者の裁定待ち**。2026-09-20 の会話で出た案。決まったら `dashboard/system.toml` を直す）」（案 1〜3 ＝ 6 段目の見出し・軸の組み合わせの段・3 人の表 は、このタスクの分担の決めと一緒に決める）／ [dashboard.md §17・§18](docs/specs/dashboard.md)
  - [ ] 「システム説明」タブの直し（⚠ **利用者の裁定待ち**。2026-09-20 の会話で出た案。決まったら `dashboard/system.toml` を直す）
    ...
    案 2: 「モデル ＝ 軸の組み合わせ」の段を足すか（入れるデータの種類 ／ 数字の下ごしらえ ／ 計算の仕方 ／ 当てにいく対象 ／ 学ぶ範囲。⚠ 組めない組み合わせがある・作った数だけ「試した数」が増える・買う線と予算はトレーダーの側の軸）
  ...
  - [ ] 名前の付け方（英語名）をそろえるか決める（⚠ **利用者の裁定待ち**。2026-09-20 の会話の案 ＝ 種類.系統.変種 の階層スラッグ〔`model.ridge`・`det.tsc.quant.w60`・`thr-ownex-lgbm-a18`〕を、既存の名前は変えずに別名の表として足す）
    ⚠ 先に [rules.md 10-1](docs/specs/experiments/feature-discovery/rules.md)（2026-09-14 の名前の付け方）を土台にする: **台帳の鍵なのはモデル名（登録名）で、実験名は鍵ではない**・既存の名前は 1 つも変えない・計算が変わる変更は必ず名前を変える。進めるなら着手時にプランを作る
...
- [ ] トレーダー 3 人（それぞれ予算と 1 本以上の予測モデルを持つ）に予算を割り振り、tastytrade の本口座で実際に売買して記録を残す（実売買の仕組み） [plan](docs/plans/live-trading-three-models.md)
  ...
  - [ ] 2026-09-22（火）までに実運用へ: 日ごとの段取り [plan](docs/plans/live-trading-go-live-0922.md)
    ...
    「実運用」＝ `T1`〜`T3` を規模 A（$300 × 3）で本番に投入し、1 日目の記録が残って口座と台帳が一致したところまで。執行の窓 15:45〜16:05 ET ＝ **12:45〜13:05 PDT**
    ...
    - [x] 〜 9/20（日）: 市場が開いていなくてもできる作業を全部終わらせる（合計 10〜15 時間【推測】。⚠ 削る順は S8 の `sim_T1`〜`sim_T3` → S9 → S6 の timer の雛形。S1〜S5・S7 は削れない）
      ...
      - [x] S6: 起動の 1 本 `run-live.sh`（日足の更新 → 訓練 → 窓まで待つ → 気配 → 推論 → `run_day.py`。既定は dry-run。⚠ 本番の鍵は書かない）＋ timer ／ cron の雛形（⚠ 入れるのは火曜の後）
        ✅ 2026-09-20: timer の雛形は `experiments/live-trading/systemd/`（user timer 2 本 ＝ 朝 09:00 ET の準備・15:40 ET の執行。鍵は `~/.config/ai-income-lab/live.env` に利用者が置く。雛形は dry-run が既定）。titan は systemd 255 で user timer が使える・`OnCalendar` の次回は Mon 12:40 PDT と解釈された【実測】。⚠ 入れるのは水曜から（D4）
        2026-09-19 夜: `run-live.sh`（プロジェクト直下）を書いた ＝ `--prepare`（朝: 日足 ＋ 外部系列）／ 日足の更新 → 実験 × 手法をトレーダーの設定から拾って予測を並列 → `out/<日付>/predict.jsonl` → 台帳の控え → `run_day.py`（「--」の後ろはそのまま渡す。既定は dry-run。⚠ 本番の鍵は書いていない）。モックで T1〜T3 の候補を通して 37 秒で 120 注文の計画まで【実測】。⚠ **残り: timer ／ cron の雛形**。⚠ 訓練を朝に分ける必要は無くなった（S2）ので、15:50 ET に起こして 1 分半後に発注が始まる形（⚠ プラン §2-5 の「15:55 に成行」より早い ＝ 手順書 S7 に書く）
      ...
      - [x] S11: 執行器の発注を 2 段にする（① dry-run ＋ 発注を先に全部出す → ② 約定の確認をまとめて待つ）＝ 初日の 120 本を窓に収める（⚠ **利用者の裁定 D6 が先**。実弾の経路の作り替えなので）
        ✅ 2026-09-20 未明: 利用者決定 D6「する」。`execute.py` の `run_one` を前半 `submit_one`（控え → dry-run → 発注）と後半 `finish_one`（約定待ち → 未約定は取消 → 約定を読む）に割り、`run_all` は submit のとき ① 全部出す → ② 1 本ずつ確かめる。出す順・控え・1 注文ごとの台帳の保存は同じ。取消の期限は 1 本ごとに「その注文を出した時刻から 600 秒」のまま。`run_day.py --serial` で元の直列に戻せる。同じ 120 本のモックの 1 日が **124 秒 → 2.4 秒**【実測】。テスト 5 本を足して執行器 120 本・`mockrun.sh`・`sim2`（筋書き 64 日。落ちた日の復元も含め、口座と台帳の差は注入した BAC だけ）が通る。⚠ **本物では ① に 120 本 ×（dry-run 0.9 ＋ 発注 1.9 秒）≒ 5.6 分【推測。sandbox の実測から】**かかる ＝ 15:50 ET に起こして 15:51 台に出し始めれば 16:00 までに収まる見込みだが余裕は小さい → 月曜に本番の dry-run の往復時間を測って関門 5 で決める（足りなければ dry-run を並べて出すなどを足す）
        派生元: S8 の実測。いまは 1 本ずつ「dry-run → 発注 → 約定まで待つ → 台帳を保存」で、約定待ちが直列に積み上がる
