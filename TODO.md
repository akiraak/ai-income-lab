...
- [ ] 下降トレンドの検知の検証（長期・中期・短期の各スケールで取れるかを見る。着手時にプランを作る）
  ...
  ✅ **前提の 1995 表は用意できた**（2026-09-11。`config/experiment/own_1995.toml` → `data/features/own_1995/d.parquet`。1995-02-10〜2026-09-04・431,759 行・63 銘柄）。**検証日数 6,619（×3.67）・検出限界 2.02bp/日（÷2.35）・10% 以上の下げは検証期間内に 11 回**【実測】（[validation-power.md §8-2](docs/specs/experiments/feature-discovery/validation-power.md)）
  ⚠ 使うときの注意（[rules.md 12-1](docs/specs/experiments/feature-discovery/rules.md)）: **検証期間は 1995 年ではなく 2000-05-10 から**（最初の 1/6 は訓練）・fold 1 は 54 銘柄・生存バイアスは深い・分割調整の「要確認」12 件中 8 件が 2018 年より前。⚠ **新しい手法を 1995 表で回した行は n_trials に数える**（[14-4](docs/specs/experiments/feature-discovery/rules.md)）
  ...
  依存: 「検証方式の改善の反映と実装」（✅ 2026-09-11 完了。1995 表・前置きの門・診断列・14 章の規約がそろった）
