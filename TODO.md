...
- [ ] 台帳の空白を埋める（未実施 13 件を手間の小さい順に回す。着手時にプランを作る）
  ...
  ⚠ **代償は小さい**【実測 2026-09-12】: 1 手法 ＝ 3 試行（θ 3 水準・形式 (A)・1 層・1 地平）の見積もりで、13 手法 ＝ **＋39 試行**。⚠ **n_trials は 253 まで来たので 253 → 約 292**。⚠ **それでも強い手法の DSR は 0.9221 → 0.915 程度にしか下がらない**【推測】（160 → 199 で 0.941 → 0.932 だった実測と同じ向き）
...
- [ ] 出来高を `trend` 層に足して検知器を回す（✅ **回すと決定**。着手時にプランを作る）
  ...
  ⚠ **そのタスクが残した示唆がここに直接効く**: ⚠ **D 系の買い% は幅 0.0008〜10.6 点しか動かない**（門の水準は 20 点）。⚠ **5 fold のうち 3 fold は B&H と 1 日も違わなかった**
  ⚠ **だから本タスクの問いは「出来高で確率に幅が出るか」**になる。⚠ **出ないなら入力ではなく検知器の中身（較正・ラベル・モデル）が律速だと分かる** ＝ どちらに転んでも軸が 1 つ閉じる
