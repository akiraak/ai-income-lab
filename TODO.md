...
- [ ] 検証方式の改善の反映と実装 [検討記録](docs/specs/experiments/feature-discovery/validation-power.md)
  派生元: 「検証方式の検討」（✅ 2026-09-11 完了。検討の結論: 保留 16 行は検出限界の 2 桁下で「再測」では白黒つかない。採る改善は 期間延長・前置きの門・診断列 の 3 つ）
  ⚠ **先に利用者の判断が要る 3 問**（[検討記録 §5-4](docs/specs/experiments/feature-discovery/validation-power.md)）: (1) 門の水準（AUC 0.52・買い% 幅 20 点）の事前固定 (2) 門前で落とした手法を n_trials に数えるか（提案: 数えない） (3) 保留 16 行を再測せず閉じる処遇でよいか
  ...
  - [ ] 診断列の実装（fold 10 等分の上乗せ符号・実効系列数を compute_trading に。採否は変えない。検討記録 §3-1・§3-5）
  - [x] 台帳の注記拡張と保留 16 行の処遇の反映（catalog_notes を試行の行にも書ける形に拡張。⚠ 判定列は変えない・台帳は手で書かない。検討記録 §6-2） [plan](docs/plans/archive/ledger-close-notes.md)
    ✅ 2026-09-11 完了。catalog_notes の `[[closed]]` 16 件 → 理由列に注記・§0 に 1 文。判定（保留 25・落とす 66・採る 0）と n_trials 91 は不変。テスト 177 件 pass（新規 7: ちょうど 1 行一致・保留のみ・空振りで停止）

- [ ] 下降トレンドの検知の検証（長期・中期・短期の各スケールで取れるかを見る。着手時にプランを作る）
  利用者の指示（2026-09-11）: 取引回数が多いと手数料の割合が増えるだけなので、保有期間を長くして上昇を拾う＝下降トレンドを検知できるものを検証したい。長期だけでなく短期・中期の下降トレンドが取れるかも検証したい
  背景: 閾値売買の実測で、負けの本質は回転コストではなく「いつ休むかの情報が無い」こと（較正 a ≈ 0。(A) は取引 4 回/fold まで減ってもなお、fold 3 を休んだ分だけ B&H に負けた。[threshold-trading.md](docs/specs/experiments/threshold-trading.md) §2-§3）。地平 1 日の方向ではなく下降トレンド（レジーム）を予測対象にする、未検証の軸
  ⚠ スケールごとにラベルの定義（下げ幅・窓）を事前固定し、スケールも「選べた自由度」として n_trials に数える（[rules.md 11 章](docs/specs/experiments/feature-discovery/rules.md) 規約 4）
  ⚠ 実効標本は日数ではなく「下降エピソードの回数」。長期は 2019〜2026 の検証期間に 2〜3 回しかなく、32 年の日足へ検証期間を伸ばすことが前提になる
  メモ（2026-09-11）: raw の日足は既に 1994-11 から 8,000 本ある（再取得不要。features の 1995 表の構築だけで済む）。1995 開始で下げ相場は 8 回程度・検出限界 ÷1.94。レジーム級（数百〜数千 bp/fold）の効果なら現行の物差しで測れることも確認済み（[validation-power.md §3](docs/specs/experiments/feature-discovery/validation-power.md)）
  候補【推測】: ボラティリティ・ゲート（ボラは方向と違い予測可能。Moreira-Muir 2017）／ 古典フィルタ（200 日 SMA・Faber 10 か月・12 か月モメンタム）／ triple-barrier・トレンドスキャニングのラベル（López de Prado）／ HMM・変化点検知
  物差しは閾値売買（対 B&H 上乗せ）をそのまま使う（threshold-trading.md §7「以後の手法検証はこの物差しで測れる」）
  依存: 「検証方式の改善の反映と実装」（期間延長の 1995 表・基準線 2 本・fold 長の規約が先に効く。検討自体は ✅ 2026-09-11 完了）
  関連: 「DL / 進化的探索の手法を増やして検証を回す」（検知器は対決の参加者にもなる）
