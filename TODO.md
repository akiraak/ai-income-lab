...
- [~] 割り当てなしの災害系列（`ex_`）の見かけの改善を、日付をずらした偽薬で確かめる [plan](docs/plans/exog-shift-placebo.md)
  ...
  - [x] Step 2: NCEI・IEM を 2010 年から取り直し、災害の表が変わらないことを確かめる
    ⚠ **取り直しで `z20` が最大 0.6 動いた。** 原因は ⚠ **pandas の rolling が窓を足し引きで更新するため、被害額（最大 3.3×10¹⁰）の系列で結果が系列の始まりに依存すること**（窓ごとに直接計算すると差 0【実測】）→ ⚠ **窓ごとの直接計算に直した**。⚠ **2018 年からの生データと 2010 年からの生データで、災害 4 本の表が完全一致**【実測 2026-09-17】
...
- [ ] 銘柄集合を広げる（✅ **段 1・段 3 は実施済み。段 2 は落とす。残るは段 3 の続きと段 4**）
  ...
  - [x] 段 4: 上場廃止銘柄の日足が取れるか確かめた — ⚠ **配信からは出ない**（2026-09-17）
    ✅ 完了。プラン: [archive](docs/plans/archive/delisted-symbols.md) ／ ⚠ **記録: [universe-widening.md §11](docs/specs/experiments/universe-widening.md)**
    ⚠ **廃止 10 本とも日足 0 本**（対照の `AAPL` `SPY` は 3,013 本）。⚠ **台帳には `active=False` で残る**ので、廃止済みかどうかは API から分かる
    ⚠ **副産物: ティッカーの使い回しで 2 社の履歴が 1 本に繋がる**（`FB` = Facebook 2014〜2022-06 ＋ ProShares の ETF 2025〜）。⚠ **検査に `gap_over_30d` を足した**（手元は `VXX` 1 本だけ・どの銘柄集合にも入っていない）
  - [ ] 段 4 の続き: point-in-time のデータの費用を試算する（⚠ **金銭が発生するので提案・試算で止める**）
    派生元: 「段 4」（2026-09-17 完了）。⚠ **配信から組めないと分かったので、残る道は有償データだけ**
    見るもの: 廃止銘柄の株価 ＋ ⚠ **いつ指数に入っていたか（point-in-time の銘柄集合）** の 2 つが要る。⚠ **個人が買える価格帯かどうかを先に見る**
    ⚠ **CRSP・Compustat は大学経由が前提で個人には売らない見込み**【推測】。⚠ **一次情報（各社の価格表）で確かめる**
