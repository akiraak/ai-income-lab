...
- [ ] 台帳の空白を埋める（✅ **「小」4 件は 2026-09-12 に完了。残る未実施は 12 件**。手間の小さい順に回す）
  ...
  ⚠ **「小」4 件が残した示唆**: ⚠ **3 手法（F1-7・F5-1・F1-4）は乱択または「全部使う」と 1 ビットも違わない売買になった**。⚠ **較正した買い% の p05-p95 幅が 0.0 点**（門の水準は 20 点）で、⚠ **選別を替えても変換を替えても律速はそこではない**（[記録 §2-1](docs/specs/experiments/selectors-small-four.md)）
