...
- [ ] システム全体と予測モデルとトレーダーに関する情報をまとめる
  ...
  - [~] vibeboard の「予測モデル」タブを、予測モデルを詳しく解説するものにする（「システム説明」の説明とのバランスをとる） [plan](docs/plans/vibeboard-models-tab-deep.md)
    ...
    - [ ] vibeboard と sidecar（3015）を入れ直して「予測モデル」タブを見る（⚠ **利用者**。`modelview.py`・`figures.py`・`systemview.py` を直したので入れ直しが要る。⚠ 3015 を掴んでいる古い sidecar も止める ＝ dashboard.md §12-3）→ 文の直しは `dashboard/models.toml` だけ（保存すれば 5 秒で入れ替わる）
  ...
  - [ ] 「システム説明」タブの直し（⚠ **利用者の裁定待ち**。2026-09-20 の会話で出た案。決まったら `dashboard/system.toml` を直す）
    ...
    案 3: その段の実例として、いまの 3 人のモデルを軸で横に並べた表を置くか（⚠ トレーダーのタブは「人どうしを見くらべるページを作らない」決まり。置くなら「システム説明」の側）
  - [ ] 名前の付け方（英語名）をそろえるか決める（⚠ **利用者の裁定待ち**。2026-09-20 の会話の案 ＝ 種類.系統.変種 の階層スラッグ〔`model.ridge`・`det.tsc.quant.w60`・`thr-ownex-lgbm-a18`〕を、既存の名前は変えずに別名の表として足す）
    関連: 「DBを使ったデータの永続化を行う」（2026-09-21 の利用者の裁定「予測モデルの命名規則を作らないと解説に使えない」で、[プラン](docs/plans/db-model-facts.md) の Phase 1 になった。案は §5 ＝ 型 ／ 予測モデル名 ／ 検証名の 3 段）
    ⚠ 先に [rules.md 10-1](docs/specs/experiments/feature-discovery/rules.md)（2026-09-14 の名前の付け方）を土台にする: **検証結果一覧の識別項目なのはモデル名（登録名）で、実験名は識別項目ではない**・既存の名前は 1 つも変えない・計算が変わる変更は必ず名前を変える。進めるなら着手時にプランを作る
...
- [ ] 「今後 2 週間くらい上がりそう ／ 下がりそう」を当てにいく形を机上で試す（予測の対象を先 10 営業日にする）
  ...
  メモ: 「システム説明」の「点に直す」の段（`dashboard/system.toml`）は毎日売り買いするように読める → 「学ぶ対象は明日だが、売り買いは毎日ではなく、点が線の上にあるあいだ持ち続ける。もっと先（20 日〜）を当てにいく型も試したが効かなかった」を足すか（⚠ 利用者の裁定待ち）
...
- [ ] トレーダー 3 人（それぞれ予算と 1 本以上の予測モデルを持つ）に予算を割り振り、tastytrade の本口座で実際に売買して記録を残す（実売買の仕組み） [plan](docs/plans/live-trading-three-models.md)
  ...
  - [ ] 実売買を 4 役（予測・売買判断・実際の売買・ビュワー）に分け直すために決めること
    ...
    - [ ] D. 実際の売買
      ...
      - [ ] D14: 1 日の上限の数え方（買い 規模 A $1,000 ／ 規模 B $10,000〔⚠ 実取引では使えない可能性〕・注文数の上限を、複数回の合計で数えるか）
...
- [~] DBを使ったデータの永続化を行う [plan](docs/plans/db-model-facts.md)
  ...
  関連: 「名前の付け方（英語名）をそろえるか決める」（裁定 ④ でこのプランの Phase 1 になった）
  ...
  - [~] Phase 1: 予測モデルの命名規則（型 ／ 予測モデル名 ／ 検証名の 3 段。下の 2 段は検証結果一覧の識別項目から機械で作る・既存の名前は変えない・検証結果一覧の全行で 1 対 1 を確かめる。⚠ 綴りは利用者の裁定）
    ✅ 2026-09-21: `ail/names.py`・`config/names.toml`・rules.md 10-2・テスト 16 本。検証結果一覧 1,155 行 ＝ 検証名 1,155 ／ 予測モデル名 591 で 1 対 1【実測】
    - [ ] 綴りを決める（⚠ **利用者**。`T1` `own.all.ridge.a` ／ `T2` `own-cs-rel-ex.all.lgbm.a~n48` ／ `T3` `own-seq.t3-quant60.ridge.a`。プラン §5）
  ...
  - [~] Phase 2: 研究の DB（`research.sqlite`）。書き手・読み手を替え、255 本を取り込み、`ledger.md` 不変・`n_trials` 667 不変・書き戻しの一致を確かめてから、⚠ 利用者の了承のあと `runs/` のファイルを消す
    ...
    - [ ] 控えを取る（⚠ **利用者が置き場を決める**。`python3 -m cli.db backup --to <道>`）
    - [ ] `runs/` の実行ディレクトリ 255 と `runs/queue/` を消す（⚠ **利用者の了承のあと**。`python3 -m cli.db remove-dirs --i-verified`）
    - [ ] sidecar（3015）を入れ直す（⚠ **利用者**。`vibetab.py`・`app/experiments.py` を直したので。実行タブが DB を読む）
  - [~] 言葉を分かりやすくする（利用者の指示 2026-09-21「表・手法・形式・窓」「鍵」「台帳」「試行」が分かりにくい）
    ...
    - [ ] vibeboard を入れ直してタブの名前「実行」を見る（⚠ **利用者**。`vibeboard.config.json` は起動時に読む。sidecar 3015 も入れ直す）
  - [ ] Phase 3: 予測モデルのタブが DB を読む（試した経緯の数と印を予測モデル名で引く・TOML と DB の食い違いを記録で確かめて直す・DB が無くても壊れない）
    ⚠ 検証結果一覧の行と判定を DB の表 `ledger_rows` にも書く案（`ledger.md` と同じ生成物）を利用者に確かめる（プラン §6・§9）
  - [ ] Phase 4: 「詳しく」の【実測】（較正の係数・門）を DB から差し込む
