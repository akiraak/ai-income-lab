...
- [ ] 買い% の中心と θ の置き方を検討する（⚠ **原因 C**。着手時にプランを作る）
  派生元: 「買い% の幅が潰れる原因を切り分ける」（✅ 2026-09-12 完了）
  ⚠ **利用者の裁定（2026-09-12）で今回は触らないことにした**（結果を見てから水準を動かさない。[rules.md 13-3 規約 4](docs/specs/experiments/feature-discovery/rules.md)）
  ⚠ **D2 中期・D3 長期は較正が正しく、幅も 8〜45 点ある。** ⚠ **潰れるのは中心が 58.5 / 68.4% にあって θ ∈ {50,55,60} が全部その下に落ちるから**（[§4](docs/specs/experiments/buy-pct-width-collapse.md)）
  ⚠ **中心が高いこと自体は正しい** — 200 営業日先が上がる確率は本当に高い（ドリフト）。⚠ **較正ではなく θ の置き方が信号に合っていない**
  ⚠ **触ると 13-3 の事前固定を開く。** ⚠ **開くなら「なぜ変えたか・変える前の結果をどう扱うか」を先に書く**（rules.md 冒頭の変更規約）
  関連: ⚠ **門の 2 条件も整合していない**（原因 B。幅 20 点は実質 AUC 0.57 の要求で、AUC 0.52 の条件は何も足していない。[§3](docs/specs/experiments/buy-pct-width-collapse.md)）
...
- [ ] 出来高を `trend` 層に足して検知器を回す（✅ **回すと決定**。着手時にプランを作る）
  ...
  ✅ **問いの立て方を 2026-09-12 に更新した**（[buy-pct-width-collapse.md §4](docs/specs/experiments/buy-pct-width-collapse.md)）: ⚠ **D 系が B&H と違わないのは幅のせいではなかった** — ⚠ **D2 中期・D3 長期は較正が正しく、幅も 8〜45 点ある。** ⚠ **潰れているのは中心**（買い% の中央値 58.5 / 68.4）で θ 3 水準が全部その下に落ちる
  ⚠ **だから本タスクの問いは「出来高で買い% の中心が動くか（θ を跨ぐ日が増えるか）」**になる。⚠ **幅ではなく `上θ50 / 上θ55 / 上θ60` の割合を見る**（`cli/calibdiag.py` が出す）
  ⚠ **D1 短期 20 日だけは不具合を踏んでいた**ので、回すときは直した較正で回る（旧行とは別の鍵になる）
