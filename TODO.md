...
- [ ] システム全体と予測モデルとトレーダーに関する情報をまとめる
  ...
  - [ ] 完成した予測モデルに入力する値と出力値の形式を知る
    利用者の指示（2026-09-23）。⚠ 2026-09-23 に確かめた ＝ TODO に同じものは無く、情報は散らばっている（1 か所にまとまった「形式」は無い）
    ✅ 既にあるもの: **入力** ＝ [dashboard/models.toml](dashboard/models.toml) の各モデルの「詳しく」（`own_` 35 列の内訳〔リターン・ボラ・Parkinson・z・レンジ内の位置・出来高比・歪度尖度・自己相関・足の形〕／ ownex 124 列 ＝ own 35 ＋ cs 26 ＋ rel 13 ＋ ex 50〔ECB 為替・米国債・NOAA・地震〕／ T3 QUANT は 60 日の観測期間 ／ StandardScaler は訓練分割で fit・欠損は補完せず落とす ／ ラベル ＝ 1 日先の対数リターン）。コードは `experiments/feature-discovery/ail/features/own.py`・`cross.py`・`config/dataset/exog_daily.toml`。**出力** ＝ 買い% 0〜100（Platt 較正の式は同じ「詳しく」）・出口% ＝ 100 − 買い%・`out/<日付>/predict.jsonl` の行（日付・`model`・`method`・銘柄・買い%・出口%・代役の終値・入力の指紋・commit ＝ [live-trading.md §0-9](docs/specs/experiments/live-trading.md) B8）・`predict.meta.jsonl`（訓練の範囲・較正・列数）・やさしい言葉の「出力スコアの読み方」（予測モデルタブの「実際の売買で使う」）
    足りないもの ＝ **実売買の 3 モデル（T1 own-ridge ／ T2 ownex-lgbm ／ T3 seq-quant）ごとに 1 つの表**: 入力（1 行 ＝ 1 銘柄 1 日 ／ 列の名前・意味・単位・観測期間 ／ 欠損と標準化の扱い ／ 予測する日の足は「今日の途中の足」＝ 終値の代役）と 出力（買い% ／ 出口% の意味と値域 ／ `predict.jsonl`・`predict.meta.jsonl` の項目と型）。⚠ モデル・較正のコードは触らない（`cli.predict` は配線だけ ＝ CLAUDE.md）
    決めること（⚠ 利用者の裁定）: 置き場 ＝ 予測モデルタブの「詳しく」に「入力と出力の形式」の節を足す か、`live-trading.md` §0-9 の近くに技術的な表を置く か、両方
    利用者の補足（2026-09-23）: **入れるデータと出力だけでなく、その前後を 1 本の流れでつなげて説明する** ＝ 予測モデルはいつ学習されるか ／ いつデータが入力されるか ／ 出力をトレーダーがどう使うか
    ✅ 2026-09-23 にコードで確かめた流れ（`cli/predict.py`・`plan.py`）: ⚠ **「完成したモデル」は保存した重みではなく作り方（実験名 ＋ 数字の選び方）で、毎営業日 15:50 ET 過ぎに `run-live.sh` が学習し直す**（2018-01-31 からの日足で、ラベルが確定した行 ＝ asof の 2 営業日前まで。9/22 asof なら訓練の終わりは 9/18。3 本並列 43〜48 秒）→ 入力は前日までの足 ＋ **今日の途中の足（15:50 ET ＝ 終値の代役）** → 買い% ／ 出口%（0〜100・出口% ＝ 100 − 買い%）→ `predict.jsonl` → 執行器: 合成 `asis`（1 本そのまま）→ **持っていない ＆ 買い% > θ で買い ／ 持っている ＆ 出口% > θ で売り ／ それ以外は hold ・ skip**（`plan.decide`）→ 予算 $300 ÷ 5 銘柄 ＝ $60 を整数株に → 15:45〜16:05 ET に成行（売りが先・1 注文 1 トレーダー）→ 翌朝 `--prepare` が公式終値で紙上の対照
    関連: 「vibeboard の「予測モデル」タブを、予測モデルを詳しく解説するものにする（「システム説明」の説明とのバランスをとる）」／ [dashboard.md §17](docs/specs/dashboard.md)
