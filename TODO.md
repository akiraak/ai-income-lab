...
- [~] 割り当てなしの災害系列（`ex_`）の見かけの改善を、日付をずらした偽薬で確かめる [plan](docs/plans/exog-shift-placebo.md)
  派生元: 「社会にインパクトを与えそうなデータを増やす」（2026-09-16 完了。[記録 §13-4](docs/specs/experiments/daily-data-sources.md)）
  - [x] Step 1: ずらしの仕組み（`ex_shift_days` ／ `--shift-days`）と、台帳で偽薬の実行を試行から外す ＋ テスト
  - [~] Step 2: NCEI・IEM を 2010 年から取り直し、災害 5 本の表が変わらないことを確かめる
  - [ ] Step 3: 偽薬 27 本（1 年 × 7 ／ 任意 × 20）を回す
  - [ ] Step 4: 順位の p 値と判定を記録 §15 に書く
  ⚠ **ずらしは過去へだけ**（循環ずらしは未来の値が入るので採らない）。⚠ **本数・ずらし幅・判定の規則はプラン §2 で結果を見る前に決めた**
  ⚠ **`impact_ex_2018` は列を使う 11 本すべてで価格だけを上回った**（＋1.92〜＋6.09bp。最良 Boruta ＋3.23bp【実測】）。⚠ **ただし fold 1 頼み（外すと 11 本すべて負）・t 0.66・DSR 0.62・乱択でも 5/5 fold で改善したので保留**
  ⚠ **`z20` の穴を塞いでも改善は消えなかった**（[記録 §14-3](docs/specs/experiments/daily-data-sources.md)）＝ ⚠ **古い値の引きずりは原因ではない**
  ⚠ **偽薬は 2 種**: 1 年ずらし（季節性を残す）／ 任意のずらし（季節性も壊す）。⚠ **列の数・系列・変換は同じにして、日付だけを変える**
  ⚠ **偽薬でも同じだけ改善するなら、災害の中身は効いていない**（効いているのは「全銘柄で同じ値の列がある」こと自体【推測】）

- [ ] 台帳（`ledger.md`）を titan で吐き直す（2026-09-16 に回し直した 8 実行を載せる）
  派生元: 「社会にインパクトを与えそうなデータを増やす」（2026-09-16 完了）
  ⚠ **Sx360 で吐いてはいけない。** ⚠ **Sx360 の `runs/` には `gpu_*` の実行が無く、LightGBM・MLP・GAN の行が消える**（手法 58 → 54 行【実測】）
  ⚠ **Sx360 の `experiments/feature-discovery/runs/` の `2026-09-16T22-23-56_own_impact_2018` 〜 `2026-09-16T22-56-17_exog_h1` の 7 つ ＋ `2026-09-16T23-05-03_exog_h1_leak` を titan の同じ場所へ持っていき**、titan で `cli.report --catalog` を吐く（台帳は `runs/` しか読まない）
  ⚠ **`2026-09-16T16-55-33` 〜 `T17-13-48` の 5 つは持っていかない**（`z20` の穴を塞ぐ前・2018-06-15 始まりの実行。[記録 §14-3](docs/specs/experiments/daily-data-sources.md)）
  ⚠ **titan 側のコードも最新にしてから吐く**（`git pull`）
  ⚠ **`ssh titan` は `eval "$(ssh-agent -s)" && ssh-add ~/.ssh/titan-ed25519` が要る**（パスフレーズの入力は利用者）
