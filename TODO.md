...
- [ ] トレーダー 3 人（それぞれ予算と 1 本以上の予測モデルを持つ）に予算を割り振り、tastytrade の本口座で実際に売買して記録を残す（実売買の仕組み） [plan](docs/plans/live-trading-three-models.md)
  ...
  - [ ] 仮データを用意し、実際の売買のコード（執行器）を動かしてテストする。時間経過は実際の何倍かで進めたり止めたりでき、管理画面でも確認できる [plan](docs/plans/live-trading-sim-clock.md)
    利用者の指示（2026-09-19）: **仮データを用意し実際の売買のコードを動かしテストを行う。時間経過は実際の何倍かで進めたり止めたりして管理画面でも確認できる**。✅ プランは 2026-09-19 に作り、Phase 0（決めごと）まで済ませた（⚠ **実装はまだ・利用者の指示待ち**。見積り 6.5〜7 営業日【推測】・費用 $0。Phase 1〜2 の 2.5 日で「速さを変えて通しで動き、管理画面で見える」まで）
    ⚠ **利用者の決定（2026-09-19）: 実売買に係わる状態とシミュレーションの状態は排他にし、必ず分かるようにする**（プラン §2-3。このタスクでいちばん上に置く決まり）＝ 機械全体のモードを 1 つ持つ（`MODE`。無ければ `real`）／ シミュレーションモードでは本物の `run_day` が起動を拒否し、実売買モードでは `simrun` と仮の時計が拒否 ／ `run.lock` で同時起動と動作中の切り替えを防ぐ ／ 記録・状態・`HALT` の木を分け、互いに 1 バイトも書かない ／ シミュレーションのトレーダーは名前が `sim_` で始まる ／ 管理画面は全ページの帯・`[SIM]`・「仮」の印・`/api/*` の `mode` で必ず分かるようにし、1 つの画面に両方を混ぜない
    形: 足すのは「仮の時計」「仮データを流す運転手 `simrun.py`」「操作の口 `simctl.py`」の 3 つだけで、執行器の売買のコードは本物と同じものを動かす。記録は `sim/<名前>/`（git 管理外・全行 `mock: true` ＋ `sim: true`）に分け、管理画面は `AIL_LIVE_DIR` でそこを読む
    いまある部品: `experiments/live-trading/mockrun.sh`（モックで 20 営業日を一気に流す。記録に `mock: true`）・`run_day.py --ignore-window`・管理画面のデモ（`dashboard/demo/live/` か `AIL_LIVE_DIR` の記録を読む。`devtools.MockServer`）。⚠ **足りないのは「時計」**: いまは日付を引数で渡して 1 日ずつ流すだけで、速さを変える・止める・再開する仕組みと、流れている途中を管理画面で見る経路が無い
    プランで決めること: (a) 仮データの中身（気配・約定・拒否 `Session offline`・受渡し T+1・半日立会・休場日をどこまで作るか。実データの日足を再生するか、作り物か）(b) 仮の時計の持ち方（執行器の「いま」を 1 か所から引く。⚠ 本番の経路では必ず本物の時計になること）(c) 速さ・停止・再開の操作をどこに置くか（⚠ 管理画面の操作はローカル面だけ・公開面は監視と停止だけ ＝ [dashboard.md §2・§3](docs/specs/dashboard.md)。CLI だけにする手もある）(d) 予測モデルの合図をどう入れるか（`kind = "file"` の仮の合図か、Phase 1 の `predict.jsonl` か）
    ⚠ **緩めないもの**: 仮データの記録には必ず `mock: true` を付け、本物の記録（`out/<日付>/`・`state/<env>/`）と置き場を分ける ／ 本番の鍵（3 段）・`HALT` の経路は変えない（取消の鍵で発注は開かない）／ 仮の時計で本番に発注できないことをテストで固定する ／ 仮データの損益・差は判定（Phase 6）に混ぜない
    関連: 「Phase 2: トレーダーと執行器 `experiments/live-trading/`（`trader.py` ＋ `config/traders/*.toml`・合成規則のテスト・状態機械・予算の上限・銘柄ごとに合算・dry-run → 発注 → 約定確認 → 取消・再送・記録は `Masker` 経由。固定の合図でモック 20 日 → sandbox で市場時間に 1 日）」
    関連: 「D16: 毎日自動で起動する仕組み（titan の timer ／ cron）。まだ入れていない」（無人運転の通し稽古として使える）
    関連: 「F23: 4 役がそれぞれ動いたか（予測の更新・判断の記録）を画面に出すか」
    - [x] Phase 0: 決めごと 4 つ → [live-trading.md §0-7](docs/specs/experiments/live-trading.md) に書いた（✅ 2026-09-19。利用者の指示「Phase0やって。実装はしないで止めて」）
      決まったこと: (排他) `MODE`・`run.lock`・両方向の拒否・木の分離・名前は `sim_` 始まり・全ページの帯 (操作) CLI だけ・管理画面は表示だけ (1) 仮データ ＝ 実データの日足の再生。値段は 2026-06-01 からの足・日付は 2026-10-01〜12-31 の営業日 64 日（半日立会 11-27・12-24 を踏む）(3) 試験用の `sim_a`（整数株 3 銘柄）・`sim_b`（金額指定 10 銘柄）・`sim_c`（2 モデルの平均・`sim_a` と銘柄が重なる・INTC で見送りが出る）各 $300 (4) 窓は 1 日 1 回・時刻の一覧で持つ・速さの初期値 ×60・再送と取消の秒数は縮めない
      ✅ **決定（2026-09-19・利用者。案 A）: Sx360 をシミュレーション専用の機械にし、tastytrade の `.env` を置かない**（titan ＝ 実売買と日足の取得。日足は titan から Sx360 へ写す。§0-7 (f)）。✅ **利用者が Sx360 の `.env` の資格情報を消した**（2026-09-19）
      ⚠ 利用者の確認（2026-09-19）: 仮の期間（2026-10〜12 月）と試験用の 3 人の設定は「よい」
      ⚠ **未決**: 筋書きを何日目に置くか（Phase 3）
      ⚠ **実装（Phase 1 以降）は利用者の指示を待つ**（2026-09-19「実装はしないで止めて」）
    - [ ] Phase 1: ⚠ **モードと排他（`mode.py`・`MODE`・`run.lock`）を最初に入れる** ＋ 仮の時計 `simclock.py` と `run_day.py` の「いま」の一本化
      ⚠ `MODE` が無ければ `real` ＝ いまの動きを 1 行も変えない。シミュレーションモードで本物の `run_day` を拒否したら、本物の `events.jsonl` に `refused_mode_sim` を残す（「起動しなかった日」の理由が後から分かる）。建玉が残っているときにシミュレーションへ切り替えるなら追加の確認
      ⚠ **仮の時計は接続先がモックのときしか受け付けない**（cert・prod・本番の鍵が付いていれば起動を拒否。テストで固定）。⚠ 仮の時計を渡さない実行は 1 行も変わらない（既存 38 本）
    - [ ] Phase 2: 仮データ（日足の再生・銘柄ごとの気配・合図の CSV）と運転手 `simrun.py`・`simctl.py`（速さ ／ 止める ／ 再開 ／ 1 日進める）。故障なしで 20 営業日を ×60 と最速で通す
      ⚠ モックの気配はいま全銘柄で 1 本 → 銘柄ごとに持てるよう `mock_server.py` を広げる（既存の selftest・`mockrun.sh`・管理画面のデモが変わらないこと）
    - [ ] Phase 3: 筋書き 9 つ（`Session offline`・5xx ／ 429・認証・資金不足・未約定・起動しない日・途中で落ちる・含み損 20%・停止ボタン）をモックと運転手に足し、1 つずつテスト
    - [ ] Phase 4: 管理画面（⚠ **表示だけ・操作は足さない**: モードに従って記録を選ぶ・全ページの帯「シミュレーション ×N ／ 停止中 ／ 仮の時刻」と `[SIM]`・「仮」の印・`/api/*` の `mode`・「今日」＝ 仮の今日・既存の停止ボタンの通し稽古・用語を `glossary.toml` に）
      ⚠ POST の経路は 1 本も増やさない（テストで固定）。`dashboard.md` §3 の権限の表は変えない
      ⚠ 停止ボタンが書く `HALT` と執行器の `TT_HALT_FILE` を同じファイルに向ける。⚠ 本物の `out/HALT` を書かないことをテストで確かめる。⚠ 公開面（g3plus）には出さない
      依存: 「Phase 2: 仮データ（日足の再生・銘柄ごとの気配・合図の CSV）と運転手 `simrun.py`・`simctl.py`（速さ ／ 止める ／ 再開 ／ 1 日進める）。故障なしで 20 営業日を ×60 と最速で通す」
    - [ ] Phase 5: 手順書（`live-trading.md`）・`CLAUDE.md`・後片付け。Phase 1（`predict.py --asof`）ができたら `T1`〜`T3` を `kind = "experiment"` で流す項を足す
