...
- [ ] トレーダー 3 人（それぞれ予算と 1 本以上の予測モデルを持つ）に予算を割り振り、tastytrade の本口座で実際に売買して記録を残す（実売買の仕組み） [plan](docs/plans/live-trading-three-models.md)
  ...
  - [ ] 管理画面（`dashboard/`）全体を設計しなおす
    ...
    利用者の方針（2026-09-18）: ⚠ **分かりやすさ重視・グラフも使う**。主な使い方は**各トレーダーの行動と実績と、その比較**（[プラン 1-6・1-7](docs/plans/dashboard-required-features.md)）
    - [ ] 管理画面に必要な機能を決める [plan](docs/plans/dashboard-required-features.md)
      利用者の指示（2026-09-18）
      - [x] Phase 1: 使う場面を書き出す（Claude が下書き → 利用者が足し引きする）（✅ 2026-09-18。残る場面 5 つ・期限つき 1 つ（記録と判定は観点 A が埋まるまで）・外す候補 3 つ。⚠ 主な使い方は「各トレーダーの行動と実績と、その比較」＝ いまの `/live` に推移と比較が無い。プラン 1-4〜1-6）
      - [x] Phase 2: いまある機能を棚卸しする（経路 31 本と照合）（✅ 2026-09-18。機能 31 行 ＋ 共通の部品 10。経路・テンプレート・JSON と照合済み。プラン 2-1〜2-4）
        旧タスク「現在の機能を列挙して、必要・不要を決める」をここに統合した（利用者の決定 2026-09-18）。必要・不要を決めるのは Phase 4 で、決めるのは利用者
      - [x] Phase 3: 突き合わせて、決定表の下書きを作る（見立て・理由・参照元）（✅ 2026-09-18。31 行 ＋ 足す 7 行・出し方の候補 4 つ。プラン 3-1〜3-4）
