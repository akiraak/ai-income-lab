...
- [ ] GPU 系を閾値売買でも回せるようにする（⚠ **いまは 5 実行とも毎日往復しか無い**。着手時にプランを作る）
  ...
  ⚠ **空白は 2 方向ある**（台帳の実測。[ledger.md](docs/specs/experiments/feature-discovery/ledger.md)）
    ⚠ **(1) モデルの軸**: ✅ **MLP は 2026-09-13 に埋めた**（own のみ。ownex は未実施）。⚠ **残るは Ridge+GAN増強・LightGBM+GAN増強 が毎日往復に 3〜4 行あるきり**。⚠ **MLP+GAN増強 は実装だけあって 1 度も回っていない**
    ⚠ **(2) 選別の軸**: ⚠ **閾値売買で回した選別は 4 本だけ**（F1-4・F1-7・F2-2・F5-1）。⚠ **9 手法が毎日往復にしか無い**（F1-1・F1-2・F1-3・F1-5・F2-3・F3-1・F3-2・F3-3・F3-5 ＝ 実装 13 − 閾値売買 4）。⚠ **`gpu_*` は F3-3 MDA と F3-1 Lasso を持っているので、移すと両方の空白が同時に埋まる**
  ...
  ⚠ **予測のスケールを先に測る**: `cli/calibdiag.py` は 2026-09-13 に直してあり、⚠ **`反復_旧` が 2 なら旧の解が止まっていたという意味**。✅ **MLP は 2026-09-13 に測った** — ⚠ **`pred_std` は Ridge と同じ尺度で潰れていない**。⚠ **GAN 増強 3 種はまだ測っていない**
  関連: 「DL / 進化的探索の手法を増やして検証を回す」
  ✅ **MLP × 閾値売買は 2026-09-13 に完了**（[記録](docs/specs/experiments/mlp-threshold-trading.md)）。⚠ **6 行とも「落とす」。n_trials 409 → 415**
    ⚠ **費用の見積りを実測で直した**: ⚠ **「MLP は 1 分未満」は毎日往復の数字で、(B) 銘柄別の 63 倍の fit を数えていなかった**。⚠ **実測は (A) 12 秒 / (B) 2 分 36 秒（1 fit 124ms）で、4 本合わせて 7 分 13 秒**
    ⚠ **GAN 増強の見積り（1 本 1.5〜2 時間）も同じ理由で過大な可能性がある**。⚠ **回すときに測り直す**
  - [ ] MLP × 閾値売買 × **ownex**（own cs rel ex。⚠ **手間 小**）
    派生元: 「MLP × 閾値売買（own 2018）」。⚠ **Ridge・LightGBM は own a/b ＋ ownex a/b の 4 本ずつあるので、MLP を横に並べるには 2 本足りない**
    ⚠ **`trade_ownex_lgbm_a.toml` / `_b.toml` の `model` を替えるだけ**。⚠ **表は `trade_ownex_ridge_a` を読む（作り直さない）**
    ⚠ **n_trials ＋6**（2 形式 × 3 θ）【推測】。⚠ **費用は own と同程度**【推測。own の実測 7 分から】
