...
- [ ] 保有日数の分布（中央値・分位点）を出し、保有が短い取引ほどコストが重いことを数字にする [plan](docs/plans/holding-days-distribution.md)
  派生元: 「疑問に思ったことを登録し解決していく」の子「保有日数の中央値など分布が知りたい。保有日数の短い取引ほど手数料が重くなってくるので」。利用者の指示（2026-09-17）で独立させた
  関連: 「逆の売買（売りの合図で建て・買いの合図で手仕舞う）の数値を確認し、通常と逆の 2 つの損益から評価軸を作れるか検討する」（Phase 3 の回し直しを同じ構成で兼ねる）
...
- [ ] 逆の売買（売りの合図で建て・買いの合図で手仕舞う）の数値を確認し、通常と逆の 2 つの損益から評価軸を作れるか検討する [plan](docs/plans/reverse-trading-check.md)
  派生元: 「疑問に思ったことを登録し解決していく」の子「売りの時に買い、買いの時に売りという逆の売買したときの数値を確認する」。利用者の指示（2026-09-17）で独立させた
  関連: 「保有日数の分布（中央値・分位点）を出し、保有が短い取引ほどコストが重いことを数字にする」（Phase 3 の回し直しを同じ構成で兼ねる）
  ⚠ **先に分かっていること**（プラン §1-3）: θ ≥ 50 では逆売買（入口% ↔ 出口%）は元の売買の**補集合**になり、逆の対 B&H 上乗せ ＝ −元の純利 − 2×コスト ＋ 5bp。⚠ **元の純利が負でない限り逆は B&H を超えない**（元が B&H に負けているだけでは足りない）。1 実行の手計算【2026-09-17】: 元の上乗せ −277 → 逆の上乗せ −1,544（θ=50）
  ⚠ **空売り版（ポジション −1）は回さない**（rules 13-4 規約 2・16-1 規約 5。状態機械の変更は検証方式の変更）。算術で見積もるだけ（プラン §2-5）
  - [x] 通常と逆の 2 つの損益から新しい評価軸を作れるか検討する（2026-09-17）
    答え: 逆の値は元の値から恒等式で決まるので情報は増えない。和・差・比・保有日と見送り日の平均差は、腕が消える／露出が混ざる／分母が潰れる、で使えない。⚠ **残るのは S ＝ 粗利 − 保有日率 × B&H 粗利（露出を揃えた選日の上乗せ）だけ**で、これは乱択ゲート（rules 14-6 b）の期待値を乱数なしで厳密に出したもの。逆売買では −S になる（配線の検査）。⚠ **診断であって採否には使わない**（rules 14-3）。詳細は [plan §1-5](docs/plans/reverse-trading-check.md)
  - [ ] Phase 1: 既存 151 実行の `per_symbol.csv` から恒等式で逆売買の純利・上乗せと S を出す（読むだけ・再実行なし）
  - [ ] Phase 2: 入替えが補集合になることをテストで縛り、`result.csv` / `checks.json` に逆売買の診断列と `selection_edge`（S）を足す（⚠ 既存の数字は 1 ビットも変えない・採否に使わない・試行に数えない）
  - [ ] Phase 3: 代表 1 構成（＋ leak 対照。holding-days の Phase 3 と同じ構成で兼ねる）を回し、算術と実測を突き合わせて `reverse-trading.md` に答えを書く
  - [ ] rules.md 14-3 に「逆売買と S は診断列。採否に使わない・試行に数えない。逆を手法として採るなら向きを事前に書いて新しい試行にする」を 1 行足す
