...
- [ ] トレーダー 3 人（それぞれ予算と 1 本以上の予測モデルを持つ）に予算を割り振り、tastytrade の本口座で実際に売買して記録を残す（実売買の仕組み） [plan](docs/plans/live-trading-three-models.md)
  ...
  - [ ] Phase 0: 決めごとと本番の読み取り・dry-run（観点 A の確認・`probe`・端株と注文種別の dry-run・トレーダーの定義（属性・合成規則）／初期の 3 人のモデル・予算・銘柄／時刻／停止条件／判定の閾値を `docs/specs/experiments/live-trading.md` §0 に書き、利用者の確認を取る）
    推奨: T1 `trade_own_ridge_a` θ=50 ／ T2 `trade_ownex_lgbm_a` θ=55 ／ T3 `trade_ownseq_ridge_a` T3 QUANT θ=50（1 人 1 モデル・合成「そのまま」）／ 対照 B&H は紙上だけ ／ 予算 $300 × 3 ＋ 予備 $100 ／ 執行は 15:50 ET の気配で合図・15:55 に成行（プラン §2-1〜§2-4）
  - [ ] Phase 1: 「今日の買い%」の経路（`cli/predict.py --asof`。`evaluate_trading` と同じ関数群で訓練 ＝ 昨日まで・検証 ＝ 今日の 63 行。既定経路は 1 ビットも変えない・先読みテスト・決定性）
  - [ ] Phase 2: トレーダーと執行器 `experiments/live-trading/`（`trader.py` ＋ `config/traders/*.toml`・合成規則のテスト・状態機械・予算の上限・3 人を銘柄ごとに合算・dry-run → 発注 → 約定確認 → 取消・再送・記録は `Masker` 経由。モックで 20 日 → sandbox で市場時間に 1 日）
  ...
  - [ ] Phase 5: 本番投入（**利用者が行う**。最小額で 1 発注 ＝ 旧 Phase 6 → 3 人を予算どおりに → 複数モデルのトレーダーを足すならその後。Claude は手順書とチェックリストまで）
