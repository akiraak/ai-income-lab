...
- [ ] トレーダー 3 人（それぞれ予算と 1 本以上の予測モデルを持つ）に予算を割り振り、tastytrade の本口座で実際に売買して記録を残す（実売買の仕組み） [plan](docs/plans/live-trading-three-models.md)
  ...
  - [ ] 2026-09-22（火）までに実運用へ: 日ごとの段取り [plan](docs/plans/live-trading-go-live-0922.md)
    ...
    - [~] 9/22（火）: 本番投入 → ⚠ **投入せず 9/23（水）へ持ち越し**（[プラン §4-1](docs/plans/live-trading-go-live-0922.md)）
      期日: 2026-09-22
      ⚠ **関門は Go のまま・技術的な問題は 1 つも出ていない**。ずれた理由は**窓に間に合う時刻に起動しなかった**ことだけ。⚠ **本番の注文は 0 件・口座は動いていない・記録も汚れていない**【実測】
      ...
      - [~] 06:45 新しい記録の作り（DB。2026-09-21 夜に替えた）を通しで確かめる（⚠ 1 つでも外れたら `git revert` で `acc5f9f` の作りに戻して、本番は今までの作りで行う）
        ...
        🔶 4（本番の dry-run）は利用者が 12:14 PDT に流した ＝ **予測は DB に入った（174 行 → `predict-20260922T191432Z.jsonl`）が、執行器は窓の外で拒否**（15:14 ET ＜ 15:45）。⚠ **dry-run そのものは通していない**ので、9/23 の投入が新しい作りでの初めての本番の発注になる
