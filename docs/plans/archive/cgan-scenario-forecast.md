...
> ✅ **完了（2026-09-19）**: Phase 0〜5 が済んだ。判定は「落とす」（cGAN は履歴ベースと区別がつかず、軽量モデルに負けた）。設計決定・事前固定した数値・採用基準・結果・採用判断は、記録 [cgan-scenario.md](../../specs/experiments/cgan-scenario.md) に書いた。⚠ **下の §0-2 の「Phase 0 で決める」は全部そこで決まっている**（食い違ったら記録が正）。
...
| 場所 | 変更 |
| --- | --- |
| `ail/scenario/` | ⚠ **Phase 0 の決定 1 で 1 つのパッケージにまとめた**（registry・台帳に載せないため）。`data.py`（✅ Phase 1。60 日窓 → 5 日の答え・分割・標準化）／ `model.py`（条件付き WGAN-GP。既存 `gan.py` は触らない）／ `metrics.py`（CRPS・被覆率・Brier）／ `baselines.py`（比較対象 3 種） |
| `config/scenario/` | ✅ `cgan_spy.toml`（ハイパラ・seed・分割を設定に分離。`config/experiment/` とは分ける） |
| `ail/config.py` | ✅ `scenario()` を足した |
| `cli/scenario.py` | 学習・評価・指定日予測・最新日予測の入口（`cli.run` には載せない。決定 1） |
| `tests/` | §8 のテスト一式（✅ テスト 1〜3 は `tests/test_scenario_data.py`） |
