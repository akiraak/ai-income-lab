...
### 4-1. 2026-09-22（火）に起きたこと — **投入は 9/23（水）へ 1 日ずれた**

⚠ **関門は Go のままで、技術的な問題は 1 つも出ていない**。ずれた理由は**発注できる時間帯に間に合う時刻に起動しなかった**こと、ただ 1 つ。

| 時刻 PDT（ET） | 起きたこと |
| --- | --- |
| 11:33〜11:58（14:33〜14:58） | ✅ DB の通しテストが全部通った（cert の `test_a` の往復・控え・突き合わせ・紙上の対照・管理画面・秘密の grep）。**`acc5f9f` に戻す必要なし** |
| 11:55（14:55） | ✅ 前営業日の注文が注文番号で読めた（1663195 ／ 1663301）＝ §0-8 の段 1 が成立 |
| 12:14（15:14） | 本番の dry-run を起動 → **発注できる時間帯の外**（15:45 より前）で執行器が拒否（`out_of_window`）。⚠ 予測 174 行は DB に入った ＝ 新しい作りは本番の経路でも動く |
| 13:01（16:01） | 本番投入を起動 → 準備（日足 53 秒 ＋ 予測）の途中で**利用者が中止**。⚠ **本番の注文は 0 件・口座は動いていない・記録も汚れていない** |

⚠ **踏んだ形（直した）**: `--wait` は「15:50 ET まで待つ」だけで、**既に過ぎていたら素通りする**。遅れて起こすと準備の約 95 秒の後に **引け（16:00 ET）後の成行**を 10 本投げることになる。
→ `run-live.sh` に**引けに間に合うかのブレーキ**を足した（`--mode submit` ＋ 今日の日付のときだけ。発注の開始の見込みが刻限〔既定 15:58 ET〕を過ぎるなら **rc=11 で起動しない**。`AIL_SUBMIT_DEADLINE` ／ `AIL_PREP_SECONDS` で動かせる・`--ignore-window` を書いた回は素通り）。

⚠ **D4（自動起動 timer）の前提が崩れた**: 「水曜から」は**火曜に手で 1 回成功を見てから**が前提だった。手動の初日が水曜になったので、**timer は木曜から**にずらす。

### 4-2. 2026-09-23（水）の段取り

| 時刻 PDT（ET） | 作業 | 担い手 |
| --- | --- | --- |
| 〜12:44 | 準備は不要（DB のテストは 9/22 に通過済み・予測は発注の回が自分で行う） | — |
| **12:45（15:45）** | **`T1`〜`T3` を規模 A で本番投入**（⚠ **12:45 に起動する**。準備に約 95 秒かかるので 12:50 起動では引けまで 8 分しかない） | **利用者** |
| 13:05〜（16:05〜） | 確認: 全注文の `final_status`・`reconcile.py --env prod show` で口座 − 売買履歴 ＝ 0・`events.jsonl`・管理画面・秘密の grep | Claude ＋ 利用者 |
| 引け後 | `TODO.md` ／ `DONE.md` ／ `CLAUDE.md` の更新・このプランを archive へ | Claude |
| 9/24（木） | timer ／ cron を入れる（⚠ 入れるのも許可を置くのも利用者） | 利用者 |

```bash
cd /home/ubuntu/ai-income-lab
TT_ALLOW_PROD_ORDERS=1 ./run-live.sh --traders T1,T2,T3 --mode submit --wait -- --env prod --i-know-this-is-real-money
```

⚠ 12:45 より前に起こせば `--wait` が 12:50（15:50 ET）まで待つ。⚠ 12:56 PDT を過ぎて起こすとブレーキが止める（それが正しい）。
...
## 12. 2026-09-23（水）の結果 — 投入した（実運用に入った）

利用者が 10:04:51 ET に起動し、`--wait` が 15:50:00 ET まで待ってから流れた【実測】。

| 段 | 結果【実測】 |
| --- | --- |
| 準備 | 日足の更新 55 秒 ＋ 予測 3 本 43 秒（15:50:00 → 15:51:38）。ブレーキ（rc=11）は掛からず |
| 発注 | 合図 15 → 意図 10 → 注文 10・約定 10・問題 0（15:51:40 → 15:51:49）。`T1`・`T3` が 5 本ずつ・`T2` は 5 銘柄とも `skip`（買い% 49.67 < θ 55） |
| 確認 | 全注文 `Filled`・`reconcile.py --env prod show` の差 0・控えの未完 0・管理画面 9 ページ 200・秘密の grep 0 件 |
| 記録 | [live-trading.md §1](../specs/experiments/live-trading.md) に 3 行・[DONE.md](../../DONE.md) |

⚠ 残り: timer は 9/24（木）から（利用者。D4）／ Phase 6 の 20 営業日目は 2026-10-20【計算】／ 手数料は見積りのまま。このプランは archive へ。
