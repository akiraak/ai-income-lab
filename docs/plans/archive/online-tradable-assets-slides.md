...
### ⚠ 2026-08-31 追記 — 上の 9 件のうち 3 件は誤検出だった

`TODO.md` の起票内容を元文書に当たり直したところ、**#3・#6・#8 は食い違いではなかった**。取り下げる。

| # | 取り下げ | 理由 |
| --- | --- | --- |
| 3 | BSM の 10 年「−8.8%」／「−0.9%」 | §13-5 に「10 年で $1,000 → $912（**−8.8%、年率 −0.9%**）」とある。⚠ **累計と年率**で同じ事実の 2 通りの書き方。§14-15 の列見出しは「長期の総リターン（**年率**）」なので両方正しい。⚠ **この誤りはプラン §5 の #3 から引き継いだもの** |
| 6 | BSM の分配「8.19〜8.60%」／「8.24%」 | ⚠ **8.24 は範囲の中**。矛盾ではない |
| 8 | §14-3 JBBB の「8.5%」／「7%」 | §13-5 が「分配利回り **7〜8.5%**」と範囲で書いている。8.5% は **30 日 SEC 利回り**（出典 §12-10 の Janus Henderson ファクトシート。⚠ **2026-08-31 に通し番号を #23 → #31 へ振り直した**）、7% は**分配率**で別の指標 |

⚠ **突合の方法の反省。**数値トークンを 1 個ずつ突き当てる照合は「同じ数字があるか」しか見ておらず、
**「累計と年率」「範囲と代表値」「SEC 利回りと分配率」のような単位・定義の違いを食い違いとして拾ってしまう。**
数字が割れているように見えたら、**先に列見出しと定義を読む**こと。

残る #1・#2・#5・#7・#9 は実在する。⚠ **うち #1（§14-14）は数字違いではなく、節が §14-15 より前の状態で
取り残されている**（実測 8 銘柄 → §14-15 で 17 銘柄、参照先が自己参照、内訳合計 79 ≠ 96）。
