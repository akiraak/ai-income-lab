...
## 8. 引き継ぎ（✅ 2026-09-22 夜に手順 7 まで全部済み）

**結果: 間に合う**（13500t の温まった状態の中央値 47.93 ／ 48.41 秒 ≤ 60 秒・売買の判定は一致）。記録と判定は [live-trading.md §0-12](../../specs/experiments/live-trading.md)。手順 4〜7 は Sx360 の Claude が行った。
...
4. **スナップショットを 13500t へ写す** ── ⚠ **titan から直接は入れない**【実測 2026-09-22】: titan の `~/.ssh/config` に 13500t が無い・tailnet に居るのは titan と sx360 の 2 台だけ・`~/g3plus-ops` も titan に無い。**Sx360 を経由する**（プランの当初の代案）。titan 側は固めて置いてある:
   ...
   ✅ 2026-09-22 20:37 PDT: Sx360 で `scp titan:` → `sha256sum -c` OK → 13500t へ → 13500t でも OK → 空の `data-live/` に `--strip-components=1` で展開（70MB）→ 63 銘柄とも最終 `time_ms = 1790035200000`。⚠ Sx360 の titan の鍵はエージェント（`~/.ssh/agent.sock`）が WSL の再起動で消えていたので、利用者が別の端末で `ssh-add` した（`!` からはパスフレーズを入れられない）
5. **13500t で測る**（g3plus-ops の手順書の 5・6。`numba-cache/` を消してから）

   ✅ 2026-09-22 20:37〜20:47 PDT（12 回とも `ok`・同時に動いていたのは cloudflared だけ）。結果は 13500t の `~/ail-bench/out/13500t-20260922-2040.jsonl`（Sx360 の `~/ail-bench/` にも写した）
6. **並べる**:

   ✅ **rc=0（判定が変わった銘柄なし）**。T3 だけ最大 0.003 ずれた。⚠ 入力の指紋が 2 台で違った → Sx360 で T1 を流すと 13500t と同じ指紋（PDT でも UTC でも）＝ 時刻帯・コンテナではなく CPU の命令（AVX-512 の有無）【推測】。詳しくは live-trading.md §0-12
 2 つの JSONL を 1 台に集めて `bench_predict.py --compare titan-20260922-2000.jsonl 13500t-….jsonl`。⚠ rc=1（判定が変わった銘柄がある）なら時間の比較より先に原因を調べる
...
7. **記録と判定**（Phase 4）: `docs/specs/experiments/live-trading.md` に節を足す（機械の仕様・冷えた ／ 温まった の表・§2-3 の判定・判定の一致・スナップショットの最終日）。TODO の Phase 2〜4 に印、判定を「13500T で予測にかかる時間を測る」へ写す。済んだらこのプランを archive へ
   ...
   ✅ 2026-09-22: live-trading.md §0-12・TODO・DONE に写し、このプランを archive へ移した
