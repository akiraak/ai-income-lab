...
| 本 | 反復数 | ⚠ 直列の見込み |
| --- | ---: | ---: |
| ⚠ **前置きの門**（どの実行にも必ず付く） | 2 選別 × 330 batch × 300 epoch ＝ **198,000** | ⚠ **1.07 時間** |
| (A) 共通 1 本（門を除く） | 4 fit/fold × 330 batch × 300 epoch ＝ **396,000** | **2.15 時間** |
| (B) 銘柄別 1 本（門を除く） | 63 銘柄 × 4 fit × 8 batch × 300 epoch ＝ **604,800** | **3.28 時間** |
| (A) 1 本 **＋門** | — | **3.22 時間** |
| (B) 1 本 **＋門** | — | **4.35 時間** |
| 1 モデル（(A)(B) × 本番・leak） | — | **15.1 時間** |
| **3 モデル ＝ 12 本** | — | ⚠ **45.5 時間（直列）／ 約 20 時間（4 本並列）** |

⚠ **着手時の見込み「1 本 1.5〜2 時間」より (B) が 2.2〜2.6 倍重い。** ⚠ **過去 4 回は過大に外していたが、今回は過小だった。**

### 5-4. ⚠ **門が全実行に 1.07 時間ずつ乗る** — ⚠ **使わないと決めた診断に費用の 24% を払っている**

⚠ **`cli/run.py` は `--ignore-gate` を付けても `gate.evaluate_gate` を必ず先に回す**（値を `checks.json` に残すため）。
⚠ **その中で 5 fold × 2 選別 ＝ 10 回モデルを fit する**ので、⚠ **GAN では 1 実行あたり 1.07 時間になる。**
...
| | 4 本並列の壁時計【推測】 |
| --- | ---: |
| batch 1,024（12 本） | **20.8 時間** |
| batch 16,384（12 本） | **7.7 時間** |
| **合計 24 本** | ⚠ **約 28 時間** |
