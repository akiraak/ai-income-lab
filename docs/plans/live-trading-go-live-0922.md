...
⚠ **関門は Go のままで、技術的な問題は 1 つも出ていない**。ずれた理由は**窓に間に合う時刻に起動しなかった**こと、ただ 1 つ。
...
| 12:14（15:14） | 本番の dry-run を起動 → **窓の外**（15:45 より前）で執行器が拒否（`out_of_window`）。⚠ 予測 174 行は DB に入った ＝ 新しい作りは本番の経路でも動く |
