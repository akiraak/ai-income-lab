...
| ★ b | **このモデルの組み立て**（軸の表） | 入れるデータ ／ 数字の下ごしらえ ／ 計算の仕方 ／ 当てにいく対象 ／ 学ぶ範囲 の 5 行（新しい欄 `axes`）。⚠ 1 本のモデルの中だけの表（モデルどうしを横に並べない） | 小。config とコードから写せる |
...
| `mlp` | 「線の引き方を 2 通り」 | **学ぶ範囲**が 2 通り（全部の株でまとめて ／ 株ごと）。0〜100 に直す式は 1 通り |
