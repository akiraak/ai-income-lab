...
| 左ペイン（タイムライン） | 日ごとの見出し（`9/21（月）`・今日は「今日」の印・済 ／ 全部）→ 時刻の順（時刻の無い行は書かれた場所に置く）。各行 ＝ 時刻・状態の記号・文面（2 行まで）。⚠ **済んだタスクは既定では出さない**（ツリーと同じ。全部済んだ日は見出しごと出ない）。`済みも出す` を入れたときだけ薄く出す（2026-09-20 に直した。§7 の追記）。過ぎたのに未完の行に「過ぎた」の印（色だけにしない）。今日の見出しの中に「いま」の線。末尾に「期日なし N 件」 |
...
### 追記（2026-09-20）: 済んだタスクは既定では出さない

- 利用者の指摘:「終わっているのだとしたらタイムラインの 20 日に表示されてしまっているのは何故？」＝ 最初の版は「1 日の進み具合を読むため」に済んだ行も薄く出していたが、ツリー（済んだタスクは並べない）と食い違い、済んだものがまだ残っているように読めた
- 直し: 既定ではツリーと同じく済んだタスクを並べない（全部済んだ日は見出しごと出ない）。左ペインの `済みも出す`（`localStorage` に覚える）を入れたときだけ薄く出す。見出しの 済 ／ 全部 の数はどちらでも出るので、進み具合は数で読める。全部済んだ日の一覧（`#tasks/@day/<日付>`）を直接開いたときは、その旨を 1 行出す
- 確認【実測。3011・ヘッドレスのブラウザ】: 既定 ＝ 9/21・9/22 だけ ／ `済みも出す` ＝ 9/20 も出る ／ 入れ直しても覚えている ／ 画面のエラー 0 件
