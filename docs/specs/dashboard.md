...
### 13-8. 見張り「今日の起動が無い」（2026-09-24。3 台の役割分け Phase 5。[プラン](../plans/archive/three-machines-phase5.md)）

> この図の主張: 見張りは**記録・暦・時刻だけ**で決める（timer ／ cron ／ systemd の状態は読まない）＝ titan でも 13500t のコンテナでも同じ作りで出る。

```mermaid
flowchart LR
  T["いま（ET）"] --> Q1{"今日は営業日 ?"}
  Q1 -- "いいえ" --> N["出さない（休場日）"]
  Q1 -- "はい" --> Q2{"16:15 ET を過ぎた ?"}
  Q2 -- "いいえ" --> N2["出さない（窓の前）"]
  Q2 -- "はい" --> Q3{"out/&lt;今日&gt;/ に記録 ?"}
  Q3 -- "はい" --> N3["出さない（記録あり）"]
  Q3 -- "いいえ" --> W["⚠ 今日の起動が無い"]
...
| 項目 | 決め |
| --- | --- |
| なぜ | `board()` の「起動しなかった日」は**記録のある最初の日と最後の日の間**の営業日しか数えないので、今日が起きなかったことは翌日に記録が付くまで出なかった（無人運転の判定の穴） |
| 定義 | 営業日（`nyse_calendar`。半日立会も営業日）で **16:15 ET**（発注できる時間帯の終わり 16:05 ＋ 猶予 10 分。`live.WATCH_AFTER_ET`）を過ぎ、`out/<今日>/` に記録が 1 つも無い。⚠ 起動して rc≠0 で落ちた日（記録はある）は別の観点（TODO「失敗したときに気づける形にする」） |
| 出す場所 | 概要の監視の帯（赤「⚠ 今日の起動が無い」。5 秒ごとの部分更新に乗る）／ 大きな数字「起動しなかった日」に今日を足し「今日（日付）を含む」／ 全体の詳細の日次の表に今日の行（`row-missing`）／ トレーダーの段のマス目に今日の「起動なし」／ `/api/live` の `watch`（`date`・`after_et`・`checked`・`missing`・`why`） |
| 数え方 | 条件を満たした日だけ、営業日の列 `bd` を今日まで延ばす（`missing` に今日が入る）。⚠ 記録の最初の日より前は今までどおり数えない。⚠ 窓の前・記録あり・休場日は列を延ばさない（数字が変わらない） |
| 判定しない（`checked: false`・`why` に理由） | 印 `NOT_PRODUCTION` のある機械（読むだけの写し。起動しないのが正しい）／ `HALT` の日（止めたのは人）／ デモ（記録は過去の日付）／ 時刻が分からない（シミュレーションで仮の時計が無い） |
...
⚠ **2026-09-21 から、試した経緯の数（何通り・内訳）は研究の DB から機械で数える**（プラン [db-model-facts.md](../plans/archive/db-model-facts.md) の Phase 3。利用者の裁定「予測モデルの解説と DB での永続化を同時にやった方が良い」「予測モデルの命名規則を作らないと解説に使えない」・`ledger_rows` を書くことの了承）。
