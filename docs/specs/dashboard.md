...
> この図の主張: 言葉の正本は 1 本の TOML、数字は実売買の設定から写すだけで、売買の記録にも管理画面（3012）にも触れない。
...
| # | 大見出し | 中身 | 出どころ |
| --- | --- | --- | --- |
| 1 | 使うモデル | モデルごとの札: やさしい呼び方・**正式な名前（実験名と手法）**・ひとこと・見るもの ／ 決め方。モデルが複数なら、点の合わせ方（平均・多数決など）を 1 文 | 設定の `[[models]]` → `traders.toml` の `[[model]]`（⚠ **実験名と手法の両方が同じもの**だけを当てる。無ければ「説明はまだ」） |
| 2 | モデルの特性 | 1 項目 1 行 ＋ **どこまで確かかの印** ＋ 理由（小さい字。空なら出さない） | `[[model]]` の `traits`（`text`・`why`・`basis`） |
| 3 | 何を見て、どう点を出すか | 見ている数字（小見出しつきのまとまり）＋ 学ぶ → 点をつける の 2 手順 | `[[model]]` の `sees`・`how` ＋ `[common]` |
| 4 | この人の決まり | 買う線と売る線（文 ＋ **点のものさし**）／ いつ ／ いくら（予算・1 本あたり・買い方・お金の決まり） | 設定の数字 ＋ `[common]` |
...
- **点のものさし**: 0〜100 の帯を 売る ／ 何もしない ／ 買う に塗り分ける。⚠ **幅は設定の買う線から出す**（θ=55 なら 45 ／ 10 ／ 45。θ=50 なら 2 帯で「何もしない」が無い）。狭い帯には短い札（`ruler_wait_short`）
...
⚠ **`traders.toml` の本文では下の左の語を使わない**（`tests/test_traders_tab.py` の `FORBIDDEN` が機械的に確かめる。⚠ **語を足すときはこの表とテストを一緒に直す**）。

| 使わない語 | 言い換え |
| --- | --- |
| 買い%・出口%・合図・シグナル | 「明日上がりそうかの点（0〜100）」 |
| θ・閾値 | 「買う線」「売る線」 |
...
1. 文を直す ＝ `dashboard/traders.toml` だけを直す（sidecar の入れ直しは要らない。見張りが 5 秒おきに mtime を見て画面を入れ替える）
2. **人を足す ＝ 実売買の設定を置くだけ**（`config/traders/<鍵>.toml`。一覧に出る）。呼び名を付けるなら `[nicks]` に 1 行（⚠ 意味の無い名前・使い回さない）
3. **新しいモデルを使う人を足す ＝ `[[model]]` を 1 つ書く**（`name`・`method` は設定と同じ綴り。`traits` には必ず `basis`）。書くまでは「このモデルの説明はまだ書かれていません」と出る。⚠ モデルや θ を替えるのは新しい試行（CLAUDE.md）なので、先にそちらの決めごと
4. `cd dashboard && .venv/bin/python -m pytest -q tests/test_traders_tab.py`（いま設定に居る人の全部のモデルに説明があること・やさしい言葉・くらべる文が無いこと・印）
