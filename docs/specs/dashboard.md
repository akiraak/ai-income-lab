...
⚠ **2026-09-18 に要不要と画面の形を決めた**（残すのは監視・実売買・停止と解除と操作の履歴の 15 行 ＋ 足す 7 行。記録と判定は観点 A まで。検証・データ・手動の注文・開発は外す。構成は「上に全体の概要、その下にトレーダー 1 人 1 段、詳細はクリック」・ナビは左ペイン・日次は「全体の詳細」・見た目はデザイン 3「数字とグラフが主役」）。⚠ **画面はまだ変えていない**。決定と見本は [dashboard-required-features.md](../plans/archive/dashboard-required-features.md)
