...
プラン: [docs/plans/feature-discovery.md](../../plans/feature-discovery.md) ／ コード: `experiments/feature-discovery/`
...
> ⚠ **自前の数字の扱いは E8 から引き継ぐ**（[プラン §0-1](../../plans/feature-discovery.md)）。**悪い結果はそのまま結論にしてよいが、良い結果は根拠「中」が上限**で、
...
⚠ **この数字は「悪い数字」なので、そのまま結論として使う**（[プラン §0-1](../../plans/feature-discovery.md) の取り決め）。
...
⚠ **この列を入れて精度が跳ね上がらなければ、検証の配線そのものが壊れている**ことを意味する（[プラン §6](../../plans/feature-discovery.md) の検査 1）。
...
| 4 | ⚠ **デフレーテッド SR をまだ通していない。** 今回は全部が負けなので不要だったが、⚠ **良い数字が出たときは必須**（[プラン §0-1](../../plans/feature-discovery.md)） |
...
（[プラン §0-1](../../plans/feature-discovery.md) の取り決めでは、良い数字が出たときに必須）。
...
| 根拠の強さ | ⚠ **「悪い数字」なのでそのまま結論にしてよい**（[プラン §0-1](../../plans/feature-discovery.md)）。⚠ **ただし「効かないことの証明」ではなく「この設定では効かなかった」である** |
...
| 4 | 検証と判定 | ⚠ **1 分足は ✅、日足は無効**（[§6-3](#6-3--汚染の大きさ--4-の日足の数字は無効)。調整前のデータで測っていた） |
| — | ⚠ **分析の構造化**（別タスク） | ✅ [rules.md](feature-discovery/rules.md) ＋ 4 層の特徴量 ＋ 実行記録（§6） |
| 5 | ⚠ **断面での検証**（`cs_` `rel_` `ll_`） | ⏳ 土台は §6-5 で用意済み |
