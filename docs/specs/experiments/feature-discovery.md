...
## 2. 特徴量を見つけ出す手法のカタログ（Phase 2・5 系統 26 件）
...
| ID | 手法 | 量 | 相関 | 検定 | 拠 | ⚠ 見どころ |
| --- | --- | --- | --- | --- | --- | --- |
| F3-1 | Lasso・Elastic Net（実装は 0 本→先頭列） | 低 | ⚠ | 無 | 強 | ⚠ **相関した特徴量の中から 1 本だけ残す**（どれが残るかは標本次第）。⚠ **2026-09-13 に判明**: この実装（`LassoCV`）は α が強いと **係数が全部 0** になり、⚠ **フォールバックが列の並び順どおり先頭の 1 本を返す**。⚠ **台帳で本数 1.0 の行は選別ではなく「先頭列 1 本」を測っている**（1.2〜1.6 は一部の fold だけ）。⚠ **数字は正しいが名前が中身と合っていない。** 直したものは F3-1b |
| F3-1b | Lasso（本数を固定・lasso_path） | 低 | ⚠ | 無 | 強 | ⚠ **F3-1 の不具合を直した版**（2026-09-13）。α を強い側から辿り、⚠ **非ゼロが k 本に達した最初の α で k 本返す**ので 0 本にならない。⚠ **F3-1 と別 ID なのは、台帳の鍵が ID だから**（同じ ID にすると既存 20 行と同じ行にまとまる）。⚠ **既存行は再計算しない**（[rules.md 14-10 規約 5](feature-discovery/rules.md)） |
...
| — | ⚠ **試した結果の台帳**（別タスク） | ✅ [ledger.md](feature-discovery/ledger.md)（生成物）。⚠ **回した件数・未実施の件数はここに書かない** — ⚠ **台帳 §3 の「系統ごとの進み具合」が唯一の出所である**（2026-09-13。⚠ **ここに写した数字が 9 件 / 16 件のまま古くなっていた**） |
