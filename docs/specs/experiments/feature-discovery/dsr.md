...
作成日: 2026-09-12。`checks.json` の `dsr` と検証結果一覧・管理画面に出る DSR を読むための図解。
プラン: [dsr-explainer.md](../../../plans/archive/dsr-explainer.md) ／ 関連: [units.md](units.md)（検証結果一覧の単位）・[validation-power.md](validation-power.md)（検出限界）
...
⚠ **実装 `stats.deflated_sharpe` に入れ直して再現したもの**である。⚠ **新しい検証は 1 つも回していない**（回すと `n_trials` が動く）。
...
```mermaid
flowchart LR
  A["手法を N 個試す<br/>（検証結果一覧の検証 = 139）"] --> B["最良の 1 つを選ぶ<br/>＝ checks.json の best"]
...
flowchart LR
  SR["実測 SR"] --> D["差 = SR − SR0"]
  N["n_trials（検証結果一覧の検証数）"] --> S0["SR0<br/>偶然の最良"]
  T["n_obs（標本の数）"] --> S0
  S0 --> D
  D --> Z["z = 差 × √(T−1) ÷ 補正"]
  T --> Z
...
### 2-1. SR0 は「検証数の対数」でしか増えない

⚠ **ここが直感に反する。** 検証を 5,000 倍にしても SR0 は 7 倍にしかならない（n_obs = 1,801 での【実測】）。

| n_trials | 2 | 10 | 36 | 67 | 94 | **139**（現在の検証結果一覧） | 278 | 10,000 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
⚠ **T（標本）は平方根で効き、N（検証）は対数の平方根でしか効かない。** だから⚠ **検証を減らすより標本を増やすほうが遥かに効く**（§5）。

## 3. 入力の 4 つはどこから来るか — ⚠ 2 経路で定義が違う

⚠ **同じ `dsr` という項目名でも、毎日往復（旧方式）と閾値売買（新方式・[rules.md 13 章](rules.md)）で中身が違う。**
...
| **n_obs** | ⚠ **実効観測数**（評価期間に回った行数 × 実効系列数 ÷ 系列数）。例: 80,641 行 → **10,870**（48 系列の実効 6.47 本） | ⚠ **検証日数**（例: 2018 表 1,801 日／1995 表 6,619 日）。⚠ **行数 63 × 日数 を使わない** |
| **n_trials** | 検証結果一覧が数えた検証数（`checks.n_trials_now()`） | 同左 |
| **歪度・尖度** | ⚠ **渡していない（既定 0 / 3 ＝ 正規分布と仮定）** | ⚠ **系列から実測して渡す**（例: 歪度 −1.02・尖度 30.4） |

> この図の主張: 上の段（毎日往復）と下の段（閾値売買）は、SR も n_obs も別の量である。DSR という項目名が同じなだけ。
...
⚠ **その 0.9276 も、いまの検証数 139 で計算し直すと 0.8318 になる**（§5-1）。

## 5. 効き方の実測 — レバーは 2 本、効きは対等ではない

> この図の主張: 検証を増やすと SR0 が上がって DSR は下がる。標本を増やすと SR0 が下がり、同時に √(T−1) が伸びるので、⚠ **DSR には二重に効く**。

```mermaid
flowchart LR
  N["検証を増やす<br/>N ↑"] -->|"√ln N でしか効かない"| S0["SR0 ↑"] --> DN["DSR ↓"]
  T["標本を増やす<br/>T ↑"] -->|"1/√T"| S0B["SR0 ↓"] --> DU["DSR ↑"]
  T -->|"√(T−1)"| Z["z の倍率 ↑"] --> DU
```

### 5-1. 検証を増やすと下がる（SR は固定・n_obs = 1,801）
...
⚠ **評価期間の延長（[rules.md 14-4](rules.md)）が効くのはここである。** 実例が橋渡し対（同じ手法・同じモデルを 2018 表と 1995 表で回した対）で、⚠ **SR は下がったのに DSR は上がった**:
...
| 2 | ⚠ **検証結果一覧の採否（採る / 保留 / 落とす）** | `catalog.judge` は ⚠ **DSR を見ない**。判定は上乗せの符号だけで決まる。DSR は「⚠ **良い数字を報告してよいか**」の関門（11 章 規約 3） |
...
| 「DSR が低いのは検定が厳しすぎるから」 | 検証 139 のぶんの下駄（SR0）を脱がせているだけ。⚠ **下駄を履いたまま報告するほうが誤り** | §1・§5-1 |
...
| # | 近似・限界 | ⚠ 効き方 |
| ---: | --- | --- |
| 1 | ⚠ **SR0 の分散を `1/n_obs` で近似している**（原典は「検証どうしの SR のばらつき」を使う） | ⚠ **検証が互いに強く相関しているとき（本プロジェクトはそう）、本当のばらつきはもっと小さい ＝ SR0 は大きめに出る**【推測】。⚠ **向きは厳しい側なので、そのままにしてある** |
...
| 4 | `n_trials` の正本は検証結果一覧であり、⚠ **検証結果一覧が数え落とせば DSR は甘くなる** | 検証結果一覧の識別項目（[units.md §4](units.md)）が自由度をすべて拾えているかに依存する |
| 5 | ⚠ **0.95 という水準に必然性はない**（慣例） | ⚠ **水準を後から下げるのは評価期間のデータでの最適化**になる。動かすなら事前に決める |
