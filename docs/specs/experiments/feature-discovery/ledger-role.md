...
| 行をどう読むか（単位） | [units.md](units.md) | 解説。実行 → 手法 → 検証 → fold → セル |
| 何を数え、どう判定するか（規約） | [rules.md 11 章・13-9・14 章](rules.md) | ⚠ **食い違ったら rules.md が勝つ** |
| 実装（識別項目・数える線引き・畳み込み） | `ail/catalog.py`（`KEY`・`is_trial`・`judge`・`_collapse`） | 同上 |
| いまの中身（行・判定・未実施） | [ledger.md](ledger.md) | ⚠ **生成物。手で書き換えない** |

⚠ **本書の数字は 2026-09-12 生成の検証結果一覧の【実測】。** ⚠ **再生成すれば動く**（実行を足すたびに行も検証数も増える）。

## 1. なぜ要るか — 「試した数」を覚えていられないから

一言でいえば、**検証結果一覧は「何をいくつ試したか」を識別項目で数え直せる形に残す装置**である。

⚠ **良い数字は手法を並べれば必ず出る**（[rules.md 11 章](rules.md) 規約 2）。だから採否は「良い数字が出たか」ではなく
「⚠ **何回試したうちの良い数字か**」で決まる。その分母が `n_trials` で、⚠ **数え落とすと必ず甘くなる**（規約 4）。

> この図の主張: 分母を人の記憶に置くと必ず小さくなる。検証結果一覧は分母を**記録から数え直せる**ようにするためにある。

```mermaid
flowchart LR
  M["手法を並べて回す"] --> Q{"試した数を<br/>どこに置くか"}
  Q -->|"記憶・報告した分だけ"| B["⚠ 分母が小さくなる<br/>DSR が甘くなる"]
  Q -->|"検証結果一覧（識別項目で数え直す）"| G["分母が実測になる<br/>DSR の基準が正しく上がる"]
  B --> X["⚠ 偶然の当たりを<br/>発見と読む"]
  G --> Y["偶然の当たりを<br/>落とせる"]
```

⚠ **検証結果一覧は「成果の一覧」ではなく「試した回数の証拠」である。** 落とした行こそ本体で、
⚠ **落とした行を消すと分母が減る ＝ 残った行が良く見える。**

## 2. 4 つの役割

> この図の主張: 検証結果一覧は 1 つの表から 4 つの別々の仕事をしている。どれも「実行の記録」からしか作れない。

```mermaid
flowchart LR
  L["検証結果一覧<br/>ledger.md"] --> A["1 分母<br/>n_trials → DSR の基準"]
  L --> B["2 判定の置き場<br/>採る / 保留 / 落とす"]
  L --> C["3 同じ識別項目の突き合わせ<br/>再現の幅 → 配線を疑う"]
  L --> D["4 次の一手<br/>未実施と手間"]
```

| # | 役割 | 中身 | どこが使うか |
| ---: | --- | --- | --- |
| 1 | **多重検定の分母** | 検証の行数が `n_trials`。⚠ **2026-09-12 生成で 139**（基準線 106 行は数えない） | `checks.n_trials_now()` が検証結果一覧に数えさせ、各実行の `checks.json` の DSR に入る（[dsr.md §1](dsr.md)） |
| 2 | **判定の置き場** | 行ごとに 採る / 保留 / 落とす（＋ 基準・門前）と理由。⚠ **2026-09-12 生成で採る 0・落とす 102・保留 37** | 人の記憶ではなく記録に置く。⚠ **判定は `catalog.judge` が数字から出す**（手で書かない） |
| 3 | **同じ識別項目の突き合わせ** | 同じ識別項目の実行を 1 行にまとめ、食い違いを「再現」列に出す | ⚠ **幅が出たら実力ではなく配線を疑う**（[rules.md 11 章](rules.md) 規約 7） |
...
検証結果一覧は 3 つの正本を突き合わせた**生成物**で、⚠ **人が書ける列は 1 つしかない**（4 つ目の入力）。
...
⚠ **数字を手で書ける欄が無い**ので、⚠ **後から都合よく動かせない。** これが検証結果一覧の一番の仕事かもしれない。
判定も理由の一部も `ail/catalog.py` が数字から出す。⚠ **気に入らない判定を書き換えるには、実行を足すしかない。**

> この図の主張: 検証結果一覧は行き止まりではなく輪になっている。⚠ **いま回す実行の DSR の基準を、これまでの検証結果一覧が決めている。**

```mermaid
flowchart LR
  R["実行を回す<br/>cli/run.py"] --> S["summary.csv を書く"]
  S --> L["検証結果一覧が数え直す<br/>catalog.trials / is_trial"]
  L --> N["n_trials"]
  N --> C["その実行の checks.json<br/>DSR の基準 SR0"]
  C -.->|"次の実行はさらに厳しい基準"| R
```

⚠ **順序に依存がある**: `summary.csv` を書いた**あと**に数えるので、検証結果一覧は既にその実行の行を含んでいる。
⚠ **そこへ「この実行ぶん」を足すと二重に数える**（2026-09-12 に直した。§5）。

## 4. 数えるもの・数えないもの

⚠ **検証結果一覧に居ることと、分母に数えることは別である。** 線引きの正本は `catalog.is_trial`。

| 行の種類 | 検証結果一覧に載るか | ⚠ n_trials に数えるか | ⚠ 理由 |
| --- | :-: | :-: | --- |
...
| ⚠ **門前**（前置きの門で回さなかった手法） | ✅ | ❌ | ⚠ **評価期間の結果で選んでいない**から（[rules.md 14-5](rules.md)）。⚠ **後から回したら数える** |
| ⚠ **leak 対照**（わざと未来を混ぜた実行） | ⚠ **別表（§5）** | ❌ | ⚠ **的中率 99% の行が本体に混ざると検証結果一覧全体が嘘になる** |

⚠ **未実施（16 件）は「効かない」ではない**（[ledger.md §3](ledger.md)）。⚠ **落とした 102 行と混同しない。**

## 5. 壊れるとどうなるか — 実例 3 つ

⚠ **どれも「行の中身」ではなく「数え方」の事故である。** 検証結果一覧の役割が分母である以上、事故はいつも分母に出る。

| いつ | 何が起きたか | ⚠ 向き |
| --- | --- | --- |
| 2026-09-08 | 表と registry で手法の表記が違い（「F1-5 単変量検定 ＋ FDR」と「F1-5 検定+FDR」）、⚠ **名前で照合すると同じ検証が 2 行に割れた**。以後 ID で照合する（`catalog.canonical`） | 分母が増える方向（厳しい側）だが、⚠ **再現の幅も嘘になる** |
| 2026-09-12 | `checks.json` の `n_trials` が自分の実行ぶんを二重に数えていた（記録 97・正しくは 94） | ⚠ **厳しい側**（DSR が 0.004〜0.013 低く出ていた）。採否も画面の印も動かない |
| 2026-09-12 | 識別項目に「期間」が無く、⚠ **39 行が開始日の違う表の実行を 1 行にまとめていた**。遡って埋めて 94 → 139 検証 | ⚠ **甘い側だった**（分母が 45 少なかった）。⚠ **数字は 1 つも再計算していない** — 代表 1 本に隠れていた実行が表に出ただけ |
...
| ⚠ 答えないこと | 代わりにどこを見るか |
| --- | --- |
| ⚠ **儲かるか** | ⚠ **どこにも無い。** 検証結果一覧は「この物差しで基準線を超えたか」しか言わない（[ledger.md](ledger.md) 冒頭: 投資助言ではなく調査資料） |
| ⚠ **効かないのか、標本が足りないのか** | 分けられない（[ledger.md §6](ledger.md) の限界 2）。大きさの目安は [validation-power.md §3-2](validation-power.md) の検出限界 |
| 良い数字を信じてよいか | [dsr.md](dsr.md)。⚠ **検証結果一覧の判定式に DSR は入っていない**（判定は上乗せの符号。DSR は報告してよいかの関門） |
