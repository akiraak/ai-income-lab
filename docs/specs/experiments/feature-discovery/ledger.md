...
生成日 2026-09-08 ／ 入口: [feature-discovery.md](../feature-discovery.md) ／ 規約: [rules.md](rules.md) ／ 手法のカタログ: [feature-discovery.md §2](../feature-discovery.md#2-特徴量を見つけ出す手法のカタログphase-25-系統-25-件)
...
⚠ **試行は 36 行（手法）＋ 14 行（基準線）。** ⚠ **「採る」は 0 件。** 落とす 24 行 ／ 保留 12 行。

⚠ **保留の内訳**: 無効・要再測（日足 × `raw`）が 9 行 ／ ⚠ **純利は正だが fold の符号が割れる**ものが 3 行。⚠ **後者は「採る」の一歩手前ではない。** ⚠ **多重検定を通していない良い数字である**（[rules.md 11 章](rules.md) 規約 2・3）。
...
⚠ **多重検定に使う `n_trials` は 36**（= 手法の行数。基準線 14 行は数えない）。⚠ **[rules.md 11 章](rules.md) の規約 4 は「手法 × 粒度 × 地平 × 銘柄集合」を全部数えることを求める。この数を数え落とすと必ず甘くなる。**
...
⚠ **先読みの列を拾わなかった手法がある**: **F3-5 クラスタ化 MDA（cMDA）**（的中率 0.5030）。⚠ **単独で圧倒的に効く 1 本を落とす設計だということである**（[§3-2](../feature-discovery.md)）。
...
| 3 | ⚠ **「未実施」は「効かない」ではない** | ⚠ **16 件は情報が無いだけである。**⚠ **落とした 24 行と混同しない** |
