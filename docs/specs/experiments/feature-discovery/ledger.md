...
生成日 2026-09-21 ／ 入口: [feature-discovery.md](../feature-discovery.md) ／ 規約: [rules.md](rules.md) ／ 手法のカタログ: [feature-discovery.md §2](../feature-discovery.md#2-特徴量を見つけ出す手法のカタログphase-25-系統-25-件)
...
> この図の主張: ⚠ **検証結果一覧は 3 つの正本を突き合わせた生成物である。** 手で書き写す欄はひとつも無い。
...
⚠ **検証は 667 行（手法）＋ 488 行（基準線）。** ⚠ **「採る」は 0 件。** 落とす 551 行 ／ 保留 116 行。

⚠ **保留の内訳**: 無効・要再測（日足 × `raw`）が 27 行 ／ ⚠ **純利は正だが fold の符号が割れる**ものが 89 行。⚠ **後者は「採る」の一歩手前ではない。** ⚠ **多重検定を通していない良い数字である**（[rules.md 11 章](rules.md) 規約 2・3）。
...
⚠ **多重検定に使う `n_trials` は 667**（= 検証の行数。基準線 488 行は数えない。⚠ **「全部使う × Ridge 以外のモデル」はモデルが処置なので検証として数える** — [plans/archive/gpu-models.md §3-4](../../../plans/archive/gpu-models.md)）。⚠ **[rules.md 11 章](rules.md) の規約 4 は「手法 × 粒度 × 地平 × 銘柄集合」を全部数えることを求める。この数を数え落とすと必ず甘くなる。**

## 1. 検証結果一覧の読み方

単位（実行 → 手法 → 検証 → fold → セル）の関係は [units.md](units.md) に図解がある（初見はそちらを先に読む）。

⚠ **1 行 = 1 回の検証。** 識別項目は **手法 × モデル × 粒度 × 地平 × 特徴量の層 × データの層 × 期間 × 検証方式 × 形式 × 較正 × 閾値** である（[利用者の指示](../../../plans/archive/feature-discovery-ledger.md)。⚠ **モデルは 2026-09-09 に識別項目へ足した** — それまでは Ridge 1 本。⚠ **検証方式・形式・閾値は 2026-09-10 に足した** — 旧実行は（毎日往復・共通・—）として読むので既存の行は割れない。[rules.md 13-9](rules.md)。⚠ **期間は 2026-09-11 に足し、2026-09-12 に記録の無い実行へも遡って当てた**（利用者の決定。[rules.md 14-4](rules.md)） — ⚠ **39 行が割れて 94 → 139 検証**になった。⚠ **数字は 1 つも再計算していない**: 代表 1 本に隠れていた実行が表に出ただけである。⚠ **較正は 2026-09-12 に足した** — Platt の数値解が止まっていた不具合の処置で、⚠ **既存の実行は記録を持たないので全部「旧」に寄り、行はどれも割れない**（[rules.md 13-2](rules.md) の 6））。
...
| 期間 | 使った表の開始日（その実行が読んだ表の最古の日） | ⚠ **開始日が違えば別の検証**（同じ識別項目にまとめると期間差が「再現の幅」に化ける）。実行が記録していなければ表の実物から遡って読む。⚠ **「—」は遡れなかったという意味** — 表が消えている・行数が実行の記録と合わない（＝ 作り直された表）・行数の記録が無い（[rules.md 14-4](rules.md)） |
...
| 較正 | Platt をどう解いたか。⚠ **旧**（〜2026-09-12。⚠ **数値解が 1 反復で止まっていた**）／ **std**（標準化して解く）／ **—**（較正を通らない＝ 毎日往復） | ⚠ **旧の行の買い% はほぼ定数**（幅 0.00002 点）で、θ を跨がず「ずっと持つ / ずっと休む」に潰れている（[buy-pct-width-collapse.md](../buy-pct-width-collapse.md)）。⚠ **旧行は再計算しない・消さない。回し直した分は別の検証として数える** |
| 閾値 | θ（%）。買い% > θ で建て、売り% > θ で手仕舞う | ⚠ **1 水準 = 1 検証**（13-9）。3 水準とも載せる（良かった閾値だけ報告しない） |
| 再現 | 同じ識別項目の実行が何本あり、一致したか | ⚠ **数字が動いたら、まず配線を疑う**（規約 7） |
...
⚠ **失敗の型は E8 の検証結果一覧と同じもの**（[E8 §6](../e8-signal-methods/failures.md)）。**X2** コストで消える ／ **X9** 標本不足。
⚠ **1 分足の `raw` は保留にしない。** 目盛りの誤りは日足にしか無い（[§3 の注記](../feature-discovery.md)）。

## 2. 検証結果一覧（試した結果）

⚠ **1155 行。** うち手法 667 行・基準線 488 行。

| ID | 手法 | 系統 | 実装 | モデル | 層 | 期間 | 粒度 | 地平 | 特徴量の層 | 検証方式 | 形式 | 較正 | 閾値 | 本数 | 的中率 | IC | 粗利bp | 純利bp | fold | 再現 | 判定 | 理由 | 実行 |
| --- | --- | --- | :-: | :-: | :-: | :-: | --- | --- | :-: | :-: | :-: | :-: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | :-: | --- | --- |
| — | 全部使う（基準）〔上位1・端数〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋7071.10 | **＋7051.10** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+5400.48bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | T3 QUANT（60日窓）〔上位1・整数株A〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋6974.94 | **＋6946.61** | 2/5 ＋−−＋− | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+5294.60bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 全部使う（基準）〔上位1・整数株B⚠〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋6908.53 | **＋6889.01** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+5238.40bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位3・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋6458.28 | **＋6440.81** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位3・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋6243.99 | **＋6187.99** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 全部使う（基準）〔上位1・整数株A〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋6095.39 | **＋6079.95** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+4429.33bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位3・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋6122.65 | **＋6070.11** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | T3 QUANT（60日窓）〔上位3・端数〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋5403.01 | **＋5367.34** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+3715.33bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位3・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋5318.68 | **＋5298.35** | 3/5 ＋＋−−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | T3 QUANT（60日窓）〔上位3・整数株B⚠〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋5307.16 | **＋5274.78** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+3622.76bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | T3 QUANT（60日窓）〔上位3・整数株A〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋5267.69 | **＋5246.87** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+3594.85bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位3・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋5254.06 | **＋5235.22** | 3/5 ＋＋−−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | ボラ上位〔T3 QUANT（60日窓）・上位5・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋5214.61 | **＋5159.81** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位1・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋5154.38 | **＋4978.38** | 5/5 ＋＋＋＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位5・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋4990.28 | **＋4941.63** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位5・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋4920.87 | **＋4900.67** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位1・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋5009.84 | **＋4839.41** | 5/5 ＋＋＋＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | ボラ上位〔T3 QUANT（60日窓）・上位3・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋4765.59 | **＋4722.43** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位5・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋4683.26 | **＋4665.47** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | T3 QUANT（60日窓）〔上位1・整数株B⚠〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋4658.54 | **＋4619.22** | 2/5 ＋−−＋− | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+2967.21bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | T3 QUANT（60日窓）〔上位1・端数〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋4561.70 | **＋4520.70** | 2/5 ＋−−＋− | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+2868.69bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位3・整数株A〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋4475.43 | **＋4458.51** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+2807.90bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位3・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋4151.51 | **＋4038.23** | 5/5 ＋＋＋＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | T3 QUANT（60日窓）〔上位5・端数〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋4070.12 | **＋4037.72** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+2385.70bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 全部使う（基準）〔上位3・端数〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋4053.01 | **＋4032.68** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+2382.07bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | T3 QUANT（60日窓）〔上位5・整数株B⚠〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋3952.84 | **＋3925.12** | 3/5 ＋＋−−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+2273.10bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位5・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋4077.27 | **＋3920.27** | 4/5 ＋＋＋−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位3・整数株B⚠〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋3834.20 | **＋3815.51** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+2164.90bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位5・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋3819.35 | **＋3681.47** | 4/5 ＋＋＋−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 全部使う（基準）〔上位5・端数〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋3640.25 | **＋3620.05** | 2/5 ＋−−−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+1969.44bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位3・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋3768.49 | **＋3601.82** | 4/5 ＋＋＋−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位1・端数〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋3684.99 | **＋3550.99** | 4/5 ＋＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+1898.97bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 全部使う（基準）〔上位1・整数株B⚠〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋3635.07 | **＋3505.19** | 4/5 ＋＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+1853.18bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位3・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋3613.30 | **＋3459.95** | 4/5 ＋＋＋−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位5・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋3488.41 | **＋3454.17** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | ボラ上位〔T3 QUANT（60日窓）・上位10・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋3420.92 | **＋3370.62** | 3/5 ＋−−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位1・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋3486.40 | **＋3342.79** | 4/5 ＋＋＋＋− | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位10・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋3298.10 | **＋3278.00** | 3/5 ＋−−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位1・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋3294.49 | **＋3273.49** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位1・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋3245.15 | **＋3224.61** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | T3 QUANT（60日窓）〔上位5・整数株A〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋3214.84 | **＋3195.41** | 4/5 ＋＋−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+1543.39bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 全部使う（基準）〔上位5・整数株B⚠〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋3201.54 | **＋3184.45** | 2/5 ＋−−−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+1533.83bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位10・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋3083.75 | **＋3068.00** | 3/5 ＋−−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | ボラ上位〔T3 QUANT（60日窓）・上位1・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋3059.97 | **＋2996.97** | 3/5 ＋＋−−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位1・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋2991.35 | **＋2930.52** | 3/5 ＋＋−−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位3・整数株A〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋2993.96 | **＋2913.45** | 3/5 −＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+1261.43bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 全部使う（基準）〔上位1・整数株A〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋3016.24 | **＋2907.45** | 4/5 ＋＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+1255.43bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位10・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋2941.22 | **＋2899.77** | 4/5 ＋＋−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | T3 QUANT（60日窓）〔上位10・整数株B⚠〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋2908.85 | **＋2885.67** | 3/5 ＋＋−−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+1233.66bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位3・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋2851.42 | **＋2835.19** | 2/5 −−−＋＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位5・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋2812.11 | **＋2724.19** | 4/5 ＋＋＋−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 全部使う（基準）〔上位3・端数〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋2814.81 | **＋2711.81** | 3/5 −＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+1059.80bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位5・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋2714.03 | **＋2698.57** | 3/5 ＋−−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 全部使う（基準）〔上位5・端数〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋2745.09 | **＋2646.69** | 3/5 −＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+994.68bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位3・整数株B⚠〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋2685.89 | **＋2592.70** | 3/5 −＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+940.68bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | T3 QUANT（60日窓）〔上位10・端数〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋2620.68 | **＋2590.48** | 3/5 ＋＋−−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+938.47bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位10・端数〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋2595.36 | **＋2575.26** | 3/5 ＋−−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+924.64bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 全部使う（基準）〔上位10・整数株B⚠〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋2570.09 | **＋2554.31** | 3/5 ＋−−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+903.70bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位10・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋2666.24 | **＋2527.44** | 3/5 ＋−＋−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位1・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋2580.11 | **＋2527.37** | 3/5 ＋＋−−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位20・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋2397.46 | **＋2377.41** | 3/5 ＋−−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | ボラ上位〔T3 QUANT（60日窓）・上位20・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋2351.09 | **＋2307.24** | 3/5 ＋＋−−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 全部使う（基準）〔上位5・整数株B⚠〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋2386.64 | **＋2299.45** | 3/5 −＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+647.43bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | T3 QUANT（60日窓）〔上位20・端数〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋2266.16 | **＋2237.91** | 2/5 ＋＋−−− | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+585.89bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位10・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋2318.15 | **＋2211.44** | 4/5 ＋＋＋−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | ボラ上位〔T3 QUANT（60日窓）・上位20・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋2201.82 | **＋2172.22** | 3/5 ＋＋−−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | ボラ上位〔全部使う（基準）・上位20・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋2151.66 | **＋2137.04** | 3/5 ＋−−＋＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 全部使う（基準）〔上位20・端数〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋2101.63 | **＋2081.58** | 2/5 −−−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+430.97bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位20・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋2193.70 | **＋2071.10** | 3/5 ＋＋−−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 全部使う（基準）〔上位5・整数株A〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋2083.48 | **＋2067.60** | 3/5 ＋−−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+416.98bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位1・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋2072.83 | **＋2056.03** | 3/5 ＋＋−−＋ | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位5・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋2049.15 | **＋2029.03** | 2/5 −−−＋＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | T3 QUANT（60日窓）〔上位20・整数株B⚠〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋2026.16 | **＋2005.83** | 3/5 ＋＋−−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+353.81bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 全部使う（基準）〔上位10・端数〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋2076.56 | **＋1986.26** | 3/5 −＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+334.25bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位10・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1967.14 | **＋1947.04** | 1/5 −−−−＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位20・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1923.39 | **＋1903.34** | 3/5 ＋−−＋＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 全部使う（基準）〔上位10・整数株B⚠〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋1964.87 | **＋1888.96** | 4/5 ＋＋＋−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+236.94bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 全部使う（基準）〔上位20・端数〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋1970.90 | **＋1883.35** | 3/5 ＋＋−−＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+231.33bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位5・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1845.07 | **＋1829.29** | 3/5 ＋−−＋＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位10・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1817.14 | **＋1801.28** | 1/5 −−−−＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位3・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1812.21 | **＋1798.24** | 2/5 −＋−＋− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位20・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1798.39 | **＋1783.75** | 2/5 −−−＋＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位5・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1796.16 | **＋1780.37** | 3/5 ＋＋−＋− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位5・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1775.57 | **＋1758.16** | 1/5 −−−−＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 全部使う（基準）〔上位20・整数株B⚠〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋1746.14 | **＋1731.47** | 2/5 −−−＋＋ | 2 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+80.86bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位1・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1742.29 | **＋1729.38** | 2/5 −＋−＋− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位20・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1807.33 | **＋1723.83** | 2/5 ＋＋−−− | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | T3 QUANT（60日窓） | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋1743.57 | **＋1713.93** | 2/5 ＋＋−−0 | 4 実行・一致 | **保留** | ⚠ 上乗せの平均だけ正（+61.92bp）。fold の符号が割れる（rules.md 13-7） ／ ⚠ **閉じる（2026-09-16・中身を確かめた）**: 5 fold のうち 4 fold が「全部持つ」か「全部休む」に倒れ、⚠ **平均を正にしたのは fold 2 の ＋589.93bp の 1 本だけ**（[tsc-threshold.md §2](../tsc-threshold.md)）【実測】 | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 全部使う（基準） | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋1718.85 | **＋1699.33** | 2/5 0−−＋＋ | ⚠ **5 実行・幅 123.40bp** | **保留** | ⚠ 上乗せの平均だけ正（+48.72bp）。fold の符号が割れる（rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 乱択上位〔T3 QUANT（60日窓）・上位20・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1685.95 | **＋1664.03** | 1/5 −＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 常に上（ドリフト） | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5216 | 0.0000 | ＋1657.02 | **＋1652.02** | 0/5 00000 | 19 実行・一致 | 基準 | ⚠ 新方式では期初買い・期末売り（B&H）。上乗せの基準 | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 常に上（ドリフト） | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5216 | 0.0000 | ＋1657.02 | **＋1652.02** | 0/5 00000 | 8 実行・一致 | 基準 | ⚠ 新方式では期初買い・期末売り（B&H）。上乗せの基準 | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位3・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1671.66 | **＋1651.53** | 2/5 −−−＋＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 常に上（ドリフト） | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5174 | 0.0000 | ＋1656.01 | **＋1650.62** | 0/5 00000 | 21 実行・一致 | 基準 | ⚠ 新方式では期初買い・期末売り（B&H）。上乗せの基準 | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 乱択上位〔T3 QUANT（60日窓）・上位10・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1650.49 | **＋1631.23** | 3/5 ＋＋−＋− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位20・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1641.11 | **＋1623.37** | 1/5 −＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位20・整数株B⚠〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋1695.23 | **＋1621.42** | 2/5 ＋＋−−− | 2 実行・一致 | **落とす** | ⚠ **基準線以下 ＝ 何も学んでいない**（対 B&H 上乗せ -30.60bp ≤ 0。rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔T3 QUANT（60日窓）・上位10・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1597.45 | **＋1581.14** | 2/5 ＋＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位5・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1593.87 | **＋1574.47** | 2/5 −＋−＋− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位3・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1507.80 | **＋1489.89** | 1/5 −−−−＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位1・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1505.17 | **＋1487.57** | 2/5 ＋＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 乱択上位〔T3 QUANT（60日窓）・上位1・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1491.89 | **＋1474.87** | 2/5 ＋＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位5・整数株A〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋1514.52 | **＋1445.05** | 2/5 −＋＋−− | 2 実行・一致 | **落とす** | ⚠ **基準線以下 ＝ 何も学んでいない**（対 B&H 上乗せ -206.97bp ≤ 0。rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位1・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1535.30 | **＋1431.49** | 2/5 −＋＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔T3 QUANT（60日窓）・上位3・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1443.40 | **＋1424.26** | 2/5 ＋＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位1・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1530.50 | **＋1423.30** | 2/5 −＋＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 全部使う（基準） | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋1467.52 | **＋1381.12** | 2/5 −＋−−＋ | 5 実行・一致 | **落とす** | ⚠ **基準線以下 ＝ 何も学んでいない**（対 B&H 上乗せ -270.89bp ≤ 0。rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位5・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1467.93 | **＋1379.45** | 2/5 −＋＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔T3 QUANT（60日窓）・上位5・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1386.26 | **＋1370.40** | 1/5 −＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 乱択上位〔T3 QUANT（60日窓）・上位3・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1371.75 | **＋1354.50** | 2/5 ＋＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位10・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1407.59 | **＋1321.15** | 2/5 −＋＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | ボラ上位〔T3 QUANT（60日窓）・上位10・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1295.92 | **＋1285.41** | 2/5 ＋＋−−− | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | T3 QUANT（60日窓）〔上位10・整数株A〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋1289.58 | **＋1279.06** | 2/5 ＋＋−−− | 2 実行・一致 | **落とす** | ⚠ **基準線以下 ＝ 何も学んでいない**（対 B&H 上乗せ -372.96bp ≤ 0。rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位10・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋1288.03 | **＋1277.52** | 2/5 ＋＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位20・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1344.11 | **＋1258.09** | 1/5 −＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位10・整数株A〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋1221.40 | **＋1216.94** | 1/5 ＋−−−− | 2 実行・一致 | **落とす** | ⚠ **基準線以下 ＝ 何も学んでいない**（対 B&H 上乗せ -433.68bp ≤ 0。rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位10・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1221.40 | **＋1216.94** | 1/5 ＋−−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位10・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1221.40 | **＋1216.94** | 1/5 ＋−−−− | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位5・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1292.87 | **＋1215.82** | 2/5 −＋＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位20・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1278.31 | **＋1204.85** | 1/5 −＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位3・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1281.56 | **＋1204.39** | 2/5 −＋＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位10・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1217.54 | **＋1144.76** | 2/5 −＋＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位1・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1176.55 | **＋1085.78** | 2/5 −＋＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位3・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1156.46 | **＋1071.32** | 1/5 −−＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位3・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1161.80 | **＋1067.27** | 1/5 −−＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位1・端数〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1079.91 | **＋1059.71** | 1/5 −−−−＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位1・整数株B⚠〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋1042.55 | **＋1023.07** | 1/5 −−−−＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位5・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋1052.18 | **＋984.38** | 2/5 −＋＋−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 乱択上位〔全部使う（基準）・上位1・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋896.24 | **＋880.42** | 1/5 −−−−＋ | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 全部使う（基準）〔上位10・整数株A〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋705.05 | **＋679.60** | 1/5 −＋−−− | 2 実行・一致 | **落とす** | ⚠ **基準線以下 ＝ 何も学んでいない**（対 B&H 上乗せ -972.41bp ≤ 0。rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位10・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋705.05 | **＋679.60** | 1/5 −＋−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位10・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋705.05 | **＋679.60** | 1/5 −＋−−− | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | T3 QUANT（60日窓）〔上位20・整数株A〕 | 検知器 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 60.0 | 0.5219 | ＋0.0070 | ＋458.27 | **＋457.35** | 0/5 −−−−− | 2 実行・一致 | **落とす** | ⚠ **基準線以下 ＝ 何も学んでいない**（対 B&H 上乗せ -1194.67bp ≤ 0。rules.md 13-7） | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位20・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋458.27 | **＋457.35** | 0/5 −−−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位20・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5219 | ＋0.0070 | ＋458.27 | **＋457.35** | 0/5 −−−−− | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| — | 全部使う（基準）〔上位20・整数株A〕 | 基準線 | ✅ | LightGBM | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 124.0 | 0.5157 | ＋0.0182 | ＋349.54 | **＋349.45** | 0/5 −−−−− | 2 実行・一致 | **落とす** | ⚠ **基準線以下 ＝ 何も学んでいない**（対 B&H 上乗せ -1301.16bp ≤ 0。rules.md 13-7） | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位20・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋349.54 | **＋349.45** | 0/5 −−−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位20・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.5157 | ＋0.0182 | ＋349.54 | **＋349.45** | 0/5 −−−−− | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 全部使う（基準）〔上位20・整数株A〕 | 基準線 | ✅ | Ridge | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 35.0 | 0.5184 | −0.0021 | ＋245.82 | **＋245.00** | 0/5 −−−−− | 2 実行・一致 | **落とす** | ⚠ **基準線以下 ＝ 何も学んでいない**（対 B&H 上乗せ -1407.02bp ≤ 0。rules.md 13-7） | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | 乱択上位〔全部使う（基準）・上位20・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋245.82 | **＋245.00** | 0/5 −−−−− | 2 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
| — | ボラ上位〔全部使う（基準）・上位20・整数株A〕 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.5184 | −0.0021 | ＋245.82 | **＋245.00** | 0/5 −−−−− | — | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 直前リターンの符号 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own cs rel ex | 閾値売買 | 共通 | std | 55 | 0.0 | 0.4978 | −0.0104 | ＋336.15 | **−117.23** | 1/5 −−＋−− | 21 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` |
...
| — | 直前リターンの符号 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own | 閾値売買 | 共通 | std | 50 | 0.0 | 0.4974 | −0.0127 | ＋287.86 | **−165.74** | 1/5 −−＋−− | 19 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-18-05_trade_own_ridge_a_topk` |
...
| — | 直前リターンの符号 | 基準線 | ✅ | — | adjusted | 2018-01-31 | 日足 | 1 本（1 日） | own seq | 閾値売買 | 共通 | std | 50 | 0.0 | 0.4974 | −0.0127 | ＋287.86 | **−165.74** | 1/5 −−＋−− | 8 実行・一致 | 基準 | ⚠ 採否の対象ではない | `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` |
...
| `2026-09-19T12-33-14_trade_own_ridge_a_topk` | runs/ | 日足 | Ridge | adjusted | 135,962 | 35 | 63 | 0 | `5cb5320` | `81ade06ba58c8e42` | — |
| `2026-09-19T12-33-27_trade_own_ridge_a_topk_leak` | runs/ | 日足 | Ridge | adjusted | 135,962 | 36 | 63 | 0 | `5cb5320` | `81ade06ba58c8e42` | ⚠ **あり** |
| `2026-09-19T12-33-41_trade_ownex_lgbm_a_topk` | runs/ | 日足 | LightGBM | adjusted | 101,157 | 124 | 48 | 0 | `5cb5320` | `81ade06ba58c8e42` | — |
| `2026-09-19T12-33-56_trade_ownex_lgbm_a_topk_leak` | runs/ | 日足 | LightGBM | adjusted | 101,157 | 125 | 48 | 0 | `5cb5320` | `81ade06ba58c8e42` | ⚠ **あり** |
| `2026-09-19T12-34-23_trade_ownseq_ridge_a_topk` | runs/ | 日足 | Ridge | adjusted | 135,962 | 95 | 63 | 0 | `5cb5320` | `81ade06ba58c8e42` | — |
| `2026-09-19T12-34-49_trade_ownseq_ridge_a_topk_leak` | runs/ | 日足 | Ridge | adjusted | 135,962 | 96 | 63 | 0 | `5cb5320` | `81ade06ba58c8e42` | ⚠ **あり** |
| `2026-09-19T13-18-05_trade_own_ridge_a_topk` | runs/ | 日足 | Ridge | adjusted | 135,962 | 35 | 63 | 0 | `5cb5320` | `81ade06ba58c8e42` | — |
| `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` | runs/ | 日足 | Ridge | adjusted | 135,962 | 36 | 63 | 0 | `5cb5320` | `81ade06ba58c8e42` | ⚠ **あり** |
| `2026-09-19T13-18-35_trade_ownex_lgbm_a_topk` | runs/ | 日足 | LightGBM | adjusted | 101,157 | 124 | 48 | 0 | `5cb5320` | `81ade06ba58c8e42` | — |
| `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` | runs/ | 日足 | LightGBM | adjusted | 101,157 | 125 | 48 | 0 | `5cb5320` | `81ade06ba58c8e42` | ⚠ **あり** |
| `2026-09-19T13-19-20_trade_ownseq_ridge_a_topk` | runs/ | 日足 | Ridge | adjusted | 135,962 | 95 | 63 | 0 | `5cb5320` | `81ade06ba58c8e42` | — |
| `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` | runs/ | 日足 | Ridge | adjusted | 135,962 | 96 | 63 | 0 | `5cb5320` | `81ade06ba58c8e42` | ⚠ **あり** |
| `legacy-min1-h390` | ⚠ 旧配線 §3-3 | 1 分足 | — | raw | 44,206 | 35 | 63 | 0 | — | — | — |
| `legacy-daily-h1` | ⚠ 旧配線 §4-3 | 日足 | — | raw | 61,680 | 35 | 63 | 0 | — | — | — |

⚠ **`runs/` は git 管理外である。** ⚠ **この検証結果一覧を同じ内容で再生成できるのは、その実行を持っている手元だけ**である（clone しただけの環境では旧配線の行しか出ない）。

## 5. ⚠ 配線の検査（検証結果一覧の対象外）

⚠ **わざと未来の値を混ぜた実行は、検証結果一覧の本体に入れない。** ⚠ **的中率 99% の行が混ざると、検証結果一覧全体が嘘になる。**
...
| — | ボラ上位〔全部使う（基準）・上位1・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋62146.59 | ＋61545.59 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位1・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋62146.59 | ＋61545.59 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位1・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋60672.25 | ＋60095.25 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位1・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋60538.33 | ＋59953.80 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位1・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋60538.33 | ＋59953.80 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位1・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋59109.97 | ＋58548.67 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位3・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋54903.11 | ＋54307.78 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位3・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋54903.11 | ＋54307.78 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位3・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋54446.09 | ＋53860.75 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位1・端数〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋51603.56 | ＋51020.56 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位3・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋50970.27 | ＋50420.11 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位3・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋50970.27 | ＋50420.11 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位3・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋50377.61 | ＋49839.45 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位1・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋50283.94 | ＋49788.45 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位1・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋50283.94 | ＋49788.45 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位5・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋50157.13 | ＋49569.53 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位5・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋50157.13 | ＋49569.53 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位1・整数株B⚠〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋50091.33 | ＋49526.30 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位5・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋49479.89 | ＋48900.49 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位1・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋49358.17 | ＋48886.01 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位3・端数〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋49402.20 | ＋48820.53 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位5・端数〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋48813.17 | ＋48233.77 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位10・端数〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋45349.67 | ＋44772.07 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位3・整数株B⚠〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋44901.48 | ＋44374.55 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位5・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋44606.60 | ＋44086.00 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位5・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋44606.60 | ＋44086.00 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位5・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋43855.52 | ＋43344.89 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位10・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋43669.01 | ＋43079.51 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位10・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋43669.00 | ＋43079.40 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位10・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋42829.93 | ＋42252.13 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位5・整数株B⚠〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋41841.32 | ＋41344.79 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位1・整数株A〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋41420.49 | ＋40948.01 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位20・端数〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋39581.76 | ＋39011.31 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位20・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋37374.77 | ＋36794.97 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位20・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋37374.03 | ＋36794.28 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位3・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋37201.90 | ＋36718.48 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位3・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋37201.90 | ＋36718.48 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位20・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋36908.59 | ＋36341.04 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位3・整数株A〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋36399.19 | ＋35936.11 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位3・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋36206.76 | ＋35740.52 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位10・整数株B⚠〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋35664.78 | ＋35201.70 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位5・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋35788.08 | ＋35200.88 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位5・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋35667.06 | ＋35080.26 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位10・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋35472.52 | ＋34891.02 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位10・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋35412.74 | ＋34830.74 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位10・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋34863.86 | ＋34384.59 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位10・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋34860.95 | ＋34381.79 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位3・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋34442.86 | ＋33863.53 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位3・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋34344.96 | ＋33764.96 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位10・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋34170.76 | ＋33701.11 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位1・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋34161.74 | ＋33570.74 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位1・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋34141.43 | ＋33551.43 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位20・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋33278.07 | ＋32702.77 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位20・端数〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋33232.60 | ＋32657.50 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位1・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋33204.66 | ＋32630.70 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位1・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋33184.25 | ＋32611.31 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位3・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋32310.33 | ＋31724.40 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位1・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋32232.75 | ＋31642.55 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位10・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋32075.58 | ＋31496.08 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位5・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋31812.07 | ＋31229.35 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位20・端数〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋31483.86 | ＋30917.21 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位3・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋31217.98 | ＋30692.47 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位1・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋31209.55 | ＋30638.68 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位3・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋31148.75 | ＋30622.38 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位5・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋30610.85 | ＋30114.95 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位5・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋30507.96 | ＋30012.11 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位3・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋29860.98 | ＋29274.45 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位3・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋29853.37 | ＋29265.77 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位10・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋29651.56 | ＋29068.34 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位10・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋29624.73 | ＋29041.71 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位5・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋29542.10 | ＋28957.70 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位3・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋29401.63 | ＋28941.45 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位20・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋29514.50 | ＋28939.17 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位5・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋29511.73 | ＋28927.93 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位20・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋29478.25 | ＋28902.97 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位3・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋29354.26 | ＋28895.20 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位3・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋29230.62 | ＋28701.47 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位1・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋29161.33 | ＋28576.13 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位1・端数〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋29131.60 | ＋28546.60 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位10・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋28630.99 | ＋28166.88 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位10・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋28577.93 | ＋28113.38 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位5・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋28291.54 | ＋27847.67 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位5・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋28291.54 | ＋27847.67 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位1・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋28306.22 | ＋27739.04 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位1・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋28274.96 | ＋27707.99 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位1・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋27975.64 | ＋27524.47 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位1・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋27952.70 | ＋27502.81 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位5・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋27609.10 | ＋27109.37 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準）〔上位5・整数株A〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋27229.67 | ＋26822.03 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位3・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋27343.10 | ＋26808.85 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位3・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋27314.98 | ＋26779.99 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位5・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋26964.81 | ＋26555.09 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
...
| — | 乱択上位〔全部使う（基準）・上位1・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋26505.18 | ＋26031.77 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位3・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋26307.86 | ＋25857.61 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
...
| — | 乱択上位〔全部使う（基準）・上位10・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋26229.32 | ＋25766.39 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
...
| — | 全部使う（基準） | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋25945.33 | ＋25494.03 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 全部使う（基準） | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋25944.76 | ＋25490.95 | `2026-09-17T17-07-01_trade_ownex_lgbm_a_leak` |
| — | 乱択上位〔全部使う（基準）・上位5・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋25983.37 | ＋25476.52 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
...
| — | 乱択上位〔T3 QUANT（60日窓）・上位5・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋25961.57 | ＋25454.23 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
...
| — | T3 QUANT（60日窓）〔上位5・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋25786.07 | ＋25343.35 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位5・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋25776.88 | ＋25333.91 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
...
| — | ボラ上位〔全部使う（基準）・上位20・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋25160.63 | ＋24718.33 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位20・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋25160.57 | ＋24718.30 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
...
| — | 乱択上位〔全部使う（基準）・上位1・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋24861.77 | ＋24374.58 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位1・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋24779.26 | ＋24291.16 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位10・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋24732.90 | ＋24255.12 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位10・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋24718.64 | ＋24240.38 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
...
| — | T3 QUANT（60日窓） | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋24299.38 | ＋23845.76 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準） | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋24299.37 | ＋23845.75 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
...
| — | T3 QUANT（60日窓）〔上位20・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋24155.33 | ＋23715.29 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
...
| — | 全部使う（基準）〔上位20・整数株B⚠〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋24128.98 | ＋23688.77 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
...
| — | 乱択上位〔全部使う（基準）・上位5・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋23553.22 | ＋23145.21 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位3・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋23436.64 | ＋22973.98 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
...
| — | 乱択上位〔T3 QUANT（60日窓）・上位3・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋23354.97 | ＋22891.98 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 全部使う（基準）〔上位20・整数株B⚠〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋22921.30 | ＋22552.37 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
...
| — | 乱択上位〔全部使う（基準）・上位20・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋22630.23 | ＋22191.51 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位20・整数株B⚠〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋22622.37 | ＋22183.55 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位20・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋22518.49 | ＋22147.99 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位5・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋22003.39 | ＋21568.01 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位5・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋21985.94 | ＋21550.65 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位20・整数株B⚠〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋21659.68 | ＋21290.25 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
...
| — | 全部使う（基準）〔上位10・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋9380.30 | ＋9249.92 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位10・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋9380.30 | ＋9249.92 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位10・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋9376.09 | ＋9245.82 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位10・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋9376.09 | ＋9245.82 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位10・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋9367.25 | ＋9236.96 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位10・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋9367.25 | ＋9236.96 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
...
| — | 全部使う（基準）〔上位10・整数株A〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋7267.49 | ＋7168.96 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位10・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋7267.49 | ＋7168.96 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位10・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋7267.49 | ＋7168.96 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
...
| — | 常に上（ドリフト） | — | adjusted | 日足 | 0.5216 | 0.0000 | ＋1657.02 | ＋1652.02 | `2026-09-10T20-06-42_trade_own_lgbm_b_leak` |
| — | 常に上（ドリフト） | — | adjusted | 日足 | 0.5216 | 0.0000 | ＋1657.02 | ＋1652.02 | `2026-09-10T20-06-42_trade_own_lgbm_b_leak` |
| — | 常に上（ドリフト） | — | adjusted | 日足 | 0.5216 | 0.0000 | ＋1657.02 | ＋1652.02 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
...
| — | 常に上（ドリフト） | — | adjusted | 日足 | 0.5216 | 0.0000 | ＋1657.02 | ＋1652.02 | `2026-09-14T10-08-36_trade_own_mlpgan16k_b_leak` |
| — | 常に上（ドリフト） | — | adjusted | 日足 | 0.5216 | 0.0000 | ＋1657.02 | ＋1652.02 | `2026-09-14T10-08-36_trade_own_mlpgan16k_b_leak` |
| — | 常に上（ドリフト） | — | adjusted | 日足 | 0.5216 | 0.0000 | ＋1657.02 | ＋1652.02 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
...
| — | 常に上（ドリフト） | — | adjusted | 日足 | 0.5174 | 0.0000 | ＋1656.01 | ＋1650.62 | `2026-09-18T03-47-08_trade_ownex_mlpgan_a_leak` |
| — | 常に上（ドリフト） | — | adjusted | 日足 | 0.5174 | 0.0000 | ＋1656.01 | ＋1650.62 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
...
| — | 全部使う（基準）〔上位20・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋768.28 | ＋760.27 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位20・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋768.28 | ＋760.27 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | T3 QUANT（60日窓）〔上位20・整数株A〕 | Ridge | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋768.28 | ＋760.27 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | 乱択上位〔T3 QUANT（60日窓）・上位20・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋768.28 | ＋760.27 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位20・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6640 | ＋768.28 | ＋760.27 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
| — | ボラ上位〔T3 QUANT（60日窓）・上位20・整数株A〕 | — | adjusted | 日足 | 0.9996 | ＋0.6641 | ＋768.28 | ＋760.27 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
...
| — | 全部使う（基準）〔上位20・整数株A〕 | LightGBM | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋631.75 | ＋625.31 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 乱択上位〔全部使う（基準）・上位20・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋631.75 | ＋625.31 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | ボラ上位〔全部使う（基準）・上位20・整数株A〕 | — | adjusted | 日足 | 0.9874 | ＋0.6789 | ＋631.75 | ＋625.31 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
...
| — | 直前リターンの符号 | — | adjusted | 日足 | 0.4978 | −0.0104 | ＋336.15 | −117.23 | `2026-09-19T13-18-51_trade_ownex_lgbm_a_topk_leak` |
| — | 直前リターンの符号 | — | adjusted | 日足 | 0.4978 | −0.0104 | ＋336.15 | −117.23 | `2026-09-18T03-47-08_trade_ownex_mlpgan_a_leak` |
...
| — | 直前リターンの符号 | — | adjusted | 日足 | 0.4974 | −0.0127 | ＋287.86 | −165.74 | `2026-09-10T20-06-42_trade_own_lgbm_b_leak` |
| — | 直前リターンの符号 | — | adjusted | 日足 | 0.4974 | −0.0127 | ＋287.86 | −165.74 | `2026-09-10T20-06-42_trade_own_lgbm_b_leak` |
| — | 直前リターンの符号 | — | adjusted | 日足 | 0.4974 | −0.0127 | ＋287.86 | −165.74 | `2026-09-19T13-18-20_trade_own_ridge_a_topk_leak` |
...
| — | 直前リターンの符号 | — | adjusted | 日足 | 0.4974 | −0.0127 | ＋287.86 | −165.74 | `2026-09-14T10-08-36_trade_own_mlpgan16k_b_leak` |
| — | 直前リターンの符号 | — | adjusted | 日足 | 0.4974 | −0.0127 | ＋287.86 | −165.74 | `2026-09-14T10-08-36_trade_own_mlpgan16k_b_leak` |
| — | 直前リターンの符号 | — | adjusted | 日足 | 0.4974 | −0.0127 | ＋287.86 | −165.74 | `2026-09-19T13-19-47_trade_ownseq_ridge_a_topk_leak` |
...
| 3 | ⚠ **「未実施」は「効かない」ではない** | ⚠ **3 件は情報が無いだけである。**⚠ **落とした 551 行と混同しない** |
