# 検証結果一覧の単位 — 実行 → 手法 → 検証 → fold → セル

作成日: 2026-09-10。検証結果一覧（[ledger.md](ledger.md)）を読むための、5 つの単位の図解。
...
| 実装（識別項目・数え方・畳み込み） | `ail/catalog.py`（`KEY`・`is_trial`・`_collapse`・`canonical`） | 同上 |
...
flowchart LR
  ...
  U -->|"読み方が分かる"| READER["検証結果一覧の読み手"]
```

⚠ **本書で引く数字はすべて【実測】で、2026-09-10 生成の検証結果一覧のもの。** 検証結果一覧を再生成すると変わる（行数・n_trials は実行を足すたびに増える）。
...
| **検証** | 検証結果一覧の 1 行。⚠ **複数の実行をまたいで、同じ識別項目の手法をまとめたもの** | [ledger.md §2](ledger.md) の行（識別項目 ＝ `catalog.KEY` の 9 列） | [rules.md §11・§13-9](rules.md) ／ `catalog.KEY` |
| **fold** | 検証の中のウォークフォワード分割 | `result.csv` の 1 行 ＝ 手法 × fold（× 閾値） | [rules.md §9](rules.md) |
| **セル** | 銘柄 × fold（最小の集計単位） | 新方式では `per_symbol.csv` の 1 行 | [rules.md §13-4・§13-7](rules.md) |

> この図の主張: 5 つの単位は素直な入れ子ではない。⚠ **実行 → 手法は入れ子だが、手法 → 検証は「複数の実行をまたいだ畳み込み」**である。ここが一番誤解されやすい。
...
flowchart TB
  ...
  MA --> T["検証 ＝ 検証結果一覧の 1 行<br/>再現: 2 実行"]
  MB --> T
...
⚠ **1 実行 ＝ 実行の記録（`runs/research.sqlite`）の 1 実行 `<時刻>_<実験名>`**（[rules.md §10](rules.md)。⚠ 2026-09-21 まではディレクトリ `runs/<時刻>_<実験名>/` で、中のファイルの道と中身は同じ）。中身は config の写し・入力の指紋・種・結果・ログで、⚠ **「どの設定・どのデータ・どのコードで出た数字か」を後から引けるようにする**ためにある。2026-09-10 生成の検証結果一覧では実行は 42 本（`runs/` 40 ＋ 旧配線 2）。
...
⚠ **`_leak` で終わる実行（わざと未来を混ぜた対照実験）は、検証結果一覧の本体に入らない**（[rules.md §7](rules.md) 規約 7）。
検証結果一覧では [§5「配線の検査」](ledger.md)の別の表に行く（2026-09-10 生成で 81 行）。実例: `2026-09-10T20-06-33_trade_own_lgbm_a_leak` は的中率 0.9980 で、⚠ **この行が本体に混ざると検証結果一覧全体が嘘になる**。

## 3. 手法 → 検証 — 複数の実行をまたいだ畳み込み

検証は「実行の中の 1 行」ではなく、⚠ **全実行の手法の行を識別項目（§4）でまとめ直した 1 行**である（`catalog._collapse`）。まとめ方は 2 方向ある。

| 向き | 規則 | 検証結果一覧での見え方 |
| --- | --- | --- |
| **まとめる** | 識別項目 9 列がすべて同じ手法は、実行が何本あっても 1 検証 | 「再現」列に本数と一致・幅が出る（例: 「2 実行・一致」「⚠ 5 実行・幅 4.35bp」） |
| **割る** | ⚠ **識別項目のどれか 1 列でも違えば、同じ手法名でも別検証** | 同じ手法名の行が複数並ぶ（モデル違い・層違い・閾値違い） |

実例（2026-09-10 生成の検証結果一覧・すべて【実測】）:
...
- **割れる**: 同じ B&H が、⚠ **数値まで同一（純利 ＋1652.02bp）なのに 6 行に割れている** — 形式（共通 / 銘柄別）× 閾値（50 / 55 / 60）が識別項目に入っているからで、⚠ **「行が割れる ＝ 数字が違う」ではなく「識別項目が違う」**である

> この図の主張: 同じ識別項目は実行をまたいで 1 行にまとまり、識別項目が 1 列でも違えば（数値が同じでも）別の行に割れる。

```mermaid
flowchart LR
  A["実行 ridge_a の<br/>B&H（共通・θ50）"] --> T1["検証結果一覧の 1 行<br/>再現: 2 実行・一致"]
...
（「F1-5 単変量検定 ＋ FDR」と「F1-5 検定+FDR」）、⚠ **名前で照合すると同じ検証が 2 行に割れる**（2026-09-08 に踏んだ）。

## 4. 識別項目の 9 列 — 検証を分ける自由度

識別項目は `catalog.KEY` の 9 列。⚠ **列の並びも名前も実装が正本**で、本書の表はその解説である。
...
| 9 | 閾値 | θ（50 / 55 / 60。旧方式は「—」） | ⚠ **1 水準 ＝ 1 検証**（[13-9](rules.md)） |

列は一度に決まったのではなく、処置の自由度が増えるたびに足された。⚠ **どの追加でも、旧実行は既定値で読むので既存の行は割れない**（後方互換）。

| 追加日 | 足した列 | 旧実行の読み方 |
| --- | --- | --- |
| 2026-09-08 | 鍵・粒度・地平・特徴量の層・層（検証結果一覧の新設） | — |
...
2026-09-10 生成の検証結果一覧では、⚠ **166 行のうち手法 91 行・基準線 75 行で、n_trials ＝ 91**【実測】。数えるかどうかは `catalog.is_trial` が機械的に決める。

> この図の主張: 数える / 数えないは行の中身から機械的に決まり、手では選ばない。

```mermaid
flowchart TB
  R["検証結果一覧の 1 行"] --> V{"検証方式は"}
  V -->|"閾値売買"| TB1{"基準線か<br/>（基準 ・乱択）"}
  TB1 -->|"はい"| N1["数えない"]
  TB1 -->|"いいえ"| Y1["数える<br/>⚠ 閾値 1 水準 ＝ 1 検証"]
...
fold は検証の中のウォークフォワード分割で、時刻順に 6 等分した後ろ 5 つが評価期間になる（fold 1〜5。パージ・エンバーゴは [rules.md §9](rules.md)。閾値売買では切れ目を日付で決めて両形式に配る — [13-6](rules.md) の 1）。
⚠ **実体は `result.csv` の行**（1 行 ＝ 手法 × fold（× 閾値））で、検証結果一覧の「fold」列の符号パターンはここから機械的に出る。
...
| 検証方式 | 符号を取る量 | 実例（2026-09-10 生成の検証結果一覧・【実測】） |
| --- | --- | --- |
...
⚠ **数字はすべて【実測】**（2026-09-10 生成の検証結果一覧と、`runs/` に実在するファイル）。検証結果一覧を再生成すると「再現」列などは変わる。

パターンは「単位の見え方が変わる」2 本を選んだ。

| | パターン A（§8-1） | パターン B（§8-2） |
| --- | --- | --- |
| 入口 | 実行 `2026-09-10T20-05-14_trade_own_lgbm_a` | 検証結果一覧の 1 行「F1-2 相互情報量 × own ex」 |
| 検証方式 × モデル | 閾値売買（新）× LightGBM | 毎日往復（旧）× Ridge |
| 辿る向き | 実行 → 検証結果一覧（上から下へ展開） | 検証結果一覧 → 実行（1 行を 5 実行へ分解） |
| 検証 | ⚠ **1 実行だけ**（「再現」列は「—」） | ⚠ **5 実行の畳み込み**（幅 4.35bp） |
| セル | per_symbol.csv 3,780 行がある | ⚠ **無い**（旧方式はセルを記録しない） |

### 8-1. パターン A — 新方式の 1 実行を上から下へ（trade_own_lgbm_a）

**実行**: `runs/2026-09-10T20-05-14_trade_own_lgbm_a/` の 1 ディレクトリ（§2 のじょうご図と同じ実行）。config.json の要点と、識別項目（§4）への効き方:

| config.json の項 | 値 | 識別項目のどの列になるか |
| --- | --- | --- |
| `model` | LightGBM | モデル |
...
- ⚠ **基準線の行は θ が変わっても同じ数字**（θ は手法側の足切りで、基準線は同じ売買が θ ごとに並ぶだけ）。検証結果一覧で B&H が「数値まで同一の 6 行」に割れるのはこのためである（§3）
...
**検証**: この実行の 12 行はすべて検証結果一覧へ行くが、⚠ **検証として数えるのは全部使う × 3 閾値 ＝ 3 行だけ**（§5: 閾値売買では乱択・「基準 」は数えない。閾値 1 水準 ＝ 1 検証）。3 行とも同じ識別項目の実行が他に無いので「再現」列は「—」＝ 1 実行だけの検証。いっぽう同じ実行の B&H（own・共通）は `trade_own_ridge_a` と識別項目が一致し（⚠ モデル列が「—」に寄る）、「2 実行・一致」へ畳み込まれる（§3 の実例そのもの）。
...
> この図の主張: 検証結果一覧の「0/5 00−00」と「−54.78bp」は、result.csv の 10 行の引き算から機械的に出る。
...
検証結果一覧の該当行（2026-09-10 生成・【実測】）: 全部使う（基準）× LightGBM × own × 閾値売買 × 共通 × θ50 ＝ 純利 ＋1597.23bp・fold「0/5 00−00」・判定 **落とす**「⚠ 基準線以下 ＝ 何も学んでいない（対 B&H 上乗せ −54.78bp ≤ 0）」。⚠ **純利 ＋1597bp が正でも、それはドリフト（B&H ＋1652bp）の写しである**（§6 で「純利の符号を使わない」理由の実物）。
...
### 8-2. パターン B — 旧方式・検証結果一覧の 1 行を 5 実行へ分解する（F1-2 相互情報量 × own ex）

今度は逆向きに、検証結果一覧の 1 行から出発して下へ降りる。検証結果一覧の該当行（2026-09-10 生成・【実測】）:

F1-2 相互情報量 × Ridge × own ex × 毎日往復 ＝ 純利 **＋2.60bp**・fold「3/5 ＋＋−＋−」・再現「⚠ **5 実行・幅 4.35bp**」・判定 **保留**。

**検証 → 実行**: この 1 行に畳み込まれた 5 実行を `runs/` から引く:
...
> この図の主張: 検証結果一覧の代表値は一番新しい実行の数字で、残り 4 本は「再現」列の幅にしか現れない。

```mermaid
flowchart LR
  E1["exog_h1（00-50）<br/>−0.45"] --> T["検証結果一覧の 1 行<br/>純利 ＋2.60<br/>⚠ 5 実行・幅 4.35bp"]
...
- ⚠ **幅の出どころは識別項目の粗さである**。識別項目の「特徴量の層」は own ex という**名前**で、プールの中身（85〜57 本）までは見ない。だから外生列の中身が違う実行が同じ検証に畳み込まれる。⚠ **幅が出たら、まず配線を疑う**（[rules.md §11](rules.md) 規約 7）——疑う先は、この表のように各実行の inputs.json を並べたものである
- **代表は新配線の一番新しい実行**（`catalog._collapse`）。検証結果一覧の純利 ＋2.60・的中率 0.5133 は `impact_ex_2018` の summary.csv の 1 行**そのまま**で、5 本の平均ではない

**実行 → 手法**: 代表 `2026-09-10T16-18-54_impact_ex_2018` の config.json は selectors 11 本（F1〜F3 系 9 ＋ 全部使う・乱択）で、⚠ **`trading` 節が無い → 旧方式**（識別項目は 毎日往復・共通・— として読む。§4）。summary.csv は 13 行 ＝ 13 手法（⚠ **閾値の列そのものが無い。1 行 ＝ 手法**）。F1-2 相互情報量はその先頭行（純利の降順で 13 手法中 1 位）。
...
→ 検証結果一覧の「3/5 ＋＋−＋−」はこの 5 行から出る。⚠ 旧方式の符号は純利そのもの（B&H との引き算は入らない。§6）。平均 ＋2.60bp は fold 1 の ＋13.25 が引っ張った数字で、⚠ **平均だけ正 → 保留**（[rules.md §11](rules.md) 規約 5）。

**セル**: この実行に per_symbol.csv は**無い**。旧方式の純利 bp は全セル（銘柄 × fold）の単純平均で、⚠ **セルは集計の途中で消えて記録に残らない**（§7）。セルを実ファイルで触れるのは新方式（パターン A）だけである。

## 付録: 検証結果一覧を読むときの早見

| 検証結果一覧で見るもの | 対応する単位 | 本書 |
| --- | --- | --- |
| §2 の 1 行 | 検証 | §3・§4 |
| 「再現」列 | 検証 ← 実行の畳み込み | §3 |
