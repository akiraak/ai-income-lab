...
⚠ **再測は 0 件と決めた。** 再測は 1 行あたり ＋3〜6 試行を払って、⚠ **同じ検出限界の下で同じ答えを得るだけ**である（§6-2 と同じ判断）。
⚠ **閉じても行は消さない・判定も変えない。** ⚠ **閉じた行は台帳の分母に残り続ける**（`n_trials` は減らない。それが正しい）。

**実装**: `ail/catalog.py` の `_CLOSED_FIELDS` に ⚠ **`銘柄` を足した**（us63 / us70 / us74 の D 系は銘柄しか違わず、⚠ **鍵が足りないと「3 行に一致」で生成が止まる**）。⚠ **足したのは照合の鍵だけで、判定の規則は 1 行も変えていない。**

**検算**【実測 2026-09-16】: ✅ 台帳の生成が止まらない（`[[closed]]` **78 件**＝ 16 ＋ 62 が 1 行ずつに当たる）／ ✅ **保留は 78 行のまま**（閉じても保留は保留）／ ✅ **判定が「保留」でない行に注記が付いていない**（0 件）／ ✅ **未処遇の保留 0 行** ／ ✅ `pytest -q tests` **332 件 pass**。
