...
## 8. ⚠ 2026-09-17〜19 — `ex_` 層の修正後に 24 本を同じ鍵で回し直した（⚠ **36 行とも「落とす」のまま**）

⚠ **本書の 36 行は `ex_` 層の穴 2 つ（[daily-data-sources.md §14-2](daily-data-sources.md)）を塞ぐ前のコードの数字である。**
2026-09-17 16:58 〜 09-19 07:21 に、コードを凍結した worktree（HEAD `696cf7b`）から 24 本（本番 12 ＋ leak 12）を手間の小さい順に回し直した（38.4 時間・GPU・失敗 0。[daily-data-sources.md §16-4](daily-data-sources.md)）。
⚠ **18:30 時点で 16k (a) の 3 対（本番 3 ＋ leak 3）が終わった**【実測】。§1-2 の (A) 9 行は **9 行とも「落とす」のまま**（上乗せは 3 モデル × 3 水準とも負）。

| 実行（§1-2 の行） | 純利の最大差 bp | θ=50 上乗せ 旧 → 新 | θ=55 | θ=60 |
| --- | ---: | --- | --- | --- |
| Ridge+(16k) (A) | 453.2 | −479.0 → −585.0 | −709.2 → −655.0 | −1,405.5 → −1,424.2 |
| LightGBM+(16k) (A) | 635.9 | −549.0 → −902.6 | −748.0 → −715.4 | −1,173.3 → −537.4 |
| MLP+(16k) (A) | 690.9 | −479.8 → −545.7 | −382.5 → −903.8 | −289.6 → −980.5 |
| leak 3 本 | 162.7〜646.5 | ＋23,269〜＋23,756・5/5 | 〃 | 〃 |

⚠ **差は素のモデル（29〜136bp。[daily-data-sources.md §16-3](daily-data-sources.md)）より 1 桁大きい** — 生成器が特徴量の分布を学ぶので `z20` の変化が増強標本ぜんぶに伝わる【推測】。fold の符号列は 9 行中 8 行で動いたが、平均の符号を跨いだ行は無い。
**残り 18 本【実測 2026-09-19】** — §1-1（batch 1,024）の 18 行と §1-2（16k）の (B) 9 行。

| 実行（§1 の行） | 純利の最大差 bp | θ=50 上乗せ 旧 → 新 | θ=55 | θ=60 |
| --- | ---: | --- | --- | --- |
| Ridge+(1,024) (A) | 774.1 | −319.1 → −386.6 | −718.1 → −555.1 | ⚠ −1,147.9 → −494.7 |
| LightGBM+(1,024) (A) | 802.7 | −770.1 → −298.6 | −858.2 → −585.3 | ⚠ −1,330.4 → −527.7 |
| MLP+(1,024) (A) | 761.4 | −406.6 → −108.4 | −69.9 → −662.9 | ⚠ −1,676.5 → −915.1 |
| Ridge+(16k) (B) | 52.0 | −755.5 → −740.3 | −671.2 → −623.7 | −879.0 → −908.9 |
| LightGBM+(16k) (B) | 264.9 | −734.9 → −999.9 | −894.3 → −959.6 | −847.9 → −1,000.1 |
| MLP+(16k) (B) | 68.0 | −663.4 → −658.5 | −635.8 → −703.8 | −800.3 → −851.8 |
| Ridge+(1,024) (B) | 120.2 | −757.0 → −877.2 | −690.7 → −703.3 | −929.0 → −927.3 |
| LightGBM+(1,024) (B) | 249.3 | −850.9 → −1,100.3 | −858.8 → −908.7 | −935.9 → −928.7 |
| MLP+(1,024) (B) | 148.1 | −704.5 → −848.2 | −692.5 → −811.2 | −850.0 → −868.9 |
| leak 9 本 | 23.3〜1,100.6 | ＋9,620〜＋23,811・5/5 | 〃 | 〃 |

**36 行の判定の変化: なし。** 36 行とも「落とす」のまま（上乗せの平均の符号を跨いだ行は 0）。leak の 36 行も「採る」のまま（§4 の「跳ねた」は変わらない）。同じ鍵なので n_trials は動かない（613 のまま）。⚠ **§1〜§3 の表は書き換えていない**（直す前のコードの数字として残す。台帳の代表は新しい実行になる）。

| 読み | 中身 |
| --- | --- |
| (A) は大きく動き、(B) はほとんど動かない | 純利の最大差は (A) 453〜803bp・(B) 52〜265bp。fold の符号列が動いたのは (A) 18 行中 17 行・(B) 18 行中 0 行（(B) は 18 行とも −−−−− のまま） |
| ⚠ 600bp 超の動き 5 行は全部 (A) の θ=60 | §1-3 の「fold 単位で全部持つ ／ 全部休むに振れる」行。1,024 の 3 行は負けが縮み（−1,148 → −495・−1,330 → −528・−1,677 → −915）、16k は LightGBM が縮み（−1,173 → −537）・MLP が広がった（−290 → −981）。⚠ **§1-3 の読み（どの fold が倒れるかは較正 1 組の置き場所で決まり、batch の性質ではない）と同じ向きの事実**だが、⚠ **回し直した実行の保有日率・較正の傾きは測り直していない**（§1-3 の表は直す前の数字のまま） |
| ⚠ 「修正で良くなった」とは読まない | 5 行のうち 4 行は負けが縮み、1 行は広がった（向きが揃わない）。どの行もまだ B&H に 100bp 以上負けている（rules.md 11 章 規約 7） |
| §2（素のモデルとの差）を修正後どうしで数え直した【実測】 | ⚠ **(B) は 18 セル中 16 で GAN 増強が負けを大きくした（修正前と同じ 16）**。(A) は正 5・負 13（修正前は正 4・負 14）。⚠ **「own の向きは ownex で再現しなかった」は動かない**。⚠ 数えたのは上乗せの平均の差の符号だけで、fold ごとの t は出し直していない（検算: 同じ数え方で修正前どうしを数えると §2 の表の 36 セルと一致した） |
