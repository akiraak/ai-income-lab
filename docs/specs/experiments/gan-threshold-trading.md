...
| **GAN 増強 3 種 × 閾値売買 × ownex**（＋18 試行） | ⚠ **未実施**（プラン §0-2 の規則で、費用が「> 2 時間」の枝だったので外した）。⚠ **「効かなかった」ではなく「未実施」**（[14-10 規約 4](feature-discovery/rules.md)）。⚠ **16k の (A) は 1 本 33 分・(B) は 3.8 時間**【実測】なので、⚠ **(B) を含めると依然として重い** |
