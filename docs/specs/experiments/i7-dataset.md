...
> （[TODO.md 前提の見直し](../../../TODO.md)）。TODO の起票理由は「**I7 / B14 の時給と案件（日本在住・日本語ネイティブ向け）を米国基準で見直す**」だった。

### 結論: **I7 の打ち切りは維持し、根拠が強くなった。B14 は水準が上がった**

**I7 と B14 で、居住地の効き方が正反対だった。**

| | I7（データセットを作って売る） | B14（AI 開発者向けの受託） |
| --- | --- | --- |
| 日本居住での位置づけ | 案 B は「**日本語ドメイン特化**の評価セット」。日本語データ不足が唯一の需要側の論拠だった | 日本在住・**日本語ネイティブ**向けの案件で $31〜65/時 |
| 米国居住での変化 | ⚠ **その論拠が丸ごと使えなくなる**。日本語の希少性は米国居住者の持ち札ではない | **言語の希少性に依存しない案件**へ移る。ソフトウェア開発なら $65〜110/時 |
| 判定 | **打ち切り維持**（根拠は強くなった） | **成立。ただし L1 の労働という性質は変わらない** |

### I7 — 打ち切りの根拠が強くなる

初版の打ち切り理由は「**個人がデータセットを作って売った実例がゼロ**」で、
これは日本に限った話ではなく**世界的に確認できなかった**もの。居住地では動かない。

⚠ **それに加えて、米国居住では唯一の需要側の論拠が消える。**

| 論点 | 日本居住 | 米国居住 |
| --- | --- | --- |
| 需要側の論拠 | 「深刻な日本語データ不足」（GENIAC・LLM-jp・Common Crawl の言語比 日本語 5%） | ⚠ **該当しない**。英語は Common Crawl の 45% で、希少性が無い |
| 案 B のテーマ | 「ソフトウェア開発の**日本語**評価セット」 | ⚠ **前提が消える**。英語で作れば、供給側の競合が桁違いに多い |
| 常設の買い取り窓口 | 無し（AWS Data Exchange は制度上可能だが実売例ゼロ） | **変わらず無し** |

**つまり米国居住では、I7 は「実例ゼロ」に加えて「言語による差別化も無い」状態になる。**
⚠ 初版が「日本語なら勝てるかもしれない」という余地を残していたのに対し、その余地が消えた。**打ち切りはより明確になる。**

なお `experiments/i7-dataset/` の生成パイプライン（qwen3:8b で日本語データを生成）は、
⚠ **米国居住前提では想定していた出口が無くなる**。他の合成データ実験に再利用できるという初版の位置づけは維持する。

### B14 — 案件と時給を米国基準で取り直した

**言語の希少性で稼ぐ形から、専門性で稼ぐ形に変わる。**

| 職種 | 日本基準（2026-08-26） | 米国基準（2026-08）【Web 確認済（二次）】 |
| --- | --- | --- |
| 全体平均 | — | $31/時 |
| RLHF・選好アノテーション | Outlier 日本語 最大 $31/時 | Outlier $30〜42 ／ Mercor $40〜55 ／ Surge $35〜50 |
| **コーディング評価（上位）** | — | **Outlier $65〜75 ／ Mercor $85〜110 ／ Surge $70〜95** |
| ドメイン専門家（法務・医療） | Mercor 日本在住ネイティブ $55〜65/時（募集終了） | Outlier $60〜95 ／ **Mercor $90〜160** ／ Surge $80〜140 |
| プラットフォーム全体のレンジ | — | Mercor $25〜250+ ／ Scale AI $15〜80 ／ DataAnnotation $15〜45 |

⚠ **確定セグメントの既存スキルは「ソフトウェア開発」なので、当たるのはコーディング評価の帯（$65〜110/時）。**
日本基準の $31〜65/時 から**おおむね 2 倍**になる。

**週 5〜15 時間での月収【推測】**（週 h × 52 ÷ 12 で月間時間に換算）

| 週あたり | 月間時間 | 月収（$65/時） | 月収（$110/時） | 年収換算 |
| --- | ---: | ---: | ---: | ---: |
| 週 5 時間 | 21.7 h | $1,408 | $2,383 | $16,900〜28,600 |
| 週 10 時間 | 43.3 h | $2,817 | $4,767 | $33,800〜57,200 |
| 週 15 時間 | 65.0 h | $4,225 | $7,150 | $50,700〜85,800 |

⚠ **上表は税引前かつ稼働が埋まる前提**である。次の 3 点を差し引く必要がある。

| 減算要因 | 内容 |
| --- | --- |
| ⚠ **自営業税** | 業務委託なので 15.3% の自営業税がかかる。第三者情報に「$55/時の表示は W-2 換算で $35〜40/時に近い」という指摘がある【Web 確認済（二次）】 |
| ⚠ **稼働が保証されない** | 「時給 $90 で週 6 時間」は「時給 $55 で週 22 時間」に劣る。**時給ではなく供給量が効く**【Web 確認済（二次）】 |
| ⚠ **供給の消失** | 初版で記録した Outlier の 2025 年のタスク枯渇（Meta の Scale AI 出資後）は構造的リスクとして残る |

**州所得税はゼロ**（WA 州）なので、州レベルの減算は無い。

### ⚠ 米国居住で新たに効く論点 — 労働者誤分類

初版が 1 行だけ触れていた「受託系は米国で労働者誤分類の訴訟が 2025〜2026 年に提起されている（Surge・Mercor・Scale）」は、
**日本居住なら他人事だったが、米国居住では自分が当事者側の分類に入る**。

⚠ 業務委託（1099）として働く前提が争われている状況で、**分類が変われば報酬体系・税・稼働の保証がすべて変わる**。
B14 を「回収が最も確実」と評価する際の前提として、この不確実性は記録しておく。

### 体系へのフィードバック（更新）

- **I7 の AI 差分判定「新規」は維持**。市場は実在し金銭も動いている。個人の入口が無いという結論も変わらない
- **I7 の打ち切り理由に 1 行足せる**: 「実例ゼロ」に加え、⚠ **言語による差別化の余地も米国居住では消える**
- **B14 の水準を $31〜65/時 から $65〜110/時 へ更新する**（ソフトウェア開発スキルの場合）。catalog の B14 備考に反映する
- ⚠ **居住地が「作る側」と「売る側」で逆に効いた**のは記録に値する。I7（作る側）は言語の希少性を失って不利になり、
  B14（労働を売る側）は単価が上がって有利になった。**同じ居住地の変更が、体系上の位置によって逆向きに効く**

### 出典（米国基準の再判定分。すべて 2026-08-28 取得）

| # | 内容 | URL |
| --- | --- | --- |
| 1 | 米国の AI トレーナー時給（Mercor $25〜250+、Scale AI $15〜80、DataAnnotation $15〜45、全体平均 $31/時） | https://www.aigigjobs.com/blog/mercor-complete-guide |
| 2 | 職種別の時給比較（コーディング評価・RLHF・ドメイン専門家。Outlier / Mercor / Surge AI、2026） | https://hirefeed.co.in/blog/outlier-vs-mercor-vs-surge-2026 |
| 3 | 自営業税を織り込むと表示時給は W-2 換算で下がる／稼働量が時給より効く | https://hirefeed.co.in/blog/outlier-vs-mercor-vs-surge-2026 |
