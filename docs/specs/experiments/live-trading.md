...
| `T3` | `trade_ownseq_ridge_a` の検知器「T3 QUANT（60日発注できる時間帯）」。`asis`・種 0 | 50 | A $300 ／ B $3,000 | 同上 | ✅ 2026-09-19 確定 |
...
| 1 日の売買回数 | トレーダーの判断（CLAUDE.md の 2 つ目の例外。2026-09-18）。⚠ **いまの執行器は 1 日 1 回の発注できる時間帯（§0-2）のまま** | — |
...
| 1 日の買いの合計の上限 | **規模 A $1,000**（既定）／ **規模 B $10,000**（同上）。⚠ **全トレーダー・全起動の合計**（2026-09-20 に直した） | `--max-day-usd`（既定 1000）。起動時に**その日の `orders.jsonl` から使った額を数え直し**（`plan.spent_today`）、`plan.DayCap` を全員で分け合う。数えるのは口座へ出した買い（約定は約定額・未確定は注文額 ＝ 保守側。取消 ／ 拒否 ／ エラー ／ dry-run は数えない）。超えた買いは `over_day_cap` で見送り、その日の合計は `end` の `day_spent_usd` に残る |
| 1 人の予算 | `budget_usd`。空き ＝ 予算 − 建玉の取得原価 − 受渡し待ちの売却代金（暦日 T+1・保守側） | `plan.size_intents`（超える買いは `over_budget` ／ `too_small` の事象として記録し見送る） |
| 発注できる時間帯 | **15:45〜16:05 ET**（⚠ **NYSE の営業日**。2026-09-18 から休場日を暦で見る）。合図は 15:50 の気配、注文は成行 | `run_day.window_refusal`（`in_window`）。外では発注せず、理由を `events.jsonl` の `out_of_window.reason` に残す（`--ignore-window` はテスト用）。暦は `tastytrade-api-sample/market_calendar.py`（管理画面と同じもの。【公表値】NYSE・取得 2026-09-18） |
| 半日立会の日 | ⚠ **発注しない（未決のあいだの既定）**。13:00 ET 引けなので、発注できる時間帯 15:45〜16:05 は引けの後になる。20 営業日の実験にかかるのは **2026-11-27（金）と 2026-12-24（木）** | `window_refusal` が「半日立会」で拒否する。⚠ **発注できる時間帯を 12:45〜13:05 に動かすかは利用者の裁定**（TODO の D13） |
| 未約定の取消 | 発注できる時間帯の終わり（発注から 600 秒） | `--cancel-after` |
...
✅ **2026-09-20 の利用者決定 D1 ＝ 案 B**: `test_a` の往復を **9/21（月）の 1 日で済ませる** ＝ 朝に買い（窓の外なので `--ignore-window`）・窓の中で売り。売りの経路を月曜のうちに本番で確かめ、火曜の窓を 3 人の投入だけに使う。同日の往復は規制上は可（受渡し済みの資金で買った株。§0-6）。⚠ 買いが決めごとの窓の外になるが、試験用（`test: true`）なので判定に混ぜない。同じ日に 2 回起動して 買い → 売り が通ることはモックで確かめた【実測 2026-09-20】。全体の段取りは [プラン](../../plans/archive/live-trading-go-live-0922.md) §3-2。
...
| 決めごと | 値 |
| --- | --- |
| 発注できる時間帯 | いまの執行器どおり **1 日 1 回・15:45〜16:05 ET**。⚠ 運転手は「時刻の一覧」で持てる（⚠ 執行器を広げるかは 2026-09-20 に「広げない」と決めた ＝ §0-2） |
| その時間帯の外 | 既定は次の時間帯まで飛ぶ（何も起きないので）。飛ばさない設定も持つ |
| 速さの初期値 | ×60（20 分 ＝ 20 秒・再送 60 秒 ＝ 1 秒）。「その時間帯の中で片付くか」を読むのは ×1〜×60 だけ |
...
| 仮の時計 | `simclock.py`（正本は `sim/<名前>/sim/control.json` の 1 ファイル） | 仮のいま ＝ 錨 ＋ 実時間の経過 × 速さ。運転手・執行器・モック・管理画面が同じファイルを同じ式で読む ＝ 別プロセスでも同じ「いま」。速さ・停止を変える側が錨を打ち直すので時刻が飛ばない。最速は待たずに錨を進める（決定的）。⚠ **運転手が終わるときは必ず時計を止める**（いない間に進むと発注できる時間帯を通り過ぎる） |
...
| 運転手 | `simrun.py`（`simrun.sh` は検査つき） | モックを起こす（空きポート・`--sim-control` で時刻と遅延も仮の時計）→ 発注できる時間帯まで飛ぶ ／ 待つ → その日の気配を流す → 執行器を起こす → `sim/status.json`。⚠ 子へ `TT_*`・`LT_*` を渡さない（`.env` は空のファイル）。`.env` に資格情報があれば警告 |
...
| 28 | `no_fill` | 取消までの 600 秒を**仮の時計の上でそのまま**待って取り消す・発注できる時間帯（〜16:05）の中で片付く・状態は進まない | ✅ |
...
#### (l) 自動で回せるもの（2026-09-20。利用者の指示「自動でテストを回せるものはある？」）

⚠ **通し運転は「落ちないこと」しか見ていなかった** ＝ 執行器を触って振る舞いが変わっても気づけなかった。2026-09-20 に 3 つ足した。

> この図の主張: 層ごとに守るものを分ける。通し運転には「集計値が変わったら気づく」という役目を与えた（ここが空白だった）。

```mermaid
flowchart TD
  U["単体・結合<br/>pytest 122 ＋ 管理画面 174"] --> S["筋書き<br/>test_sim_scenarios（16）"]
  S --> L["限界の通し運転（新）<br/>test_sim_limits（7）"]
  L --> G["黄金の集計値（新）<br/>test_sim_golden（12）"]
  G --> B["ブラウザ検査（手で流す）<br/>見た目・操作感"]
```

```bash
./run-tests.sh            # ⚠ ふだんはこれ 1 本（プロジェクト直下）。執行器 → 管理画面 → selftest → mockrun（約 1 分 34 秒【実測】）
./run-tests.sh --fast     # pytest 2 つだけ（約 38 秒【実測】）
./run-tests.sh --full     # ＋ 黄金の集計値（通し運転 64 営業日 × 2 本。合計 2 分 3 秒【実測 2026-09-20・Sx360】）
```

| 何を守るか | どこ | 中身 |
| --- | --- | --- |
| **黄金の集計値** | `tests/test_sim_golden.py` ＋ `tests/golden/sim{1,2}.json` | `sim1`／`sim2` を最速で通し、**出来事の種類ごとの件数・注文の数**を覚えた値と突き合わせる。⚠ **値が変わったら、直す前に理由を確かめる**（更新は `AIL_UPDATE_GOLDEN=1` のときだけ）。⚠ **重い**ので `AIL_GOLDEN=1`（＝ `--full`）のときだけ。日足が無い機械では skip |
| 決めごとの不変条件 | 同上 | 1 注文 1 トレーダー（`parts` は必ず 1 件）／ 整数株の人に端数が無い ／ 口座 − 台帳 ＝ 0（⚠ `sim2` は筋書きで抜いた BAC の −100 だけ）／ 全行に印・秘密なし |
| 筋書きが効いた証拠 | 同上 | `out_of_window` 2（半日立会）・`journal_recovered` 2（⚠ 2026-09-22 に 1 → 2。発注が 2 段になり、落ちた日の買い 2 本が両方とも口座に届いてから落ちるため）・`auth_failed` 1・`drawdown_warning` は `sim_b` だけ・止まる銘柄は BAC だけ・起動しなかった日がある |
| **予算の上限** | `tests/test_sim_limits.py` | 何日流しても取得原価が予算を超えない。⚠ **予算は「枠に丸める」（`target = min(per_symbol, available)`）ことで守られていて、`over_budget` は現在の経路では出ない** — 枠で 1 株も買えないときは `too_small` になる（⚠ 下の「気づいたこと」） |
| **1 日に 2 回の発注** | 同上 | 同じ日に 2 回起きて、⚠ **二重に買わない**（1 回目で持ったら 2 回目は `hold`）。⚠ 発注できる時間帯は 15:45〜16:05 ET に固定なので、2 回ともその中に置く |
| ⚠ その時間帯の外は拒む | 同上 | 10:00 に起こしても `out_of_window` で拒まれる。⚠ **2026-09-20 に「広げない」と決めた**（予測モデルが日足なので 1 日に複数回動かす意味が無い ＝ §0-9）ので、この検査は**決定の番人**になる |
| **停止と解除** | 同上 | 途中で `HALT` を置くとその日から `halted`（rc=3・発注なし）、消すと翌日から起動して売買が戻る。⚠ `halt` は筋書き（設定）に入れない（(i) の決めごと「人が押す」）＝ テスト自身が置く |
...
`tests/test_sim_run_day.py` が「本物の `MODE` が存在しない」ことを前提にしていた（→ 前後で変わらないことを見る形に直した）。
⚠ **テストは `LT_MODE_DIR`／`AIL_MODE_DIR` を必ず tmp に向ける**（(a) の決まりの延長）。
...
### 0-9. 今日の買い% ・1 日の流し方・2 段の発注・本番投入の手順（2026-09-20。[プラン](../../plans/archive/live-trading-go-live-0922.md)）
...
✅ **2026-09-23（水）に投入した**（9/22 は起動が遅れて見送り ＝ §1 の落とし穴）。結果は §1 の 3 行。初日は T1・T3 が 5 本ずつ・T2 は 0 本（5 銘柄とも同じ買い% で θ 未満 ＝ 上の「1 本も買わない」側）。

#### (f) 予測モデルの入力と出力の形式 — いつ学習し、何が入り、何が出て、トレーダーがどう使うか（2026-09-23。利用者の指示「予測モデルタブの「詳しく」と §0-9 の両方に書いて」。やさしい言葉の写しは vibeboard の予測モデルタブ「実際の売買で使う」の段「モデルに入るものと出てくるもの」）

> この図の主張: 「完成したモデル」は保存した重みではなく作り方で、毎営業日 15:50 ET 過ぎに学習し直してから今日の買い% を出し、執行器がその場でそれを注文に変える。

```mermaid
flowchart LR
  D[("data-live/<br/>日足 2018-01-31 〜 今日の途中の足")] --> T["表を組む<br/>1 行 ＝ 1 銘柄 × 1 営業日"]
  T --> TR["訓練の行<br/>ラベルが asof より前に確定"]
  T --> TE["予測する行<br/>asof の 1 行 × 銘柄"]
  TR --> F["fit（毎日）<br/>StandardScaler → 学習器 → Platt"]
  TE --> F
  F --> P["買い% ／ 出口%<br/>predict.jsonl"]
  P --> S["signals.py<br/>今日の行だけ読む"]
  S --> DC["plan.decide<br/>θ と持ち株で 買い ／ 売り ／ hold"]
  DC --> SZ["plan.size_intents<br/>枠 ÷ 中値 → 整数株"]
  SZ --> O["注文 15:45〜16:05 ET<br/>1 注文 1 トレーダー"]
```

**いつ**（平日。時刻は ET。【実測 2026-09-23 titan ＝ §1】）

| 時刻 | 何が起きる | 学習・入力・出力 |
| --- | --- | --- |
| 09:00 | `run-live.sh --prepare`（日足 ＋ 外部系列 約 2 分 → 紙上の対照） | データを揃えるだけ。学習も予測もしない |
| 15:50:00 | `--wait` が明ける → 日足の更新 55 秒 | 末尾に**今日の途中の足**（終値の代役）が入る |
| 15:50:55 〜 15:51:38 | `cli.predict` × 3（並列 43 秒。13500t は 48 秒） | ⚠ **ここで学習 ＋ 予測**。重みは保存せず、毎回 2018-01-31 から fit し直す（訓練と推論を分けない ＝ (a) の B7） |
| 15:51:39 〜 15:51:49 | `run_day.py`（10 秒） | 買い% → 意図 → 注文 → 約定の確認 → 売買履歴 |
| 翌朝 | `--prepare` の `paper.py` | 公式終値で紙上の対照（差 3）。予測には使わない |

**入力の形式**（1 行 ＝ 1 銘柄 × 1 営業日。元は `data-live/adjusted/d/<銘柄>.csv` ＝ `time_ms`・`open`・`high`・`low`・`close`・`volume`・分割だけ調整）

| | T1 `trade_own_ridge_a` | T2 `trade_ownex_lgbm_a` | T3 `trade_ownseq_ridge_a`（T3 QUANT〔60日窓〕） |
| --- | --- | --- | --- |
| 列 | `own_` 35（`ail/features/own.py`） | 124 ＝ own 35 ＋ cs 26 ＋ rel 13 ＋ ex 50（`cross.py`・`relative.py`・`exog_live`） | 表は 95（own 35 ＋ `own_seq60_r0..r59`）。⚠ 使うのは 60 日の道筋だけ → QUANT の変換で 653 列 |
| 観測期間（過去何日を見るか） | 最長 60 日 | own 60・rel 60・ex は前日差と 20 日 z | 60 日 |
| 株の外の系列 | 無し | ECB 為替 7・米国債 7・NOAA 9・USGS 2 を 1 日ずらし（前の値の引き継ぎは 7 日まで） | 無し |
| 予測する銘柄 | us63 の 63 | 会社株 48（ETF は材料にだけ使う） | 63 |
| 前処理 | StandardScaler（訓練で fit）・inf → NaN → 落とす | 同左。⚠ ex_ が古いと 48 本とも落ちて `predict` が失敗 → その回の執行器は起きない（T1・T3 も） | QUANT の変換（訓練で fit）→ StandardScaler |
| ラベル y（訓練だけ） | 1 日先（horizon 1）の終値との対数リターン | 同左 | 同左 |
| 訓練の範囲 | 2018-01-31 〜 asof の 2 営業日前（`label_end < asof`。前の営業日の行はラベルが今日の終値を含むので入れない） | 同左 | 同左 |
| asof 2026-09-22 の実測【13500t 2026-09-23】 | 訓練 136,529 行・予測 63 行・表 38.4 秒 ＋ fit 0.2 秒 | 101,589 行・48 行・40.5 ＋ 2.0 秒 | 136,529 行・63 行・38.9 ＋ 6.2 秒 |

**出力の形式**（`out/<日付>/predict-<時刻>.jsonl`。2026-09-21 から中身は `live.sqlite`。1 行 1 銘柄 × モデル ＝ 63 ＋ 48 ＋ 63 ＝ 174 行）

| 項目 | 型 | 意味 |
| --- | --- | --- |
| `date` | `YYYY-MM-DD` | asof（予測した日 ＝ 今日）。⚠ 執行器は今日と違う行を読まない（古い予測で売買しない） |
| `model` ／ `method` | 文字列 | 実験名 ／ 数字の選び方・作り方の登録名。⚠ トレーダー設定の `[[models]]` の `name` ／ `method` と突き合わせ、違えば止まる |
| `symbol` | 文字列 | 銘柄 |
| `buy` ／ `exit` | 0〜100 の実数（小数 6 桁） | 買い%（Platt 較正を通した「1 日先に上がる」の見込み）／ 出口% ＝ 100 − 買い% |
| `proxy_close` ／ `proxy` | 実数 ／ 真偽 | その日の終値の代役に使った値（15:50 の途中の足）／ `--proxy` で上書き・追加した足か |
| `input_fingerprint` ／ `commit` | 16 進 16 桁 ／ 7 桁 | 訓練と予測の入力の指紋（⚠ CPU で変わる ＝ §0-12。比べるのは判定）／ コードの commit |

```json
{"date": "2026-09-22", "model": "trade_own_ridge_a", "method": "全部使う（基準）", "symbol": "NKE", "buy": 50.326527, "exit": 49.673473, "proxy_close": 36.0816, "proxy": false, "input_fingerprint": "67a353b73999ac5e", "commit": "789b3dd"}
```

予測の記録 `predict.meta.jsonl`（1 モデル 1 行）: `experiment`・`method`・`asof`・`table`・`layer`・`form`・`seed`・`calibration`（較正の版）・`features`（列数 35 ／ 124 ／ 95）・`input_fingerprint`・`commit`・`data_dir`・`train_rows`・`train_start`・`train_end`・`test_rows`・`dropped_symbols_nan_features`・`horizon`・`proxy_symbols`・`fitted`（較正の係数など、学んだ中身の控え）・`seconds`（`table` ／ `fit_predict`）・`log`。

**トレーダーの使い方**（`run_day.py` → `signals.py` → `plan.py`。数字は 2026-09-23 の本番【実測 ＝ §1】）

| 段 | 規則 | 記録 | 9/23 の例 |
| --- | --- | --- | --- |
| 合図 | `signals.py` が今日の日付の行だけ読み、トレーダーの `[[models]]` ごとに (買い%, 出口%) を集める。⚠ 無い銘柄は「合図なし」 | `signals.jsonl` ＝ `date`・`env`・`run_id`・`trader`・`symbol`・`buy`・`exit`・`inputs`〔[モデル名, 買い%, 出口%] の一覧〕・`position_before`・`test` | T1 T 50.93 ／ T2 5 銘柄とも 49.67 ／ T3 BAC 50.97 |
| 合成 | `combine`（`asis` ／ `mean` ／ `majority` ／ `unanimous`）。3 人とも `asis` ＝ 1 本のモデルの値がそのまま合図 | 同上 | — |
| 決める（`plan.decide`） | 持ち株 0 ＆ 買い% > θ → **買い** ／ 持ち株 1 ＆ 出口% > θ（⇔ 買い% < 100 − θ）→ **売り** ／ それ以外 → 持っていれば `hold`・いなければ `skip`。不等号は厳密 | `events.jsonl` の `hold` ／ `skip`（買い% ・ 出口% つき） | T1・T3（θ 50）は 5 銘柄とも買い ／ T2（θ 55）は 5 銘柄とも `skip` |
| 大きさ（`plan.size_intents`） | 1 銘柄の枠 ＝ min(予算 ÷ 銘柄数, 空き)。`shares` なら 枠 ÷ 気配の中値 を整数に切り捨て（1 株に満たなければ `too_small`）。空き ＝ 予算 − 建玉の取得原価 − 受渡し待ち。1 日の上限（全トレーダーで 1 つ）も見る。⚠ 買い% の高さで金額を変えない | `orders.jsonl` の `parts`〔トレーダー・株数・金額〕 | 枠 $60 ÷ 中値 25.335（T）→ 2 株 ／ 35.765（NKE）→ 1 株 |
| 注文（`execute.py`） | 1 注文 1 トレーダー（合算しない）。売りが先。成行。窓 15:45〜16:05 ET。① 全部 控え → dry-run → 発注 ／ ② 1 本ずつ約定を確かめて売買履歴へ | `orders.jsonl`（`quote_at_signal`・`fills`・`amounts`）・`state/<env>/<名前>.json` | 10 本 Filled・発注 → 約定 0.11〜0.16 秒 |
| 持ち株の出口 | 次の日以降、出口% > θ になった日に売る（1 日 1 回なので同日の往復は起きない ＝ §0-9 の前段） | 同上 | — |

⚠ **読み違えやすい所**: (1) 「完成したモデル」＝ 作り方（実験名 ＋ 数字の選び方）。重みは毎日変わる（`predict.meta` の `fitted` が控え）。(2) 入力の最後の行は**終値ではなく 15:50 の途中の足**なので、翌朝の公式終値で出し直すと買い% は少し動く（9/22: 15:14 ET の足と 16:01 ET の足で T1 PFE が 0.175 ずれた ＝ §0-13 の合否 ②）。(3) 出力は「上がる確率」の較正値で、大きさの予測ではない。売買の金額も買い% に比例しない。
...
**✅ 2026-09-22 の通しテスト（本番投入の前・⚠ 戻す必要なし）**【実測 11:33〜11:58 PDT】: cert の `test_a` を往復（買い 1665665 @ $25.08 ／ 売り 1665671 @ $25.07・どちらも `Filled`）→ 控え（`intent` → `submitted` → `done` が 2 組とも閉じた）・売買履歴（持ち株 0・履歴 4 行）・記録（`orders.jsonl` 2 行）が DB の上で通った ／ `reconcile.py --env cert show` ＝ 差 0・控えの未完 0 ／ `paper.py` → `daily.csv` 5 行 ／ 管理画面（3012）の 7 ページ ＋ `/api/live` が 200・`mode: real`・今日の起動が出る ／ 秘密の grep ＝ `.env` の 7 項目のどれも `livefs.py dump` に現れない・`Bearer` 0 件・口座番号 50 か所ともマスク済み（⚠ `refresh_token_rotated` 88 件はイベント名で値ではない）。
⚠ **429 は本番前にもう 1 度出た**: 買いの約定確認の照会が 429 → 待って取り直して `Filled` を読めた（`transitions` の `poll retry:429`）＝ 9/21 に直した穴（§1 の (1)）が効いている。
✅ **前営業日の注文は注文番号で読める**【実測】: cert の 1663195（`Filled` 1 株 $25.59）・1663301（`Filled` 1 株 $25.55）＝ 控えからの復元（§0-8 の段 1）が成立する。日をまたいだ未完を人が閉じる形に落とさなくてよい。

### 0-12. 13500T で予測にかかる時間（2026-09-22 夜。[プラン](../../plans/archive/predict-timing-13500t.md)）

**判定: 間に合う**（プラン §2-3 の線 ＝ 温まった状態の中央値が 60 秒以下）。13500T は予測を 3 本並列で **約 48 秒**で済ませた（titan は約 37 秒。約 1.3 倍かかる）。売買の判定は 2 台で 1 銘柄も変わらなかった。⚠ 判定したのは**予測の時間だけ**。13500T に売買を移すかどうかは TODO「13500T に管理画面と毎日の売買を動かす環境を作る」の「決めること」で、まだ決まっていない。

> この図の主張: 準備の見積り 120 秒のうち、予測に使えるのは 65 秒。13500T の予測はその中に 15 秒ほどの余裕を残して収まる。

```mermaid
flowchart LR
  A["日足の更新<br/>55 秒【実測 titan】"] --> B["予測 3 本を並列<br/>titan 37.6 秒 ／ 13500T 48.4 秒<br/>（線 60 秒・使える分 65 秒）"]
  B --> C["執行器: 発注を始める<br/>（刻限 15:58 ET）"]
```

同じ入力（titan の `data-live/` のスナップショット。63 銘柄・足は 2026-09-22 の途中まで・sha256 `863f95b7…`）で、同じスクリプト（`bench_predict.py --asof 2026-09-18,2026-09-22 --repeat 6`）を流した。1 回目は numba のキャッシュを空にした冷えた状態。

| 機械 | asof | 冷えた状態（1 回目） | 温まった状態（2〜6 回目の中央値） | 最小〜最大 |
| --- | --- | --- | --- | --- |
| titan | 2026-09-18 | 39.46 秒 | **37.41 秒** | 36.67〜37.92 |
| titan | 2026-09-22 | 37.62 秒 | **37.60 秒** | 37.31〜38.66 |
| 13500T | 2026-09-18 | 45.38 秒 | **47.93 秒** | 47.70〜49.78 |
| 13500T | 2026-09-22 | 49.50 秒 | **48.41 秒** | 47.68〜49.91 |

【実測】titan は 2026-09-22 20:01〜20:09 PDT、13500T は同日 20:37〜20:47 PDT。12 回とも `ok`。

| 機械 | CPU | メモリ | 動かし方 | 同時に動いていたもの |
| --- | --- | --- | --- | --- |
| titan | AMD Ryzen 9 7950X（32 論理・AVX-512 あり） | 47 GiB | WSL2・コンテナ無し・Python 3.12.3 | — |
| 13500T | Intel Core i5-13500T（20 論理・AVX-512 なし） | 30.6 GiB | Docker の使い捨てコンテナ（`python:3.12-slim`・Python 3.12.14・ネットワーク無し） | cloudflared だけ（計測の前は負荷 0.00） |

主要パッケージの版は 2 台とも同じ（numpy 2.4.2 ／ pandas 2.3.3 ／ sklearn 1.7.2 ／ lightgbm 4.7.0 ／ numba 0.67.0 ／ aeon 1.5.0）。⚠ **コンテナの有無と OS が違う ＝ 同じ土俵ではない**（差のうちどれだけがコンテナの分かは分けていない）。

内訳（温まった状態の中央値【実測】）: 差はほとんど**表づくり**に出る。

| 予測モデル | 表づくり titan ／ 13500T | 学習と予測 titan ／ 13500T |
| --- | --- | --- |
| `trade_own_ridge_a`（T1） | 32.5 ／ 41.8 秒 | 0.12 ／ 0.14 秒 |
| `trade_ownex_lgbm_a`（T2） | 34.7 ／ 44.0 秒 | 1.34 ／ 2.33 秒 |
| `trade_ownseq_ridge_a`（T3 QUANT（60日窓）） | 32.9 ／ 41.9 秒 | 2.69 ／ 4.04 秒 |

- **冷えた状態の上乗せは無い**（13500T の 1 回目は温まった状態より速いか 1 秒以内）＝ §2-3 の「30 秒以上遅ければ 1 回目で線を引き直す」に当たらない
- **売買の判定は 2 台で一致**（`--compare` の rc=0）。T1・T2 の買い% ／ 出口% は小数 6 桁まで同じ。T3 だけ最大 0.003 ずれたが、売買基準値をまたいだ銘柄は無い（⚠ asof 2026-09-22 の CAT は θ=50 と 0.0006 差だったが、動いたのは T1 で、T1 は差 0）
- ⚠ **入力の指紋（`input_fingerprint`）は 2 台で違う**（例 T1 `ac70e2df…` ／ `3da4bdb1…`。各機械の中では 12 回とも同じ）。Sx360（Intel i7-1355U・AVX-512 なし・コンテナ無し・PDT）で T1 を流すと、時刻帯を PDT にしても UTC にしても **13500T と同じ `3da4bdb1…`** になった【実測】＝ 時刻帯・コンテナ・Python の版のせいではない。⚠ 原因は **CPU の命令（titan だけ AVX-512 を持つ）で、表の浮動小数の最後の桁が変わること**【推測】（表そのものの差分はまだ取っていない）。⚠ **機械を替えると指紋は変わるものとして扱う**（指紋で「同じ入力か」を機械をまたいで確かめない）
...
| python の道 | ⚠ **`run-live.sh`・`live_update.sh` は `experiments/feature-discovery/.venv/bin/python` と `experiments/tastytrade-api-sample/.venv/bin/python` を決め打ち**で読む ＝ コンテナの中でこの 2 つの道が動くこと。推す形: image の venv を `/opt/venv` に作り、clone の 2 つの `.venv` を `/opt/venv` への symlink にする（どちらも git 管理外。host では宙に浮くが host では使わない）。⚠ **`.gitignore` の `.venv/` はディレクトリにしか当たらず symlink は未追跡に見える** ＝ 13500t の clone の `.git/info/exclude` に `.venv` を足す（無いと auto-update の ④「汚れている」で止まり続ける。2026-09-22 夜 Step 2-2 で踏んだ）。`dashboard/.venv` も同じ symlink |
...
| ③ 本番 dry-run | ✅ 2026-09-24 11:04 ET | 13500t のコンテナで `TT_ALLOW_PROD_DRY_RUN=1 … ./run-live.sh --traders T1,T2,T3 --mode dry-run -- --env prod --allow-prod-dry-run --ignore-window`（Sx360 の Claude が手で 1 回）。rc=0・106 秒。合図 15 → 意図 10 → 注文 10 が **全行 `mode: dry-run`・`submitted` 無し・`final_status: dry-run`・error 0**。買付余力の効果 **$35.65〜$57.44**（T 2 株 $51.38 ／ PFE 2 株 $57.44 ／ NKE 1 株 $35.65 ／ VZ 1 株 $47.43 ／ BAC 1 株 $55.95 × T1・T3）・dry-run 1 本 197〜304 ms・T2 は 5 銘柄とも `skip`・口座の持ち株は `positions_outside_ledger`・`state/` は作られていない。⚠ この日の 13500t の日足は 11:04 ET の途中の足なので、買い% は titan の 15:50 ET の値とは別物（比べない） |
| ④ 市場時間中の所要 | ✅ 2026-09-24（手動の回 ＋ cron の回） | **cron の回（無人）**: 15:40:01 ET に host cron が起こし `--wait` が 15:50:00 まで待つ → **日足の更新 56 秒**（15:50:56）→ **予測 3 本並列 44 秒**（15:51:40。表 38.0〜40.8 秒 ＋ fit 0.13 ／ 2.0 ／ 3.67 秒）→ 執行器 4 秒 → 15:51:44 `end rc=0` ＝ **15:50 から 100 秒 ≤ 120 秒**。手動の回（11:03〜11:05 ET）も 56 ＋ 45 ＝ 101 秒。titan の 9/23 は 55 ＋ 43 秒・9/24 の timer は 55 ＋ 44 秒。⚠ 9/24 15:50 ET は titan の timer（dry-run）と 13500t の cron（dry-run）が**同じ資格情報で同時に**ログイン・更新・予測・dry-run を通したが、**429 も認証の失敗も無し**（両方 rc=0）|
| ⑤ 秘密の grep | ✅ 2026-09-23 夜 ／ ✅ 2026-09-24 ③ の後にもう一度 | 9/23: `livefs.py dump .`（216 行・75,791 字）に `.env` の **7 項目とも 0 件**（本番 ・ sandbox の client secret 40 字・refresh token 557 ／ 586 字・client id 36 字・口座番号 8 桁・`TT_ENV`）・`eyJ` 0 件・`Bearer` 0 件。9/24（695 行。dry-run と管理画面の監視の記録が増えた後）: **秘密の 6 項目とも 0 件・`eyJ` 0・`Bearer` 0**。`TT_ENV`（＝ `cert`。環境名で秘密ではない）は 388 件当たるが、中身は管理画面の監視の記録の `"env": "cert"`・記録の道 `monitor/tastytrade-cert-<日付>.jsonl`・エラー文の `api.cert.tastyworks.com` ＝ 期待どおり |
...
⚠ ③ で気づいたこと（問題ではない）: `.env` を置くと 13500t の管理画面も **API の監視**（cert と prod の認証・口座・DXLink。`dashboard/data/monitor/`）を回し始める ＝ titan と 2 台で同じ資格情報の監視が並ぶ。9/24 08:00 PDT 時点で prod・cert とも認証 OK・`refresh_token_rotated` は 0 行（回っていない）・cert の `refresh_fail` 5 件と `api_error` 67 件は sandbox 側の 502 ／ 400 ／ 500（前からある挙動）。⚠ 切り替え（Phase 4）で titan の管理画面を止めるかは別に決める。


### 0-9. 1 日に複数回の発注に広げるか — ⚠ **広げない**（2026-09-20 利用者決定）

利用者の指示（2026-09-20）: **「1 日に複数回広げても意味がないので広げない。ただし 1 日に別銘柄を購入したり、買いと売りを同時に出すのは問題ない」**。
いったん「広げる」と決めた後、⚠ **利用者の指摘「そもそも分足などのデータを取っているのか。予測モデルが分足で学習しているのか」で調べ直して取り消した。**

| 調べたこと | 実測 2026-09-20 | 効いてくること |
| --- | --- | --- |
| 1 分足はあるか | **ある。63 銘柄 × 8,000 本**（`data/adjusted/m/`）。⚠ **8,000 本が提供元の上限 ≈ 30 日**で、手元は **8 営業日（2026-08-27〜09-08）だけ**。⚠ 過去に遡って取れない | 日中のモデルを学習・検証する材料が無い |
| 3 人のモデルは何で学習しているか | ⚠ **全部 日足**（`dataset = "daily"`・`bar_minutes = 1440`・`horizon = 1`） | **1 日に何回起こしても買い% は 1 日 1 回しか変わらない** ＝ 2 回目は同じ合図を見て `hold` になるだけ |
| 短い足は成立するか | ⚠ **成立しないと結論済み**（[feature-discovery.md](feature-discovery.md)。2026-09-08）: 「1 分足で 1〜5 分先を当てにいく設計は、特徴量を何にしても成立しない。符号を 100% 当てても 5 分先では往復 10bp のコストに届かない（8.88bp）」＝ **精度ではなく値動きの幅の問題**。土俵は 1 日先以降 | 日中の判断そのものに土俵が無い |

**決まったこと**

| 項目 | 決定 |
| --- | --- |
| 発注できる時間帯 | ⚠ **1 日 1 回・15:45〜16:05 ET のまま**（`run_day.WINDOW_START/END` を触らない） |
| 1 日に複数の銘柄を売買する | ✅ **可**（いまのとおり。1 回の起動で、銘柄ごとに買い・売りを決める） |
| 同じ日に買いと売りを同時に出す | ✅ **可**（いまのとおり。⚠ **売りが先**・1 注文 1 トレーダー ＝ §0-1）。⚠ 1 日 1 回なので、**同じ銘柄の同日往復は起きない**（C9 は当面もめない） |
| 再検討する条件 | **日中のホライズン（30 分〜数時間先）で成立するモデルができたとき**。⚠ その前に分足の入手（30 日の壁）を解く必要がある |
...
#### (a) 印 ＝ `experiments/live-trading/NOT_PRODUCTION`（git 管理外。`MODE` と同じ型の JSON）

> この図の主張: 印は**本番の発注（`--env prod --mode submit`）だけ**を、許可の 3 段より前で拒む。plan・dry-run・cert は通る ＝ 印のある titan は読むだけの写し・手順の稽古・cert の試験に使える。

```mermaid
flowchart LR
  S["run_day.py ／ sample.py"] --> M["MODE（sim なら rc=5）"]
  M --> N{"NOT_PRODUCTION がある<br/>かつ prod の発注 ?"}
  N -- "はい" --> R["拒否 rc=7<br/>events.jsonl に refused_not_production"]
  N -- "いいえ" --> P["許可の 3 段 → run.lock → HALT → 時間帯 → 認証（今までどおり）"]
```

| 項目 | 決め |
| --- | --- |
| 中身 | `{"machine": <置いた機械の hostname>, "since", "by", "reason"}`。読むときに今の hostname と突き合わせる |
| 拒むもの | **`--env prod` かつ `--mode submit`**（執行器 `run_day.py`）／ **本番の発注系の手順 4・5・5limit・6**（`sample.py`）。`TT_ALLOW_PROD_ORDERS=1` ＋ `--i-know-this-is-real-money` があっても拒む（**許可より印が先**）。rc=7・本物の `events.jsonl` に `refused_not_production`（`machine`・`host`・`matches_host`・`since`・`by`） |
| 拒まないもの | `--mode plan`・`dry-run`・`--env cert`・`reconcile.py`・管理画面（読むだけ） |
| 取り違え | 印の `machine` が今の hostname と違う（印を写した・ホスト名を変えた）→ **それでも拒む**が、メッセージ・記録・帯に「この機械の印ではない」と出る ＝ 13500t に titan の印が紛れ込んだら、起動しなかった理由がすぐ分かる（消すのは `notprod.py clear`）。壊れた印（JSON でない・`machine` が無い）も拒む ＝ 読めない印を「無い」と読まない |
| 操作 | `notprod.py status ／ set [--reason …] ／ clear`（⚠ CLI だけ・**置くのも外すのも利用者**）。`set` は自分の hostname を書く（他の機械の名前は書けない）。置く・外すは `mode.log` に 1 行（`not_production_set` ／ `not_production_cleared`） |
| 早見 | `run-live.sh` は submit ＋ `--env prod` ＋ 印 で 95 秒むだにせず rc=7（関門の本体は `run_day.py`） |
| 管理画面 | 全ページの最上部に灰の帯「本番の機械ではない」・`/api/*` に `not_production`（[dashboard.md §13-6](../dashboard.md)）。⚠ 表示だけ・公開面は読まない |
| 守り 2 枚 | 印（仕組み）＋ timer を止め `live.env` から許可を外す（運用）。⚠ 印は timer を止める代わりではない（timer が起きるたびに rc=7 の記録が増えるだけ） |

#### (b) 切り替えの手順（Phase 4。市場の外の日。⚠ 発注しない日に）

> この図の主張: 順は「titan を止める → 印 → 写す → 13500t で差 0 → 13500t を開ける」。逆にしない（13500t を先に開けると 2 台が同時に発注する日ができる）。

```mermaid
flowchart LR
  A["① titan: timer を止め<br/>live.env の許可を外す"] --> B["② titan: 印を置く<br/>notprod.py set"]
  B --> C["③ titan: 写し<br/>livefs.py backup ＋ sha256"]
  C --> D["④ Sx360 → 13500t へ運ぶ<br/>Phase 2 の DB は脇へ"]
  D --> E["⑤ 13500t: reconcile show 差 0<br/>印なし"]
  E --> F["⑥ 13500t: 留め金を外し<br/>live.env を submit"]
  F --> G["⑦ 翌営業日の cron を見る"]
```
...
| 順 | 何を | 結果 |
| ---: | --- | --- |
| ③ | `livefs.py backup` → 13500t 役の `live.sqlite` | 0.045 秒・整合性 ok・写しの sha256 `d426d113…` が転送後も `OK`・`stats` は元と同じ **115 本・1,602 行 ／ 丸ごと 6（前の中身 14）** |
| ⑤ | 13500t 役で `reconcile.py --env prod show`（`LT_MODE_DIR`・`LT_STATE_DIR`・`LT_OUT_DIR` を作業用の置き場へ） | 4 人・口座（9/23 の記録）− 売買履歴 ＝ **5 銘柄とも差 0**（T 4 ・ PFE 4 ・ NKE 2 ・ VZ 2 ・ BAC 2 ＝ T1 と T3 が半分ずつ）・控えの未完 0・rc=0 |
| ② | titan 役に `notprod.py set --reason 稽古` → `run_day.py --traders T1,T2,T3 --env prod --mode submit --ignore-window`（⚠ 許可も資格情報も付けない） | **rc=7**「この機械は本番の機械ではない」・`events.jsonl` に `refused_not_production`（`machine: titan`・`matches_host: true`）。同じ印のまま `--mode plan` は印で止まらず「資格情報が無い」まで進む（rc=1） |
| 戻し | `notprod.py clear` → `status` | 印なし。`mode.log` に `not_production_set` → `not_production_cleared` の 2 行 |
...
⚠ **20 営業日（Phase 6）のあいだは執行器（`run_day.py` ほか）を変えない**。コードを変える案は 10/20 の判定の後に回す。

**失敗の知らせ方**

> この図の主張: 売買の回（`run.sh trade`）には触らず、後ろに別の見張りを置く。機械が落ちた日は見張りも動かないので、外の側が「来ない」で知らせる。

```mermaid
flowchart LR
  T["run.sh trade<br/>15:40 ET"] --> L["logs/trade.log<br/>end rc=N"]
  L --> C["check.sh<br/>16:20 ET"]
  K["market_calendar<br/>休場・半日"] --> C
  C -- "終了コードだけ" --> H["外の死活監視<br/>AIL_HC_URL"]
  H -- "fail ／ 0 以外 ／ 来ない" --> M["利用者にメール"]
```
...
| 段 2 | 13500t の host cron に `check.sh`（g3plus-ops `trade-runner/`。平日 16:20 ET）。今日の `end rc=` を読み、`AIL_HC_URL`（`live.env`）へ `/<rc>`・行が無ければ `/fail`・休場日と半日立会は `/0` を送る。⚠ **送るのは日付・市場の区分・終了コードだけ**（口座番号・金額・銘柄・トークンは送らない）。⚠ **`run.sh` は変えない**（見張りが壊れても売買は止まらない） |
...
| 項目 | 決めたこと |
| --- | --- |
| 起動しなかった日 | §1 の表に 1 行「起動なし」（トレーダーは「全員」・注文 0・事象に理由）。⚠ **20 営業日は起動した営業日で数える**（起動しなかった日は数えず、判定のときに件数を別に書く ＝ 無人運転の柱） |
| 約定後の実際の手数料 | 10/20 の判定の後に `/accounts/{n}/transactions` を sandbox で確かめてから入れる。それまでは dry-run の見積り |
| 4 役への分け直し（A1〜A5・F23） | いまは分け直さない。再開は 1 日に何回も売買するモデルができたとき（D13 の再検討の条件と同じ）。そのときの見立て: 呼び名は 予測 ／ 判断 ／ 発注 ／ ビュワー（ディレクトリ名は変えない）・機械は全部 13500t |
| C10 更新の時刻がずれるモデル | 判断は取引時間帯に 1 回・その日の日付の予測だけを使う（いまの作り） |
| C11 入れ替え・編集 | 編集はしない。変えるときは新しい識別名の新しい人を作り、古い人は窓の中で持ち株を売り切ってから外す（売買履歴は残す） |
| C12 停止の単位 | 止めるのは全員（`HALT`）だけ。含み損 20% は警告だけ（§0-2）。1 人だけ止めるなら 13500t の `live.env` の `AIL_LIVE_TRADERS` から外す（利用者） |
| D13 注文の種類・気配 | 成行（`Market`・DAY）のまま・気配は 15:50 ET に取るまま（差 1 を素直に測れる。指値は約定しない日ができる） |
| G24 1 日に何度も売買するときの差 | いまは決めない（1 日 1 回に固定したため起きない） |

## 1. 記録（日次）

Phase 5-1・Phase 6 で埋める。1 日 1 行 × トレーダー。数字は全部【実測】。

⚠ **起動しなかった営業日も 1 行書く**（トレーダーは「全員」・注文 0・事象に「起動なし」と理由）。⚠ **20 営業日は起動した営業日で数える**（§0-15）。
...
| 2026-09-22 | `test_a`（cert・DB の作りの通しテスト。判定に混ぜない） | 2（買い 11:33 PDT ／ 売り 11:33 PDT。どちらも `--ignore-window`） | 2 ／ 2（買い 1665665 @ $25.08 ／ 売り 1665671 @ $25.07。起動から 10.4 ／ 4.0 秒） | 気配は 2 回とも bid 25.07 / ask 25.08 | ⚠ **買いの約定確認で 429** → 待って取り直し `Filled`（`transitions` の `poll retry:429`）。ほかの問題 0 | 往復 −$0.01・手数料 $0.001 ＋ $0.0012・口座 − 売買履歴 ＝ 0・控えの未完 0・秘密の grep は空。§0-11 の作り（DB）で通した |
| 2026-09-23 | `T1`（本番・規模 A・1 日目） | 5（買い T 2 ・ PFE 2 ・ NKE 1 ・ VZ 1 ・ BAC 1。発注 15:51:40〜43 ET・窓の中） | 5 ／ 5（508891490 ／ 508891504 ／ 508891511 ／ 508891521 ／ 508891531。発注 → 約定 0.11〜0.16 秒） | 5 本とも 0.00（約定 ＝ 中値: T 25.335 ／ PFE 28.095 ／ NKE 35.765 ／ VZ 46.575 ／ BAC 55.945） | なし（`skip`・`retry`・`over_budget`・`journal_unresolved` 無し。429 も無し） | 代金 $245.145・手数料 $0.007（dry-run の見積り）・予算 $300 の 81.7% を使用 |
| 2026-09-23 | `T2`（本番・規模 A・1 日目） | 0 | — | — | 5 銘柄とも買い% 49.67 < θ 55 で `skip`（`events.jsonl` に 5 行） | 5 銘柄の買い% が同点（仕様どおり ＝ 銘柄を選ばないモデル。§0-1） |
| 2026-09-23 | `T3`（本番・規模 A・1 日目） | 5（同上。発注 15:51:43〜46 ET） | 5 ／ 5（508891536 ／ 508891544 ／ 508891547 ／ 508891554 ／ 508891570。発注 → 約定 0.12〜0.15 秒） | T +0.83 ／ PFE 0.00 ／ NKE +0.67 ／ VZ +0.60 ／ BAC +1.79（中央値 +0.67。同じ気配で T1 の 1〜3 秒後に出た分が不利） | なし | 代金 $245.164・手数料 $0.007（見積り） |
...
⚠ **2026-09-22 に踏んだ落とし穴 1 つ（直した）— 本番投入が 9/23 へ 1 日ずれた理由**:

**`--wait` はブレーキではない。** `run-live.sh --wait` は「15:50 ET まで待つ」だけで、⚠ **既に過ぎていたら素通りする**。13:01 PDT（16:01 ET）に起こしたところ、待たずに準備（日足の更新 53 秒 ＋ 予測）に入った ＝ そのまま進めば **引け（16:00 ET）後の成行**を 10 本投げるところだった（利用者が準備の途中で中止したので、⚠ **本番の注文は 0 件・口座は動いていない**）。
→ `run-live.sh` に**引けに間に合うかのブレーキ**を足した。

| | |
| --- | --- |
| 掛かる条件 | `--mode submit` ＋ 今日の日付。⚠ `plan` ／ `dry-run` ／ 過去の日付 ／ 「--」の後ろに `--ignore-window` を書いた回は素通り |
| 止める線 | 「いま ＋ 準備の見積り」が**刻限**を過ぎるなら起動しない（rc=11） |
| 既定 | 準備 120 秒（`--skip-update` なら 60 秒。【実測】更新 53〜55 秒 ＋ 予測 37〜38 秒）／ 刻限 15:58 ET |
| 動かす | `AIL_PREP_SECONDS` ／ `AIL_SUBMIT_DEADLINE` |

⚠ **起動は 12:45 PDT（15:45 ET）**。12:50 起動では引けまで 8 分しかない。⚠ **12:56 PDT を過ぎるとブレーキが止める**（それが正しい）。

**`run-live.sh` が起動してすぐ見る 3 つ**（⚠ **排他と発注できる時間帯の本体は `run_day.py` の側**。ここは 95 秒むだにしないための早見）:

| # | 見るもの | 止まり方 |
| ---: | --- | --- |
| 1 | 機械がシミュレーションモードか（`MODE`） | rc=5。⚠ 0.03 秒で止まる【実測】。戻すのは `simctl.py mode real` |
| 2 | 別の執行器 ／ 運転手が動いているか（`run.lock`） | rc=6。⚠ **取れるかだけ見てすぐ放す**（持ち続けると後で起こす `run_day.py` が取れなくなる） |
| 3 | 引けに間に合うか | rc=11（上の表） |

⚠ **早見が壊れても実売買は止めない**（`run-live.sh` は確かめられなければ「進む」。本物の関門は `run_day.py` の rc=5 ／ `refused_lock_busy` の側にある）。
