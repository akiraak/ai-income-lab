...
#### (k) 自動で回せるもの（2026-09-20。利用者の指示「自動でテストを回せるものはある？」）
...
| 筋書きが効いた証拠 | 同上 | `out_of_window` 2（半日立会）・`journal_recovered` 1・`auth_failed` 1・`drawdown_warning` は `sim_b` だけ・止まる銘柄は BAC だけ・起動しなかった日がある |
