...
| **1 日に 2 つの窓** | 同上 | 同じ日に 2 回起きて、⚠ **二重に買わない**（1 回目で持ったら 2 回目は `hold`）。⚠ **執行器の窓は 15:45〜16:05 ET に固定**なので、窓はその中に 2 つ置く |
| ⚠ 窓の外は拒む | 同上 | 10:00 の窓は `out_of_window` で拒まれる ＝ **1 日に何度も売買する形（C9・D13）に進むときは、まず `run_day.WINDOW_START/END` を広げる必要がある**。この検査は広げたときに落ちて気づくための印 |
...
上の追加で `halted`・`too_small`（枠）・`out_of_window`（窓の外）が加わった。⚠ **`over_budget` は到達できない**（丸めが先に効く）ので、
