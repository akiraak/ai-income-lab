...
| 2〜4 | 認証・REST・ストリーミングの【実測】 | 🔶 **本番の読み取りは着手済み**（§0。認証・口座照会・気配が通った）。sandbox 側（手順 4・5・6）は**利用者の sandbox 資格情報待ち** |
...
| 観点 | 現況 | 根拠 |
| --- | :--: | --- |
| **A 認証の寿命** | 🔶 半分実測 | 交換は 3.4 秒で通り、`expires_in` 900 秒・refresh token は回らないことを確認（§0-3）。⚠ JWT は `exp - iat` が **954 秒**で応答値と 54 秒ずれる。残るのは「実際に 401 になる時刻」と「翌営業日に同じ refresh token が使えるか」 |
| **B 常駐** | ✅ | **本物の API で確認済み**（§0-3）。REST ＋ websocket 2 本だけで動き、常駐プロセスも GUI も要らない。WSL2 で追加の設定は不要だった |
| **C 現在値** | 🔶 半分実測 | **入金 0 の本口座で REST・DXLink とも取れた**（§0。2026-09-05）。残るのは**市場が開いている時間の遅延**の測定だけ（土曜は配信が無く測れない）。sandbox 側は【公表値】どおり全経路 502 |
| **D 発注の往復** | 🔶 半分実測 | **手順 4（dry-run → 指値 → 取消）は通った**（§0-3）。手順 5（約定 → 反対売買）は**市場時間でしか試せない**ことが分かった（時間外は成行が preflight で弾かれ、指値も約定しない） |
