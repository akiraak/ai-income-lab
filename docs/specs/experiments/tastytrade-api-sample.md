...
| 6 | 本番で 1 株 | ➡ **2026-09-17 に「トレーダー 3 人 … 実際に売買して記録を残す」（[プラン](../../plans/live-trading-three-models.md)）の Phase 5 へ取り込んだ**。利用者が発注を行う点は変わらない |

## 0. 実測 1 — 入金前の本番口座で気配は取れる（2026-09-05 14:09 ET）

⚠ **2026-09-17 に API の挙動をもう 1 つ実測した**: ⚠ **上場廃止銘柄は台帳（`/instruments/equities/{sym}`）には `active=False` で残るが、日足は 1 本も出ない**（10 本とも 0 本。対照の `AAPL` `SPY` は 3,013 本）。⚠ **記録は [universe-widening.md §11](universe-widening.md)**。
...
> ⚠ **2026-09-08 の実行で、この節の 2 点が覆った**（§0-4）。(1) JWT の「954 秒」は `exp − iat` の読み違いで、失効は `expires_in` どおり **900 秒**。
> (2) 手順 4・5 の可否は「市場が開いているか」だけでは決まらない。**開場中でも `Session offline` で拒否されることがある**。
> ⚠ この日の手順 4・5 の記録は `out/` に残っていない（9/5 12:10 PT 時点で空だった）。いま `out/` にある 9/5 の成功はすべて selftest のモック。

| 手順 | 結果 | 実測値 |
| --- | :--: | --- |
| 1 認証 | ✅ | 3,402 ms。`expires_in` **900 秒**（⚠ ここに書いていた「JWT の `exp − iat` は 954 秒」は読み違い。§0-4 で訂正）。refresh token は**回転しない**（応答に含まれない）。`openid` を付けたので `id_token` も返る |
...
### ⚠ `/orders/live` は「本日の注文」で、終わったものも混ざる

サーバの監視を本物の sandbox に繋いだ直後、管理画面の「働いている注文」に **7 件**が出た。中身は Filled 2・Cancelled 1・Rejected 4 で、
**働いている注文は 0 件**だった。tastytrade の `/accounts/{n}/orders/live` は「**その日の注文**」を返す仕様で、終端の状態も含む。

- 停止ボタン側は最初から `Received / Routed / In Flight / Live / Contingent` で絞っていたが、**監視側が絞っていなかった**（2026-09-08 に修正）
- 直したあとは「働いている注文 0 · 本日の注文 7（Filled 2 / Cancelled 1 / Rejected 4）」と出る
- ⚠ **自動売買を書くときも同じ罠を踏む。**「`/orders/live` の件数 ＝ 未約定の注文数」ではない
...
## 0-5. 実測 5 — 着金後の資金の見え方（2026-09-16 16:48 ET、読み取りのみ）

§5 に残っていた「着金後の資金の見え方（保留期間・引き出し制限）」を、着金後の `--step probe`（発注はしない）で埋めた。
⚠ **着金の日付そのものは特定できない。** 着金前の最後の記録が 9/5、着金後の最初の記録が 9/16 で、間に残高を取った記録が無い
（9/7 は Labor Day なので、着金は 9/8〜9/16 の 7 営業日のどこか）。

| 項目 | 着金前（§0、2026-09-05） | 着金後（2026-09-16） | 読み |
| --- | ---: | ---: | --- |
| `cash-balance` | 0.0 | **1000.0** | ✅ **着金した** |
| `pending-cash` | 1000.0（Credit） | **0.0** | ✅ 保留が消えた（`pending-cash-effect` も空） |
| `net-liquidating-value` | 0.0 | **1000.0** | |
| `cash-settle-balance` | — | 1000.0 | |
| `equity-buying-power` / `derivative-buying-power` | 1000.0 / 1000.0 | 1000.0 / 1000.0 | ⚠ **着金で 1 円も増えていない。** `pending-cash` の段階で既に与信されていた（§0-2 の結論のとおり） |
| `available-trading-funds` | 0.0 | **0.0** | ⚠ **着金しても 0.0 のまま。** この項目は買付余力の指標として読めない（§0-2 で「効いていない」と分かっていたものが、着金後も動かないことで裏付いた） |
| `cash-available-to-withdraw` | 0.0 | **0.0** | ⚠ **まだ引き出せない。** ACH の引き出し保留は 9/16 時点でも続いている。保留の長さは着金日が分からないので測れない（最短でも「着金〜9/16」） |
| `day-trading-buying-power` / `maintenance-requirement` | — | 0.0 / 0.0 | 現物の現金口座扱い（`equities-margin-calculation-type: Cash Secured Margin`）と整合 |
| 取引状態 | — | `is-frozen` false / `is-in-margin-call` false / `is-closed` false / `is-pattern-day-trader` false | 発注を妨げる状態は無い |

同じプローブで取れた気配の側（§0 の再確認）:

| 測ったもの | 結果 |
| --- | --- |
| REST `/market-data/by-type` SPY | ✅ bid 754.07 / ask 754.10 / last 754.10（`updated-at` 2026-09-16T20:48:19Z ＝ 16:48 ET、引け後の値） |
| `GET /api-quote-tokens` | ✅ `level: "api"`、`dxlink-url` は §0 と同じ `wss://tasty-openapi-dxlink-md-ws.dxfeed.com/realtime` |
| DXLink（15 秒） | ✅ AUTHORIZED 520ms → CHANNEL_OPENED 597ms → FEED_CONFIG 751ms。Trade 1 ＋ Quote を含む **6 イベント**（引け後なので少ない） |

**Phase 6（本番で 1 株）への含み**: ⚠ **着金を待った意味は資金面では無かった**（買付余力は着金前から 1000.0 で、1 株は通る — §0-2）。
変わったのは「引き出しがまだできない」ことが分かった点だけで、⚠ **1 株の往復を試すのに障害は無い**。実行するのは利用者（CLAUDE.md の例外）。
...
| 観点 | 現況 | 根拠 |
| --- | :--: | --- |
| **A 認証の寿命** | ⏳ | `expires_in` 900 秒・refresh token は回らない。**2026-09-08 に 920 秒待って 401 を確認**し、**900 秒で失効**すると確定（§0-4）。⚠ 以前書いた「954 秒」は `exp − iat` の読み違いで、`iat` は grant 作成時刻の固定値だった。9/5 の refresh token が 3 日後もそのまま使えることも確認。⚠ 残るのは**営業日を 2 日跨ぐ交換**だけ（9/5 は土曜なので実測の営業日はまだ 1 日）。サーバの監視を回し続ければ翌営業日に埋まる |
| **B 常駐** | ✅ | **本物の API で確認済み**（§0-3）。REST ＋ websocket 2 本だけで動き、常駐プロセスも GUI も要らない。WSL2 で追加の設定は不要だった |
| **C 現在値** | ✅ | **市場時間内に測れた**（§0-4。2026-09-08 14:25〜14:51 ET）。遅延は**サーバの時計で −0.12 秒**（12 回の中央値、ばらつき 0.24 秒）。⚠ こちらの時計で測ると WSL2 のずれで ±1 秒揺れるので、`Date` ヘッダで補正した値を使う。DXLink は 60 秒で 229 イベント。sandbox 側は【公表値】どおり全経路 502 |
...
| **E レート制限** | ✅ | **60 回/分・10 回/秒（30 連射）とも 429 は出ず**、中央値 132〜136 ms（§0-3、9/5 モック）。**2026-09-08 に本物の cert でも再現**（429 なし、中央値 133.9 ms）。成立条件「1 分に 60 回の照会が通る」を満たす |
...
**2026-09-08 時点で 6 観点のうち 5 つが ✅。残るのは A だけで、それも「あと 1 営業日置く」だけ。**

> この図の主張: 観点は「何を待つか」で 3 つに分かれる。文書で決まるもの、1 回の実行で決まるもの、**時間を置かないと決まらないもの**。A だけが 3 つ目に残った。

```mermaid
flowchart LR
  DOC["公式文書 ＋ OpenAPI"] --> B["B 常駐 ✅"]
  DOC --> F["F SDK ✅"]
  RUN["市場時間に 1 回回す<br/>2026-09-08"] --> C["C 現在値 ✅<br/>遅延 −0.12 秒"]
  RUN --> D2["D 往復 ✅<br/>指値・取消・約定・反対売買"]
  RUN --> E["E 制限 ✅<br/>60 回/分で 429 なし"]
  WAIT["時間を置く<br/>サーバの監視で自動"] --> A["A 認証 ⏳<br/>営業日を 2 日跨ぐ交換"]
```

**判定は管理画面が記録から自動で組み立てる**（`/judge`。[dashboard.md §5](../dashboard.md)）。手で書くのはこの表ではなく、根拠の記録（`out/*.jsonl`）と監視ログ（`data/monitor/*.jsonl`）の側。
⚠ **モックの記録（`mock: true`）は判定から除外される。** 9/5 の `out/` に残る手順 4・5 の成功はすべて selftest のモックで、本物での往復は 2026-09-08 が初めてだった。
...
| 11 | 公式文書「sandbox は $3 未満の指値が即約定」 | **市場が開いているときだけ**。土曜に出した指値 $2.00 は `Received` のまま約定しなかった。成行に至っては `tif_no_after_hours_opening_market_orders` で発注自体ができない。⚠ **さらに、市場が開いていても約定するとは限らない**（#13） | 補足候補（条件が抜けている） |
| 12 | 公式文書「access token は 15 分」 | ✅ **決着（2026-09-08）。`expires_in` 900 秒が正しい**（920 秒待って 401）。⚠ 以前ここに書いた「JWT の `exp − iat` は 954 秒」は読み違いで、**`iat` は grant を作った時刻で固定**され、トークンを発行しても動かない。寿命を測るなら `exp` − 発行時刻を使う | **自分の記述の訂正** |
| 13 | — | ⚠ **cert は市場時間内でも `Session offline` で注文を拒否することがある**（2026-09-08 14:24。同時刻の `market-time` は `Open`、買付余力も十分）。25 分後には通った。**一時的な状態**（§0-4） | **新規。自動売買の設計に効く**（拒否を「注文が悪い」と読まず再送する） |
| 14 | — | ⚠ **`/accounts/{n}/orders/live` は「その日の注文」を返し、Filled / Cancelled / Rejected も混ざる**（2026-09-08）。「働いている注文」を数えるには `Received / Routed / In Flight / Live / Contingent` で絞る必要がある | **新規。管理画面の不具合として実際に踏んだ**（§0-4） |
| 15 | — | ⚠ **気配の遅延は開発機（WSL2）の時計では測れない**。同じ応答の `Date`（サーバの時計）で補正すると −0.12 秒で安定するのに、素で測ると 1.8 秒揺れる（2026-09-08、12 回。§0-4） | **新規。計測の方法論** |

## 5. 未実測とその理由

2026-09-08 の実行で 5 つが埋まった（下の「済んだもの」）。2026-09-16 に着金後の資金の見え方が埋まった（§0-5）。残るのは 3 つ。

| 残る項目 | 理由 | いつ埋まるか |
| --- | --- | --- |
| **営業日を 2 日跨ぐ refresh**（観点 A の本体） | 実測できた営業日がまだ 1 日（9/5 は土曜で営業日に数えない） | **サーバの監視で自動**。2026-09-08 19:23 UTC から g3plus で cert に繋いで回している。翌営業日に入れば判定 A が ✅ になる |
| 429 が出る水準 | 60 回/分・10 回/秒では出なかった。どこまで上げると出るかは未探索 | ⚠ 発注は連打しない前提で、必要になったら |
| 約定価格と気配の差（約定品質） | 本番の入金と発注が要る（利用者が行う） | Phase 6 |

**2026-09-08 に済んだもの**: 認証の寿命（900 秒で 401）／ 3 日前の refresh token の再利用と 24 時間リセット後の grant ／ 気配の遅延（市場時間内、サーバの時計で −0.12 秒）／ 手順 5 の約定・建玉・反対売買 ／ `Live` を経由する遷移（`Routed → In Flight → Filled`、買い 6.8 秒）

**2026-09-16 に済んだもの**: 着金後の資金の見え方（§0-5。`cash-balance` 1000.0・`pending-cash` 0.0。⚠ `cash-available-to-withdraw` は 0.0 のままで、⚠ 買付余力は着金で増えていない）

## 6. 出典

2026-09-08 の実測は本書 §0-4。参照した API の応答（`market-time/sessions/current`、`orders/{id}` の `reject-reason`、`Date` ヘッダ）は記録（`out/*.jsonl`）に残っている。以下は【公表値】の出典（すべて 2026-09-05 取得）。
