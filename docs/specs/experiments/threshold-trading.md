...
> ⚠ **訂正（2026-09-12）: この 1 行は誤りだった。**【実測 2026-09-12。[buy-pct-width-collapse.md §2](buy-pct-width-collapse.md)】
> ⚠ **`a ≈ 0` の主因は較正の不具合である** — `LogisticRegression(lbfgs)` の `tol=1e-4` が予測のスケールに依存し、
> ⚠ **1 日リターンの尺度（σ ≈ 0.0017）では 1 反復で「収束」と判定されて `a` が初期値 0 から動かない。**
> ⚠ **同じ標本・同じ目的関数で標準化してから解き直すと対数尤度が上がる**（25 行中 22 行）ので、
> ⚠ **「情報が無い」のではなく「解けていない」。**
> ⚠ **根拠にした leak 対照は反証になっていない** — leak でも同じ不具合は起きており、
> ⚠ **信号が強すぎて幅 100 点に達してしまうだけだった**（同じ leak 実行の中で、leak 列を選ばない乱択だけが本番と同じ形で潰れている）。
> ⚠ **§0 以下の数字はこの不具合を含んだまま出たものである。** ⚠ **数字は 1 つも書き換えていない**（処置は Phase 2 の裁定待ち）。
