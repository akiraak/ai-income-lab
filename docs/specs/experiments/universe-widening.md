...
| C 上場廃止銘柄を入れる | ⚠ **いまのデータ源では 0 本**（broker の配信は上場中の銘柄だけ）【推測】 | ⚠ **12-1 を直接潰せる唯一の道。データ源の調査が先** |
...
| 4 | ⚠ **上場廃止銘柄を入れる** | ＋数千 | ⚠ **いまのデータ源では取れない**【推測】 | ⚠ **12-1 を直接潰す唯一の道** | データ源の調査が先。⚠ **有償なら CLAUDE.md の方針で提案・試算まで** |
...
⚠ **段 4 はデータの問題である。** broker の配信は上場中の銘柄だけなので、⚠ **廃止銘柄の日足はここからは出ない**【推測】。
⚠ **確かめるのは安い** — tastytrade の instruments で廃止銘柄を引けるか試すだけで、⚠ **これは「API の挙動」なので CLAUDE.md の例外に当たり実測してよい。**
⚠ **2026-09-17 に実測し、この【推測】は当たった**（[§11](#11-段-4-の実測--廃止銘柄の日足は配信から出ない実測-2026-09-17)。10 本とも 0 本）。
...
| 4 廃止銘柄 | ⚠ **配信からは組めない**（2026-09-17 に実測。[§11](#11-段-4-の実測--廃止銘柄の日足は配信から出ない実測-2026-09-17)） | ⚠ **台帳には `active=False` で残るが日足は 0 本。** 次は有償データ（point-in-time）の費用の試算まで |
...
| 3 | ⚠ **生存バイアスは消えない。むしろ増える**（いまの指数構成銘柄を過去に当てはめる） | 12-1。⚠ **段 4 が解けるまで残る限界として書く** |
...
## 11. 段 4 の実測 — ⚠ 廃止銘柄の日足は配信から出ない【実測 2026-09-17】

プラン: [delisted-symbols.md](../../plans/archive/delisted-symbols.md) ／ コード: `experiments/feature-discovery/cli/delisted.py`（⚠ **読み取りだけ**）
⚠ **銘柄 12 本と判定の規則は、回す前にプラン §2 で決めた。** ⚠ **廃止の事由と日付は【推測】**で、判定には使っていない。

> この図の主張: ⚠ **台帳には残るが、値動きは出ない。** ⚠ **段 4 は配信からは組めない。**

```mermaid
flowchart LR
  D["上場廃止の 10 本<br/>SIVB・FRC・TWTR ほか"] --> A["台帳（instruments）<br/>✅ 行は残る・active=False"]
  D --> B["日足（DXLink Candle）<br/>⚠ 10 本とも 0 本"]
  C["生きている対照<br/>AAPL・SPY"] --> E["✅ 3,013 本<br/>2014-09-22 〜 2026-09-17"]
  B --> V{"判定"}
  E --> V
  V --> R["❌ 取れない<br/>→ 有償データの試算まで"]
```

### 11-1. 結果【実測 2026-09-17 11:11 PDT】

| 枠 | 銘柄 | 台帳（`/instruments/equities/{sym}`） | ⚠ 日足 |
| --- | --- | --- | ---: |
| 破綻 | `SIVB` `FRC` | ✅ あり（`active=False`。SVB FINANCIAL GROUP ／ First Republic Bank） | ⚠ **0 本** |
| 非公開化 | `TWTR` | ✅ あり（`active=False`。TWITTER INC） | ⚠ **0 本** |
| 買収 | `ATVI` `VMW` `SPLK` `PXD` `XLNX` `CERN` `ABMD` | ✅ あり（`active=False`。⚠ **5 本は `listed-market` が `OTC` に付け替わっている**） | ⚠ **0 本** |
| ⚠ 改名の対照 | `FB` | ⚠ **別の商品になっていた**（下の 11-2） | 2,250 本 |
| ⚠ 生きている対照 | `AAPL` `SPY` | ✅ あり（`active=True`） | ✅ **3,013 本**（2014-09-22 〜 2026-09-17） |

⚠ **対照が返っているので、0 本は「測れていない」ではなく「出ない」である**（プラン §2-3 の前提を満たした）。
⚠ **台帳に行が残るのは収穫**: ⚠ **ティッカーが廃止済みかどうかは API から分かる**（`active` と `listed-market`）。値動きだけが無い。

### 11-2. ⚠ 思わぬ発見 — ティッカーの使い回しで、2 社の履歴が 1 本に繋がる

⚠ **`FB` を「改名の対照」として入れたら、いまの `FB` は ProShares S&P 500 Dynamic Buffer ETF だった**（BATS・`is-illiquid=True`）。
⚠ **ところが返ってきた日足は、Facebook の履歴と ETF の履歴が 1 本に繋がっていた。**

| 期間 | 本数 | 終値の幅 | 出来高の中央値 | 中身 |
| --- | ---: | --- | ---: | --- |
| 2014-09-22 〜 2022-06-08 | 1,942 | 72.63 〜 382.18 | 約 2,000 万株 | ⚠ **Facebook**（2022-06 に META へ改名） |
| 2022-06-09 〜 2024-12-31 | ⚠ **0** | — | — | ⚠ **空白**（約 3 年） |
| 2025-01-02 〜 2026-09-16 | 308 | 39.91 〜 45.40 | ⚠ **328〜1,442 株** | ProShares の ETF |

⚠ **これは段 4 より広く効く罠である。** ⚠ **銘柄集合はティッカーで書いてあるので、使い回されたティッカーを入れると、
別会社の履歴が 1 本の系列として学習される**（しかも出来高は 4 桁違う）。

**手元のデータを調べた【実測 2026-09-17】**: 保存済み 86 銘柄のうち ⚠ **30 日を超える空白があるのは `VXX` の 1 本だけ**
（2019-01-30 → 2019-05-02 の 92 日。終値は 153.08 → 106.96）。⚠ **iPath の VIX ETN が満期を迎え、後継が同じティッカーを継いだ時期と一致する**【推測】。
✅ **`VXX` はどの銘柄集合にも入っていない**（`lowcorr` の `equity_like` にあるだけで、売買の対象 11 本にも `us63` / `us70` / `us74` / `midcap48` にも無い）
⚠ **＝ 既存の結果は影響を受けていない。**

**手当て**: ⚠ **足の検査に「30 日を超える空白」を警告として足した**（`ail/data/check.py` の `gap_over_30d`。⚠ **止めない**
— 上場廃止・取引停止でも同じ形になるので、⚠ **数えて人が見る**）。⚠ **調整後の 86 銘柄で 1 件 / 1 銘柄（`VXX`）**【実測】。

### 11-3. 判定

| 問い | 答え |
| --- | --- |
| ⚠ **廃止銘柄の日足は配信から取れるか** | ❌ **取れない。** 10 本とも 0 本（対照は 3,013 本）。⚠ **プラン §2-3 の「取れない」に当たる** |
| 段 4（生存バイアスを消す）は進められるか | ⚠ **配信からは組めない。** ⚠ **次は有償データ（point-in-time の銘柄集合 ＋ 廃止銘柄の株価）の費用の試算まで**（金銭が発生するので提案・試算で止める。2026-08-27 の方針） |
| 台帳の照会は無駄だったか | ⚠ **無駄ではない。** ⚠ **`active=False` で「このティッカーはもう無い」ことは API から分かる**ので、銘柄集合の点検には使える |
| ⚠ **副産物** | ⚠ **ティッカーの使い回しで 2 社の履歴が 1 本に繋がる**（`FB`）。⚠ **検査に空白の警告を足した。** 手元では `VXX` 1 本だけで、どの実験にも入っていない |
| 限界 | ⚠ **試したのは素のティッカーだけ**（`ail/data/sources/tastytrade.py` と同じ購読の書き方）。⚠ **廃止銘柄を別の記法で引ける可能性は否定していない。** ⚠ **廃止の日付は【推測】**（判定には使っていない） |

⚠ **資金は動かしていない**（2026-08-27 の方針）。⚠ **発注系の経路には触れていない**（読み取りの API 呼び出しだけ。CLAUDE.md の 2026-09-05 の例外 ＝ API の挙動の実測に当たる）。
