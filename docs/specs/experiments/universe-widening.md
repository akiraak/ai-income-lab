...
⚠ **2026-09-17 に Sx360 で同じことを独立に測り、同じ結論になった**（[§11](#11-2026-09-17-の独立した測り直し--同じ結論と-ティッカー使い回しの発見)。⚠ **そのとき別の罠が 1 つ見つかった**）。
...
## 11. 2026-09-17 の独立した測り直し — ⚠ 同じ結論と、ティッカー使い回しの発見

⚠ **本線の記録は [universe-delisted.md](universe-delisted.md)**（titan・2026-09-16）。
⚠ **Sx360 が titan の commit を取り込む前に同じことを独立に測っていた**ので、⚠ **重複しない部分だけをここに残す。**
⚠ **Sx360 側のプランと道具（`cli/delisted.py`）は重複なので消した。** 測ったのは 2026-09-17 11:11 PDT・読み取りだけ。

| | titan（2026-09-16・本線） | Sx360（2026-09-17） |
| --- | --- | --- |
| 廃止銘柄 | 6 本とも日足 **0 本** | ⚠ **10 本とも 0 本**（`SIVB` `FRC` `TWTR` `ATVI` `VMW` `SPLK` `PXD` `XLNX` `CERN` `ABMD`） |
| 台帳（instruments） | ✅ `active=false`・CUSIP・最後の上場市場 | ✅ 同じ（買収の 5 本は `listed-market` が `OTC` に付け替わっていた） |
| 対照 | `AAPL` `SPY` で 8,001 本 | `AAPL` `SPY` で 3,013 本（⚠ **こちらは 12 年ぶんしか要求していない**） |
| 結論 | ⚠ **無償の経路では生存バイアスを消せない** | ⚠ **同じ** |

⚠ **2 台で独立に測って同じ結論になった**（銘柄も本数も違うのに）。⚠ **道具は titan の `delisted_probe.py` を正とする。**

### 11-1. ⚠ 思わぬ発見 — ティッカーの使い回しで、2 社の履歴が 1 本に繋がる

⚠ **「改名の対照」として `FB` を入れたら、いまの `FB` は ProShares S&P 500 Dynamic Buffer ETF だった**（BATS・`is-illiquid=True`）。
⚠ **ところが返ってきた日足は、Facebook の履歴と ETF の履歴が 1 本に繋がっていた。**

| `FB` の期間 | 本数 | 終値の幅 | 出来高の中央値 | 中身 |
| --- | ---: | --- | ---: | --- |
...
⚠ **銘柄集合はティッカーで書いてあるので、使い回されたティッカーを入れると別会社の履歴が 1 本の系列として学習される**（出来高は 4 桁違う）。
⚠ **これは段 4（廃止銘柄）とは別の、いま使っているデータに効く罠である。**

**手元のデータ【実測 2026-09-17】**: 保存済み 86 銘柄のうち ⚠ **30 日を超える空白があるのは `VXX` の 1 本だけ**
（2019-01-30 → 2019-05-02 の 92 日・終値 153.08 → 106.96。⚠ **iPath の VIX ETN が満期を迎え、後継が同じティッカーを継いだ時期と一致する**【推測】）。
✅ **`VXX` はどの銘柄集合にも入っていない**（`lowcorr` の `equity_like` にあるだけ）⚠ **＝ 既存の結果は影響を受けていない。**

**手当て**: ⚠ **足の検査に `gap_over_30d`（30 日を超える空白）を警告として足した**（`ail/data/check.py`。⚠ **止めない** —
上場廃止・取引停止でも同じ形になるので、⚠ **数えて人が見る**）。⚠ **調整後 86 銘柄での該当は 1 件 / 1 銘柄（`VXX`）**【実測】。
