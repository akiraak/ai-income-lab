...
| B14 | AI 開発者向けの評価・RLHF・専門データ作成の受託 | L1 | P3 | ゼロ | 〜3ヶ月 | 発注元の資本関係一つでタスクが消える（2025 年の Outlier の枯渇）。業務委託で稼働が保証されない | 2026-08-26 追加。I7 調査で最も金銭が動いていた経路。⚠ 業務委託（米国で誤分類訴訟） |
...
| D7 | 既存コンテンツの AI 学習許諾（声・映像・写真・書籍） | L2 | P6 | ゼロ | 〜3ヶ月 | 在庫が無いのに新規制作すると単価（画像はセント未満/枚）に見合わない | 2026-08-26 追加。集約サービス経由（音声・映像・画像ライブラリ）。副: 9 資源。⚠ 声（ElevenLabs）は**日本から受取可・成立**（時間あたり $96〜1,280。ただし有料プラン $60/年が必須で、日本語話者の実例は未発見）。動画（Troveo）は条件が NDA 下で**保留**。写真は**不成立**（[d7-content-licensing.md](../experiments/d7-content-licensing.md) 2026-08-27） |
...
| E8 | 保有資産の譲渡益（値上がり益）を繰り返し実現する運用 | L3 | P6 | 小〜大 | 〜1年 | 含み益を所得と錯覚し、税と再投資先を考えずに売る。短期売買に落ちると L1 になる | 2026-08-26 追加（異議 D、Haig–Simons）。単発の不用品処分は体系外。⚠ 税務（譲渡所得） |
...
| I7 | 独自データセットの提供・販売 | L2 | P3 | ゼロ〜中 | 〜1年 | データの権利処理を怠り、公開後に使えなくなる | ⚠ 著作権、個人情報保護法、規約。個人の入口は B14（受託）と D7（許諾）に分かれる（[i7-dataset.md](../experiments/i7-dataset.md) 2026-08-26） |
| I8 | 計算資源・回線の貸出（GPU・ホスティング） | L3 | P5 | 中〜大 | 3年〜 | 電力・冷却・陳腐化を織り込まず、償却前で赤字になる | 両端が機械＝ P5。⚠ 個人の短期回収は不成立。単価がハード価格の 2〜3 年回収前提で設計されており、GPU の陳腐化期間と重なる（[i8-gpu-rental.md](../experiments/i8-gpu-rental.md) 2026-08-27） |
