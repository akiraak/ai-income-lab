...
体系の全体表は狩猟採集の行を「分配・贈与」と書いており、こちらは人類学と整合している。
食い違っているのは一本線の図と仮説 2 の文言だけなので、修正は軽い（**異議 A**）。
...
| --- | --- | --- | --- | --- | --- |
| B | AI 差分の判定が「曝露」止まりで、「実際に置き換わったか」を見ていない | Humlum & Vestergaard (2025); Brynjolfsson, Chandar & Chen (2025) | [ai-delta.md](ai-delta.md) 全 61 件 | 判定に「置換の実証: 有／無／未」の列を足す。消滅 7 件のうち実証があるのは B6・C1 のみと明記する | 中 |
...
異議 A〜D は体系の修正候補として [TODO.md](../../../TODO.md) に起票した。E〜G は注記で済むため、次に該当ファイルを触るときに反映する。
