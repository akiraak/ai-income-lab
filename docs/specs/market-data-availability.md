...
2026-09-08 に実測した（[e8-signal-methods §2-1 データ経路](experiments/e8-signal-methods/data-routes.md)）。

| 経路 | 取れたもの【実測 2026-09-08】 |
| --- | --- |
| tastytrade DXLink `Candle`（本番の資格情報） | **個別銘柄の日足 8,003 本・1993-10-31 から**（SPY）。1 分足は**約 8,000 本 = 6 週間**で頭打ち。8 銘柄の日足 10 年を 1 セッションで同時取得できた |

⚠ **「登録が要る＝【公表値】止まり」は本プロジェクトの方針から来る線引き**であって、データが薄いという意味ではない。
⚠ **既に開いている口座（tastytrade）は方針の例外の内側**なので、そこからは L2 の長期履歴が取れる。
