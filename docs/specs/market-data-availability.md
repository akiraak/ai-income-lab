...
**96 件のうち 49 件は tick まで取れ、14 件は時系列にならない。**残りはその中間に散る。
...
| **L5** データなし | **14** | 括り 3・相対で記録が残らないもの 11 |
| **未確認** | 3 | MYGA・トークン化国債・StockX |
| | **96** | |
...
1. ⚠ **無償かつ無登録で tick が取れるのは、暗号資産と予測市場だけだった。**米国株は無償でも必ずアカウント登録を要求する（§3）。
2. ⚠ **「板がある」と「過去データがある」はずれる。**§14 で板ありとした 56 件のうち、L4 以下に落ちるものがある（SREC・NFT・トークン化不動産・Masterworks）。逆に板が無くても参照値なら日次で取れるものがある（現物金・国債利回り）。
...
| **S4** 相対・ディーラー | 6 | 1 | | 1 | 1 | 1 | 1 | 1 |
| **S5** 暗号資産・オンチェーン | 5 | 1 | 1 | | | 2 | | 1 |
| **S6** 現物マーケットプレイス | 11 | 1 | 1 | 2 | 2 | 1 | 3 | 1 |
| **S7** 相対・非公開 | 18 | | | 3 | 4 | 4 | **7** | |
| **合計** | **96** | **49** | **2** | **9** | **11** | **8** | **14** | **3** |
...
| # | 経路 | 確認できたこと |
| --- | --- | --- |
| 1 | FRED `fredgraph.csv` | API キー無しで CSV。`DGS30` 12,925 行 1977-02-15〜、`DFII10` 6,173 行 2003-01-02〜、`NASDAQNQH2O` 409 点 2018-10-31〜2026-08-26 |
...
| 対象 | 理由 |
| --- | --- |
| **Stooq** | 第三者記事は「無登録で日足・5 分足の CSV が取れる」と書くが、2026-09-01 時点でトップページが **JavaScript の proof-of-work による bot 検証**を要求した。規約本文にも到達できなかったため、**データ用エンドポイントは叩いていない**。表では「未確認」として扱う |
...
| 15 | ブローカード CD | CUSIP（銘柄ごと） | S3 | 無し | — | **L5** | — | ⚠ TRACE 対象外。二次流通の値段は各社の在庫画面のみで、時系列の公開源を確認できず |
...
| 2 | 変額・インデックス年金 | 無し | S4 | 無し | — | **L5** | — | ⚠ 契約ごとのサブアカウント基準価額。横断の公開時系列を確認できず |
...
| 3 | 住宅ローン債権 | ローン番号 | S7 | 無し | — | **L5** | — | ⚠ 成約価格の公開を確認できず |
...
| 6 | オルタナ総合（YieldStreet） | 無し | S7 | 無し | — | **L5** | — | ⚠ 個別案件のみ。横断の公開時系列を確認できず |
...
| 3 | 返品在庫のロット・倉庫競売 | ロット ID | S6 | 無し | — | **L5** | — | ⚠ 公式 API・落札履歴の公開を確認できず |
...
| 8 | 貸株（FPSL） | 無し（銘柄の借株レート） | S7 | 無し | — | **L5** | — | ⚠ 借株レートは証券会社の顧客画面のみ。公開の時系列を確認できず |
| 9 | 貸付・ソーシャルレンディング | 無し | S7 | 無し | — | **L5** | — | ⚠ 各社の実績開示のみ。横断の公開時系列を確認できず |
...
| **D6** | Stooq | T | L1 5 分足（第三者記事） | 未確認 | 無償とされる | ⚠ 不明 | ⚠ **2026-09-01 時点でトップページが JS の proof-of-work bot 検証を要求。規約本文に到達できず、データ取得は行っていない** | 【未確認】 |
...
⚠ **S1 の 45 件は、無償の経路がすべて「要登録」に着地した。**無登録で取れるとされる唯一の候補（D6 Stooq）は規約を確認できず、実測も見送った。**この束は【公表値】止まりである。**

### A-2. 公的機関・無償（⚠ 実測できたのはここが中心）

| ID | サービス | 種類 | 最小粒度 | 履歴 | 費用 | 認証 | ⚠ 規約・注意 | 出典 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **D10** | **FRED** `fredgraph.csv` | R | L2 日次 | `DGS30` **12,925 行 1977-02-15〜**、`DFII10` **6,173 行 2003-01-02〜** | 無償 | **不要** | ⚠ **系列によって履歴が切られている**（`BAMLH0A0HYM2` は 2023-09-04〜の 793 行しか返らない）。`GOLDPMGBD228NLBM` は **404 = 配信停止** | 2026-09-01 取得【実測】／[FRED API 規約](https://fred.stlouisfed.org/docs/api/terms_of_use.html) |
...
L4・L5 に落ちた **22 件**と、未確認の 3 件について、代替の経路を 1 行ずつ書く。
...
### B-3. 代替が無く、L5 で確定するもの

⚠ **「探しきれなかった」と「無い」を分ける。**下記のうち、**運営・制度そのものが「無い」と言っているもの**だけを確定として扱い、残りは「未確認」と書いた。

| 対象 | 判定 | 根拠 |
| --- | --- | --- |
| 協同組合の持分（14-11-10） | **L5 確定** | §14 が「売買できる持分ではない」と書いている。板も約定も存在しない |
| I Bonds / EE Bonds の約定（14-3-3・4） | **L5 確定** | 板が無いので約定が存在しない。取れるのは半期の利率改定（L3） |
| IPv4 の移転価格（14-8-2） | **L5 確定** | **ARIN の移転ログに価格フィールドが無いことを実測で確認**。価格は仲介業者の集計しかない |
| 音楽印税の二次市場（14-8-1） | **L5 確定** | 運営自身が「二次市場は無いに等しい」と明記（§14-8） |
...
| ブローカード CD の二次流通（14-3-15） | **未確認** | TRACE 対象外であることは確認したが、他の公開源を探しきれていない |
| 住宅ローン債権（14-10-3）・YieldStreet（14-10-6）・貸付（14-11-9）・返品在庫（14-11-3）・貸株（14-11-8）・変額年金（14-4-2） | **未確認** | 探した範囲では公開の時系列が見つからなかった。⚠ **L5 と断定していない** |
...
**2026-12-14 以降は、この行を L5（データなし）として扱う。**§0 の件数は調査日（2026-09-01）時点のもので、この日を過ぎると **L4 が 8 → 7 件、L5 が 14 → 15 件**になる。

⚠ **ただし L5 確定（付録 B-3）とは区別する。**運営・制度が「無い」と言っているのではなく、**参照経路が時間で失われる**ケースである。SEC EDGAR には各作品シリーズの Reg A+ 書類（1-K / 1-SA）が残るため、二次流通の記述が拾える可能性はある【推測・未確認】。必要になったらそこから当たり直す。
...
| 2 | **粒度に根拠がある** | ✅ 全行の最小粒度に提供元の記述または実測を紐づけた。根拠が無い 3 行は「未確認」と明記（14-4-1・14-4-4・14-9-1） |
| 3 | **⚠ 断定と推測を分ける** | ✅ 実際に取得したのは **7 経路のみ**で【実測】。残りは【公表値】。到達できなかった Stooq・Fiscal Data のパスは【未確認】 |
| 4 | **⚠ L5 の根拠を残す** | ✅ L5 の 14 件のうち、**確定 7 種**（運営・制度自身の記述、または実測でフィールド不在を確認）と**未確認 6 件**を **付録 B-3** で分けた |
