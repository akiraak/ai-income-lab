...
| E8 | 保有資産の譲渡益 | **通過** | ⚠ ただし実現ルールが無いとゲート 3 で L1 に落ちる。⚠ **実現ルールの中身は 2026-09-08 に調べた** → [e8-signal-methods.md](experiments/e8-signal-methods.md)・下の §4-3 |
...
> 2026-09-07: 税の一次情報（IRS・WA DOR・RCW・日本の条文）の記録は [trading-tax.md](trading-tax.md) にまとめた。この節の数値は同書 §2-1・§3-1 と一致する。
> 追加された論点 — WA 州の §1256 の扱い（2026 年〜）、⚠ 2028 年からの州所得税（$1M 超に 9.9%）、予定納税、wash sale の口座またぎ、1099-DA — は同書を見る。
...
### 4-3. ⚠ E8 の「何を計算して売買を決めるか」（2026-09-08 追記）

E8 は §2 のゲートを通るが、**実現ルール（＝ 何を見て売り買いするか）が空**のままだった。
2026-09-08 に、**過去の市場データだけを入力にする手法 93 件**のカタログを作って判定した → [e8-signal-methods.md](experiments/e8-signal-methods.md)。

| 問い | 結果 |
| --- | --- |
| 手法は何件あるか | **93 件**（12 系統）。根拠が **強** は 15 件、うち**方向を当てる行は 4 件だけ** |
| 5 つの反証ゲートを通るか | **4 件**（A4 時系列モメンタム・J1 断面モメンタム ＋ 道具 2 件）。⚠ **残ったのは実質モメンタムだけ** |
| 年 1000%（利用者の目標）に届くか | ⚠ **届かない。** 到達した最良は税引後 **年 22〜37%**【推測】＝ 達成率 0.022〜0.037 |
| E1・E2 と比べてどうか | ⚠ **期待値では上回る**（$22,000〜37,000 対 $2,601〜3,542）が、⚠ **最大ドローダウン 30〜50% を伴い、根拠も 中 対 強**で、**同じ土俵の数字ではない** |
| ⚠ 段 1（現物のみ）なら | **年 8.7%**【推測】＝ ほぼ市場平均。⚠ **週 5〜15 時間を投じる理由が説明できない** |

⚠ **上限を作っているのはコストでも税でもなかった。** 対数成長率が **SR² / 2** で頭打ちになるため、年 1000% は
**コストと税を引いた後のシャープレシオ 2.30 をフルケリーで回し続けること**を要求する。
公表された最良（Medallion の 30 年で年 63.3%）の **5.4 倍**にあたる。

⚠ **副産物**: [market-data-availability.md](market-data-availability.md) が「米国の個別銘柄はどの経路も登録が要る」としていた点について、
**既にある tastytrade の口座からは日足が 1993 年まで取れる**ことが実測で分かった（[E8 の記録 §2-1 データ経路](experiments/e8-signal-methods/data-routes.md)）。
...
> 📊 **この節の一覧をスライドにしたもの: [online-tradable-assets-slides.html](online-tradable-assets-slides.html)**（全 36 枚。vibeboard の `Specs` タブで開く）。⚠ 数値の正は本書側にある。
> ⚠ スライド側は 96 件それぞれに「どんな商品か」の 1 行解説を足してある（本書には無い）。**解説に数値は入れていない**ので、突合すべき値は本書と同じまま。
...
| 小売 FX・CFD | FX 業者 | ⚠ **業者との相対で板が無い。**常時反対売買には応じるが、値段は業者の建値 | — | ⚠ 継続的な判断が要る。⚠ **CFD の米国リテール向け提供可否は未確認**（CFTC / NFA の一次情報を当てていない） |
...
| 候補 | 買う場所 | 売る場所 | 値動き | ⚠ 属性 |
| --- | --- | --- | --- | --- |
| **スニーカー** | **StockX** | **StockX の板**（⚠ **本物の bid/ask を持つ**） | — | ⚠ **売り手数料 9%（数量で 7% まで）＋ 決済 3%** |
| トレーディングカード | **TCGplayer**（eBay 傘下）・StockX | ⚠ **出品型。常設の買い注文があるかは未確認** | — | 上場代替は EBAY |
