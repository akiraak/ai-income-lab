...
| [experiments/tastytrade-api-sample.md](experiments/tastytrade-api-sample.md) | 上で判定「高」の tastytrade の API を実際に動かした記録。6 手順の【実測】と 6 観点（認証・常駐・現在値・往復・制限・SDK）の判定。**2026-09-08 の市場時間に 5 観点が ✅**（残るは営業日を 2 日跨ぐ交換だけ） | 「その経路が本当にプログラムから動くか」を見るとき | 367 |
...
> **2026-09-08 の追記** — tastytrade の API 検証（この体系で唯一の実行を伴う手法）が判定の山を越えた。
> 6 観点のうち **5 つが ✅**（常駐・現在値・発注の往復・レート制限・SDK）。残る「認証の寿命」も、
> g3plus の[管理画面](dashboard.md)が sandbox に繋いで監視を回しているので、翌営業日に自動で埋まる。
> ⚠ **ただしここで測っているのは API の挙動だけで、収入・費用の【実測】ではない**（[§7](#7-読むときの注意) の 2）。
...
| **E8 譲渡益のシグナル**（横断） | **条件付き成立 ／ ⚠ 目標には届かない**（09-08） | 手法 93 件のうち 5 ゲートを通るのは 4 件（実質モメンタム 2 件）。⚠ **目標の年 1000% に対し達成率 0.022〜0.037** | — （09-08 の新規判定） | [e8-signal-methods.md](experiments/e8-signal-methods.md) |
...
**候補 8 件の内訳**: 判定済みの D7（条件付き成立）と **E8 譲渡益**（2026-09-08。条件付き成立だが ⚠ **目標の年 1000% には届かない**）、未判定の 6 件 —
E1 上場株の配当・E2 債券の利子（いずれも L3。時間を使わない資金枠）と、
D1 印税・H4 有料メンバーシップ・B5 受託開発・B14 AI 開発者向け受託。

> **2026-09-08 の追記（E8）** — [e8-signal-methods.md](experiments/e8-signal-methods.md) で「何を計算して売買を決めるか」を 93 件のカタログにして判定した。
> 到達した最良は税引後 **年 22〜37%**【推測】で、E1（$2,601）・E2（$3,542）を期待値では上回るが、⚠ **最大ドローダウン 30〜50% を伴い根拠も 中 対 強**である。
> ⚠ **上限を作っているのはコストでも税でもなく、対数成長率が SR² / 2 で頭打ちになること**だった。
