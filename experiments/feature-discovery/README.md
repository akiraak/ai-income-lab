...
| `own_impact_2018` ／ `impact_ex_2018` ／ `impact_2018` ／ `impact_placebo_2018` ／ `impact_both_2018` | 35 ／ 59 ／ 47 ／ 47 ／ 59 | — | ⚠ **災害を銘柄へ割り当てると効くか。** ⚠ **偽薬は「割り当ての入れ替え」。** ⚠ **割り当ては偽薬と区別できず、割り当てなしは保留**（記録 §13）。⚠ **割り当てなしは日付を過去へずらした偽薬（`--shift-days`）と比べる**（記録 §15） |
