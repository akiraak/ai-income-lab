...
def build() -> str:
    ...
    a("")
    a(f"⚠ **多重検定に使う `n_trials` は {len(methods)}**（= 手法の行数。基準線 "
      f"{len(rows) - len(methods)} 行は数えない）。⚠ **[rules.md 11 章](rules.md) の規約 4 は"
    ...
    a("## 1. 台帳の読み方")
    a("")
    a("⚠ **1 行 = 1 回の試行。** 鍵は **手法 × 粒度 × 地平 × 特徴量の層 × データの層** である"
    ...
    a("")
    head = ["ID", "手法", "系統", "実装", "層", "粒度", "地平", "特徴量の層", "本数",
    ...
    for r in rows:
        ...
        body.append([r["ID"] or "—", r["手法"], r["系統"], r["実装"], r["層"], r["粒度"],
                     r["地平"], r["特徴量の層"] or "—", _f(r["本数"], 1), _f(r["的中率"]),
    ...
    a("")
    L += _table(["実行", "出所", "粒度", "層", "行", "特徴量", "銘柄", "種", "commit",
                 "入力の指紋", "先読み"],
                ["---", "---", "---", ":-:", "---:", "---:", "---:", ":-:", "---", "---", ":-:"],
                [[f"`{r['実行']}`", r["出所"], r["粒度"] or "—", r["層"] or "—",
    ...
    a("")
    if d["leak"]:
        L += _table(["ID", "手法", "層", "粒度", "的中率", "IC", "粗利bp", "純利bp", "実行"],
                    ["---", "---", ":-:", "---", "---:", "---:", "---:", "---:", "---"],
                    [[r["ID"] or "—", r["手法"], r["層"], r["粒度"], _f(r["的中率"]),
