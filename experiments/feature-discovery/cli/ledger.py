...
def build() -> str:
    ...
    a("")
    head = ["ID", "手法", "系統", "実装", "モデル", "層", "期間", "粒度", "地平", "特徴量の層",
            "検証方式", "形式", "較正", "閾値", "本数",
    ...
    for r in rows:
        ...
        body.append([r["ID"] or "—", r["手法"], r["系統"], r["実装"],
                     r.get("モデル") or "—", r["層"], r.get("期間") or "—", r["粒度"],
                     r["地平"], r["特徴量の層"] or "—",
                     r.get("検証方式") or "毎日往復", r.get("形式") or "共通",
                     r.get("較正") or "—", r.get("閾値") or "—",
                     _f(r["本数"], 1), _f(r["的中率"]),
    ...
    if untouched:
        a(f"⚠ **{' と '.join(untouched)} は 1 件も触っていない。** "
          "⚠ **「特徴量を選ぶ」手法に偏っており、「特徴量を作る」側がほとんど空である。**")
    else:
        a("⚠ **どの系統も 1 件以上は回した。** ⚠ **ただし「回した」は「効いた」ではない**"
          "（判定の列を見ること）。")
    a("")
